...
$sql = "insert into " . $dbprefix . "asaleh (id,  name_t,sano, sadate,  mcode,sp_code,  sa_type, inv_code,  total,bprice,customer_total, tot_pv, uid,remark,txtInternet,chkInternet,txtTransfer,chkTransfer,txtCash,chkCash,txtCredit1,chkCredit1 ,send,scheck,lid,checkportal,optionCash,optionCredit1,optionTransfer,locationbase,name_f,mbase,crate,cprovinceId,camphurId,cdistrictId,czip,caddress) values ('$mid' ,'$name_t','$sano' ,'$sadate' ,'$mcode','$sp_code', '$satype' ,'$inv_code' ,'$total11' ,'$btotal11','$ctotal11' ,'$tot_pv11' ,'" . $sp_code . "','','$txtInternet','$chkInternet','$txtTransfer','$chkTransfer','$txtCash','$chkCash','$txtCredit1','$chkCredit1','$sletter','register','" .$sp_code . "','3','$optionCash','$optionCredit1','$optionTransfer','" . '1' . "','$name_f','$locationbase','" . '1' . "','$province','$amphur','$district' ,'$zip','$address') ";                        //====================LOG===========================
...
//        writelogfile($text);
    ...
    for ($i = 0; $i < sizeof($pcode1); $i++) {
        $sql = "insert into " . $dbprefix . "asaled (sano,inv_code,pcode,pdesc,price,bprice,customer_price,pv,qty,amt,locationbase) values ('$mid','','$pcode1[$i]','$pdesc1[$i]','$price[$i]','$bprice[$i]' ,'$cprice[$i]' ,'$pv[$i]','$qty[$i]','$totalprice[$i]','" . '1' . "') ";
        //====================LOG===========================
//            $text="uid=".$_SESSION["usercode"]." action=saleoperate =>$sql";
//            writelogfile($text);
        //=================END LOG===========================
...
//    $sqlUpdate1 = "update " . $dbprefix . "member set ewallet = ewallet-'$total11', bewallet = bewallet-'$total11' WHERE mcode='{$sp_code}' ";
//    $rs = mysql_query($sqlUpdate1);
//
//    $option = "register ($sano)";
//    log_ewallet('ewallet', $sp_code, $sano, $total11, 'O', date("Y-m-d"), $option);
//}
