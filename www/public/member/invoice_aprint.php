...
$sql .= " LEFT JOIN ".$dbprefix."invent ON (".$dbprefix."asaleh.inv_code=".$dbprefix."invent.inv_code) WHERE $wherexx 
 and (".$dbprefix."asaleh.mcode = '".$_SESSION["usercode"]."' or ".$dbprefix."asaleh.uid = '".$_SESSION["usercode"]."') ";
...
if(mysql_num_rows($rs)<=0){ 
    ?><table width="300" align="center" bgcolor="#990000"><tr><td align="center">Not Found <?=$sano?>
    <br /><input type="button" value="Close" onClick="window.close()" /></td></tr></table><?
    exit;
}
...
for($i=0;$i<sizeof($bill);$i++){
    ...
    $pdf->Image('../logo.jpg',$offsetx+$offsettab-10,$offsety+$offsetnline,30); //file,x,y,w=0,h=0
    ...
    $pdf->Cell($offsettab,10,"ใบเสร็จรับเงิน/ใบกำกับภาษี",0,0,"C"); 
    //$pdf->SetY($offsety+3*$offsetnline);
...
//function MultiCell($w,$h,$txt,$border=0,$align='J',$fill=0)
    ...
    $pdf->SetX($offsetx+(2*$offsettab)+5);
    $string = preg_replace("/[\\n\\r]+/", "",$remark[$i]);
    $pdf->MultiCell((7*$offsettab),4,$string,0,"L"); 
...
//------------------------------------------
    ...
    if($k==$npage-1){
        ...
        $pdf->SetY($offsety+(22*$offsetnline)+7);
        $pdf->SetX($offsetx+(9*$offsettab)+3);
        $pdf->Cell($offsettab,10,number_format(($sum-$sum*100/107),2,'.',','),0,0,"R"); 

        $pdf->SetY($offsety+(22*$offsetnline)+14);
        $pdf->SetX($offsetx+(9*$offsettab)+3);
        $pdf->Cell($offsettab,10,number_format(($sum*100/107),2,'.',','),0,0,"R"); 
