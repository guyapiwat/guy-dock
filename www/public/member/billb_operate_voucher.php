...
if($mcode==""){
	echo "<script language='JavaScript'>alert('".$wording_lan['tab4']['1_74']."');window.location='index.php?sessiontab=3&sub=23&state=1'</script>";	
	exit;
}
$arr_member = searchmember($dbprefix,$mcode);
if($arr_member["locationbase"] != $_SESSION["m_locationbase"]){
	echo "<script language='JavaScript'>alert('".$wording_lan["operate"]["3"]."');window.location='index.php?sessiontab=3&sub=23&state=1'</script>";	
	exit;
}
...
if($radsend == '1' and $GLOBALS["sending"] == '1' ){
	...
	if($total > $showvoucher1){
		...
		echo "<script language='JavaScript'>alert('".$wording_lan["operate"]["77"]."');window.location='index.php?sessiontab=3&sub=23&state=1'</script>";	
	...
	if($_SESSION["m_locationbase"] == '0'){
		echo "<script language='JavaScript'>alert('Problem Please Try Again!');window.location='index.php?sessiontab=3&sub=23&state=1'</script>";	
		exit;
	}

	$sql="insert into ".$dbprefix."asaleh (id,  sano, sadate,  mcode,  sa_type, inv_code,  total,customer_total, tot_pv,tot_sppv,tot_weight,tot_fv, uid,send,txtoption,chkDiscount,txtVoucher,trnf,name_t,caddress,cdistrictId,camphurId,cprovinceId,czip,online,checkportal,hpv,htotal,hdate,locationbase,bprice,name_f,mbase,cname,cmobile,crate ) values ('$mid' ,'$sano' ,'$sadate' ,'$mcode', '$satype' ,'$inv_code' ,'$total','$tot_cus' ,'$tot_pv','$tot_sppv','$tot_weight','$tot_fv' ,'".$_SESSION['usercode']."','$radsend','$txtoption','on','$total','$trnf','$name_t','$caddress','$cdistrict','$camphur','$cprovince','$czip','1','3','$tot_pv','$total','$hdate','".$_SESSION["m_locationbase"]."','$tot_base','$name_f','".$arr_member["locationbase"]."','$cname','$cmobile','".$_SESSION["m_crate"]."') ";

	if (! mysql_query($sql)) {
		echo "<script language='JavaScript'>alert('".$wording_lan["operate"]["6"]."')window.location='index.php?sessiontab=3&sub=23&state=1'</script>";
		exit;
	}
	logtext(true,$_SESSION['usercode'],'voucher product',$total);
	//====================LOG===========================
	$text="uid=".$_SESSION["usercode"]." action=saleoperate =>$sql";
	writelogfile($text);
	//=================END LOG===========================
	//minusEwallet($dbprefix,$_SESSION['usercode'],$total,$tot_base);
	minusVoucher($dbprefix,$_SESSION['usercode'],$total,$tot_base);
	...
	for($i=0;$i<sizeof($pcode);$i++){
		...
		$sql="insert into ".$dbprefix."asaled (sano,pcode,pdesc,price,bprice,customer_price,pv,sppv,weight,fv,qty,amt,inv_code,locationbase,vat) values ('$mid','$pcode[$i]','$pdesc[$i]','$price[$i]' ,'$bprice[$i]' ,'$cprice[$i]' ,'0','$special_pv[$i]','$weight[$i]','$fv[$i]','$qty[$i]','$totalprice[$i]','$inv_code','".$_SESSION["m_locationbase"]."','$vat') ";
		//====================LOG===========================
