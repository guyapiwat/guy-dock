...
$sql = "SELECT pos_cur,name_t,mdate,caddress,czip,cdistrictId,camphurId,cprovinceId,pos_cur,name_f,sp_code,mtype1,status_terminate,mobile from ".$dbprefix."member WHERE mcode='$mcode' ";
$rs = mysql_query($sql);
$pos_old = '';
if(mysql_num_rows($rs)>0) {
	...
	if($cmobile == '')$cmobile = mysql_result($rs,0,'mobile');
}
...
if($satype == "H"){
	if($status_terminate == '1'){
		echo "<script language='JavaScript'>alert('".$wording_lan["tab1_mem_110"]."');window.history.back();</script>";	
        exit;
	}
