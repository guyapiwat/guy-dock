...
if(	$ftrcode != '')$wwhere = " and a.rcode between '".$ftrc[0]."' and '".$ftrc[1]."' ";
		...
		$sql = "SELECT *,a.total as total1, a.mcode as mcode1,m.pos_cur as poscur FROM ".$dbprefix."sapv a left join ".$dbprefix."member as m on (a.mcode=m.mcode) ";
		...
		$rec->setLink($PHP_SELF,"sessiontab=5&sub=50&ftrcode=$ftrcode&fmcode=$cmc");
		$rec->setBackLink($PHP_SELF,"sessiontab=5");
		if(isset($page))
			$rec->setCurPage($page);
		$rec->setShowField("rcode,fdate,tdate,mcode1,name_t,poscur,total_pv,percen,total1");
	//	$rec->setFieldDesc("".$wording_lan["rcode"].",".$wording_lan["mcode"].",".$wording_lan["name"].",".$wording_lan["sp_code"].",".$wording_lan["PV"].",".$wording_lan["Amount"].",".$wording_lan["uid"].",".$wording_lan["type"]."");
		$rec->setFieldDesc("".$wording_lan["rcode"].",".$wording_lan["tab5"]['2_2'].",".$wording_lan["tab5"]['2_3'].",".$wording_lan["mcode"].",".$wording_lan["name"].",".$wording_lan["tab2_1_8"].",".$wording_lan["PV"].",%".$wording_lan["Bonus"].",".$wording_lan["Bonus"]."");
		$rec->setFieldAlign("center,center,center,center,left,center,right,right,right");
		$rec->setFieldSpace("5%,10%,10%,10%,25%,10%,10%,10%,15%");//10
