...
if ($cmc=="") {
	$cmc=$_SESSION["usercode"];
		...
		$sql = "SELECT DATE_FORMAT(a.fdate, '%Y-%m-%d') as fdate,a.rcode, a.mcode,m.name_t,m.acc_no,m.bankcode,b.bankname,a.total,a.tax,a.bonus ";
		...
		$rec->setLink($PHP_SELF,"sessiontab=5&sub=11&ftrcode=$ftrcode&fmcode=$cmc");
		$rec->setBackLink($PHP_SELF,"sessiontab=5");
		if(isset($page))
			$rec->setCurPage($page);
		$rec->setShowField("fdate,mcode,name_t,acc_no,bankname,total,tax,bonus");
		$rec->setFieldDesc("ÇÑ¹·Õè,ÃËÑÊÁÒªÔ¡,ª×èÍ,àÅ¢ºÑ­ªÕ,¸¹Ò¤ÒÃ,âº¹ÑÊÂÙ¹ÔàÅàÇÅ,ÀÒÉÕ (5%),ÊØ·¸Ô");
		...
		$rec->setFieldLink(",,,,,index.php?sessiontab=5&sub=8&fdate=,,");
