...
$sql="SELECT * FROM ".$dbprefix."member ORDER BY lr DESC";
	...
	for($i=0;$i<sizeof($mcode);$i++){
		...
		$sql = "SELECT tab.sadate as sadate,tab.sano as sano,a.rcode,a.mcode,taba.name_t,a.fdate,a.upa_code,tabb.upa_name,tabb.pos_cur,CASE a.lr WHEN '1' THEN tab.hpv ELSE '0' END AS lcl,CASE a.lr WHEN '2' THEN tab.hpv ELSE '0' END AS lcc,CASE a.lr WHEN '3' THEN tab.tot_pv ELSE '0' END AS lcr,tab.htotal,
		...
		$rec->setFieldSpace("9%,9%,56%,9%,9%,9%,9%,9%,10%");//10
