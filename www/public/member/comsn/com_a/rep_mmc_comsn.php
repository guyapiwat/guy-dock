...
if(	$ftrcode != '')$wwhere = " and a.rcode between '".$ftrc[0]."' and '".$ftrc[1]."' ";
		...
		$sql = "SELECT *,a.total as total1,a.mcode as mcode1 FROM ".$dbprefix."mmc a left join ".$dbprefix."member as m on (a.mcode=m.mcode) ";
...
//echo $sql;
		...
		$rec->setLink($PHP_SELF,"sessiontab=5&sub=130&ftrcode=$ftrcode&fmcode=$cmc");
		$rec->setBackLink($PHP_SELF,"sessiontab=4");
		if(isset($page))
			$rec->setCurPage($page);
		$rec->setShowField("rcode,fdate,tdate,mcode1,name_t,sp_code,total1");
	//	$rec->setFieldDesc("".$wording_lan["rcode"].",".$wording_lan["mcode"].",".$wording_lan["name"].",".$wording_lan["sp_code"].",".$wording_lan["PV"].",".$wording_lan["Amount"].",".$wording_lan["uid"].",".$wording_lan["type"]."");
		$rec->setFieldDesc("".$wording_lan["rcode"].",".$wording_lan["tab5"]['2_2'].",".$wording_lan["tab5"]['2_3'].",".$wording_lan["mcode"].",".$wording_lan["name"].",".$wording_lan["tab4"]["3_9"].",Key\nregister");
		$rec->setFieldAlign("center,center,center,center,left,center,right,right");
		$rec->setFieldSpace("9%,10%,12%,10%,30%,15%,15%,15%,15%");//10
