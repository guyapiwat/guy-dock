...
if(	$ftrcode != '')$wwhere = " and a.rcode between '".$ftrc[0]."' and '".$ftrc[1]."' ";
		...
		$sql = "SELECT *,a.total as total1,a.mcode as mcode1 FROM ".$dbprefix."dc a left join ".$dbprefix."member as m on (a.mcode=m.mcode) ";
...
//echo $sql;
		...
		$rec->setLink($PHP_SELF,"sessiontab=5&sub=3&ftrcode=$ftrcode&fmcode=$cmc");
		$rec->setBackLink($PHP_SELF,"sessiontab=4");
		if(isset($page))
			$rec->setCurPage($page);
		$rec->setShowField("rcode,mcode1,name_t,sp_code,pv,level,gen,percer,total1");
	//	$rec->setFieldDesc("".$wording_lan["rcode"].",".$wording_lan["mcode"].",".$wording_lan["name"].",".$wording_lan["sp_code"].",".$wording_lan["PV"].",".$wording_lan["Amount"].",".$wording_lan["uid"].",".$wording_lan["type"]."");
		$rec->setFieldDesc("".$wording_lan["rcode"].",".$wording_lan["mcode"].",".$wording_lan["name"].",".$wording_lan["sp_code"].",ร้ายได้W/S,".$wording_lan["lv"].",".$wording_lan["gen"].",%โบนัส,".$wording_lan["Bonus"]."");
		$rec->setFieldAlign("center,center,left,center,right,center,center,right,right");
		$rec->setFieldSpace("4%,10%,30%,10%,10%,8%,8%,8%,12%");//10
