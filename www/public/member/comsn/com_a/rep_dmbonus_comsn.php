...
if (isset($_GET["ftrcode"])){$fdate=$_GET["ftrcode"];}else{$fdate="";}
		...
		$sql = "SELECT a.fdate as fdate,a.tdate as tdate,a.rcode, a.mcode as mcode,m.name_t,m.acc_no,m.bankcode,b.bankname,a.total*1 as total,a.tax*1 as tax,a.bonus*1 as bonus,1";
		...
		$rec->setFieldDesc($wording_lan["cdate"].",".$wording_lan["mcode"].",".$wording_lan["name"].",".$wording_lan["commission"]."");
		$rec->setFieldAlign("center,center,left,right");
	//	$rec->setFieldSpace("10%,10%,30%,10%,20%,10%,10%,10%");//10
		$rec->setSum(true,false,",,,true");
		$rec->setFieldFloatFormat(",,,2,2,2");
		$rec->setFieldLink(",,,index.php?sessiontab=5&sub=3&ftrcode=,,");
