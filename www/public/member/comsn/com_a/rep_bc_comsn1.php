...
if ($cmc=="") {
	$cmc=$_SESSION["usercode"];
		...
		$sql = "SELECT tab.sadate as sadate,tab.sano as sano,a.rcode,a.mcode,taba.name_t,a.fdate,a.upa_code,tabb.upa_name,tabb.pos_cur,CASE a.lr WHEN '1' THEN tab.tot_pv ELSE '0' END AS lcl,CASE a.lr WHEN '2' THEN tab.tot_pv ELSE '0' END AS lcc,CASE a.lr WHEN '3' THEN tab.tot_pv ELSE '0' END AS lcr,tab.total,
		...
		$sql .= " LEFT JOIN (SELECT mcode,rcode ,fdate,upa_code,pv,lr FROM ".$dbprefix."asaleh_bm) AS a ON (a.mcode=tab.mcode  ) where tab.tot_pv >0 and cancel = 0  and (tab.sa_type = 'A' or tab.sa_type = 'Q' or tab.sa_type = 'C') ";
		if($fdate !=""){
		...
		$sql .= " UNION SELECT tab.sadate as sadate,tab.hono as sano,a.rcode,a.mcode,taba.name_t,a.fdate,a.upa_code,tabb.upa_name,tabb.pos_cur,CASE a.lr WHEN '1' THEN tab.tot_pv ELSE '0' END AS lcl,CASE a.lr WHEN '2' THEN tab.tot_pv ELSE '0' END AS lcc,CASE a.lr WHEN '3' THEN tab.tot_pv ELSE '0' END AS lcr,tab.total,
...
//echo $sql;
		...
		$rec->setLink($PHP_SELF,"sessiontab=5&sub=222&cmc=$cmc&lr=$lr&fdate=$fdate");
