...
if ($cmc=="") {
	$cmc=$_SESSION["usercode"];
		...
		$sql = "SELECT c.rcode,c.mcode,m.name_t,c.pv,c.percer,c.total ";
		$sql .= " FROM ".$dbprefix."pc  c ";
		...
		$rec->setLink($PHP_SELF,"sessiontab=5&sub=10&ftrcode=$ftrcode&fmcode=$cmc");
		$rec->setBackLink($PHP_SELF,"sessiontab=5");
		if(isset($page))
			$rec->setCurPage($page);
		$rec->setShowField("rcode,mcode,name_t,pv,percer,total");
		$rec->setFieldDesc("ÃËÑÊÃÍº,ÃËÑÊÊÁÒªÔ¡,ª×èÍ,¤Ğá¹¹,%âº¹ÑÊ,ä´é,");
		$rec->setFieldAlign("center,center,left,right,center,right");
		$rec->setFieldSpace("15%,15%,25%,15%,15%,15%");
