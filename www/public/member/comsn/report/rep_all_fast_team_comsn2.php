...
<?
	...
	require("connectmysql.php");
		...
		$sql = "select m.rcode,m.fdate,m.tdate,m.dmbonus,m.embonus,m.total as alltotal ";
		$sql .= " from ".$dbprefix."commission_b m
		left join ".$dbprefix."bround r on (m.rcode = r.rcode)
		";
		...
		$rec->setLink($PHP_SELF,"sessiontab=5&sub=1033&ftrcode=$ftrcode&fmcode=$fmcode");
		...
		$rec->setShowField("rcode,fdate,tdate,dmbonus,embonus,alltotal");
		$rec->setFieldFloatFormat(",,,2,2,2");
		$rec->setFieldDesc("".$wording_lan["rcode"].",".$wording_lan["tab5"]['2_2'].",".$wording_lan["tab5"]['2_3'].",Matching,AllSale,Total");	
		$rec->setFieldAlign("center,center,center,right,right,right,right,right");
		
		$rec->setSum(true,false,",,,true,true,true,true,true");

		$rec->setShowMobile(true,"true,false,false,true,true,true,true");
