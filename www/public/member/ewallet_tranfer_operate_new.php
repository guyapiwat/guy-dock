...
$selectch = "select * from  ".$dbprefix."member where mcode = '$mcode'";
...
if($_SESSION["usercode"] == $mcode and $spayment=='1'){
...
if($_SESSION["usercode"] != $mcode and $spayment=='2'){
    echo "<script language='JavaScript'>alert('เป็นรหัสตัวเองเท่านั้น'); window.history.back()</script>";     
    exit;
}
...
{              
    echo "<script language='JavaScript'>alert('Not Enough $test !! please try again'); window.history.back()</script>";     
    exit;
}
...
if($_GET['state']==0){ 
	if($spayment == '1'){   //// Tranfer  
       ...
       // $sano = gencodesale_ewallet('EO',$table);
        ...
        $sql = "SELECT MAX(id) AS id ,MAX(sano) as sano FROM  ".$dbprefix.$table1." ";
        $rs = mysql_query($sql);
        $mid = mysql_result($rs,0,'id')+1;
        $sano = gencodesale_news('E',"ali_".$table1,"ONLIN",$data['location_base']);
        mysql_free_result($rs);
        $insert = array(                    
            "id"             => $mid,
            "sano"           => $sano,
        ...
        insert($dbprefix.$table1,$insert); 
          
        mysql_query("update ".$dbprefix."member set $table = $table-'".$txtMoney."' where mcode='".$_SESSION['usercode']."' ");
        mysql_query("update ".$dbprefix."member set $table1 = $table1+'".$txtMoney."' where mcode='".$mcode."' ");
        ...
        log_ewallet($table1,$mcode,$sano,$txtMoney,'TI',$sadate,$optionx);
		echo "<script language='JavaScript'>window.location='index.php?sessiontab=3&sub=2';</script>";    
		exit;
    }
    ...
    if($spayment == '2'){   //// Tranfer 
       ...
       // $sano = gencodesale_ewallet('EO',$table);
        ...
        $option = "transfer to ".$mcode."(".$sano.")";
        log_ewallet($table,$_SESSION['usercode'],$sanoecom,$txtMoney,'TO',$sadate,$option);  
        $optionx = "transfer from ".$_SESSION['usercode']."(".$sanoecom.")";    
        log_ewallet($table1,$mcode,$sano,$txtMoney,'TI',$sadate,$optionx);
		echo "<script language='JavaScript'>window.location='index.php?sessiontab=3&sub=5';</script>";    
		exit;
    }
    ...
    if($spayment == '3'){  ///    
        ...
        $rs = mysql_query($sql);
        $mid = mysql_result($rs,0,'id')+1;
        $sano = gencodesale_news('W',"ali_".$table,"ONLIN",$data['location_base']);  
        $sanoewallet = $sano;
        mysql_free_result($rs); 
