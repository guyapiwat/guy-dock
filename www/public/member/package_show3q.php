...
function chknum(key){
	...
	function saleadd(pcode,pdesc,price,pv){
		...
		place += "</tr>";
		place += "<tr><td colspan='9' align='right'><input name='button' type='button' onclick='<?=(isset($_GET['id'])?"ebillcheck()":"ibillcheck()")?>' value='ตรวจสอบ' />&nbsp;<input type='submit' value='บันทึก' name='ok' id='ok' disabled='disabled' />&nbsp;<input name='reset' type='reset'  onclick=\"window.location='index.php?sessiontab=4&sub=1'\" value='ยกเลิก' /></td></tr>";
		place += "</table>";
...
$sql = "SELECT *,'0' as totpv1 FROM ".$dbprefix."product_package where st = 1  and locationbase = '".$_SESSION["m_locationbase"]."' and sa_type = 'Q' ";
...
if(isset($page))
	$rec->setCurPage($page);
$rec->setShowField("pcode,pdesc,price,totpv1");
...
$rec->setSpecial("./images/add_pic.gif","","saleadd","pcode,pdesc,price,totpv1","IMAGE","");
