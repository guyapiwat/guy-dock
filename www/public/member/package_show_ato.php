...
function chknum(key){
	...
	function saleadd(pcode,pdesc,price,pv,bv,fv){
		...
		place += "<td style='"+style_l+style_t+style_b+hidden1+"'>ราคา</td>";
		...
		place += "<td style='"+style_l+style_t+style_b+"' align='right' colspan='5'>รวม</td>";
...
$sql = "SELECT * FROM ".$dbprefix."product_package where  price <> 0 and ato = 1";
...
$rec->setFieldDesc("รายละเอียด,ราคา,PV");
