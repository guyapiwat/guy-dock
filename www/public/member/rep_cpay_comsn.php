...
<?
		...
		$sql = "SELECT ".$dbprefix."cmbonus.rcode, ".$dbprefix."cmbonus.mcode,name,acno,bankname,sum(total) as tot";
		$sql .= ",(sum(total)*5)/100 as tax,(sum(total)-(sum(total)*5)/100) as totall ";
		$sql .= "FROM ".$dbprefix."cmbonus ";
		$sql .= "LEFT JOIN (select mcode as mc,acc_name as name,acc_no as acno,bankcode as bcode from ".$dbprefix."member) AS taba ON (".$dbprefix."cmbonus.mcode=taba.mc) ";
		$sql .= "LEFT JOIN (SELECT bankcode AS bc,bankname FROM ".$dbprefix."bank) AS tbbank ON (taba.bcode=tbbank.bc) ";
		$sql .= " group by ".$dbprefix."cmbonus.rcode,".$dbprefix."cmbonus.mcode ";
		$sql .= "HAVING sum(total)>0";
		if($ftrcode!=""&&$smc!=""){
			$sql .= " AND rcode between '".$ftrc[0]."' and '".$ftrc[1]."' and ".$dbprefix."cmbonus.mcode='".$smc."'";
		}else if($ftrcode!=""){
			$sql .= " AND rcode between '".$ftrc[0]."' and '".$ftrc[1]."'";
		}else if($smc!=""){
			$sql .= " AND ".$dbprefix."cmbonus.mcode='".$smc."'";
		}
		...
		$rec->setLink($PHP_SELF,"sessiontab=5&sub=6&ftrcode=$ftrcode");
		$rec->setBackLink($PHP_SELF,"sessiontab=4");
		if(isset($page))
			$rec->setCurPage($page);
		//$rec->setShowIndex(true);
		$rec->setShowField("rcode,mcode,name,acno,bankname,tot,tax,totall");
		$rec->setFieldDesc("รอบ,รหัสมาชิก,ชื่อ,เลขบัญชี,ธนาคาร,ค่าแนะนำ,ภาษี 5%,สุทธิ");
...
<form name="rform" method="post" action="./index.php?sessiontab=5&sub=6">
