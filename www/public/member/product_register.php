...
function chknum(key){
	...
	function saleadd(pcode,pdesc,price,pv){
		...
		place += "</tr>";
		//alert(tag.length);
		//alert(Math.floor((tag.length-skip)/9));
		//alert(tag.length);
		for(i=0;i<Math.floor((tag.length-skip)/8);i++){
			showprice = 0;
			showpv = 0;
			step = i*8+bgskip;
			place += "<tr>";
			place += "<td style='"+style_l+style_bd+"' align='center'>"
			place += "<input type='button' value='ลบ' onclick=\"saledel('" + tag[step].value + "','" + tag[step].value + "','" + tag[step].value + "','" + tag[step].value + "')\"></td>";
			...
			place += "<td style='"+style_l+style_bd+"' align='right'><input onkeyup='cal()' style='text-align:right;' name='qty[]' type='text' size='5' value='" + num + "'  onKeyPress='return isNumberKey(event)' onChange=\"if(this.value == '' || this.value.substring(0,1) == '0' ){alert('ไม่สามารถใส่ค่า 0 00 ช่องว่างได้');this.value=1;cal();}\">";
			step++;
		...
		if(out){
			...
			place += "<td style='"+style_l+style_bd+"' align='right'><input onkeyup='cal()' name='qty[]' style='text-align:right;' type='text' size='5' value='1'  onKeyPress='return isNumberKey(event)' onChange=\"if(this.value == '' || this.value.substring(0,1) == '0'){alert('ไม่สามารถใส่ค่า 0 หรือ ช่องว่างได้');this.value=1;cal();}\"></td>";
...
$sql = "SELECT * FROM ".$dbprefix."product where st = 1 and price > 0  and locationbase = '".$_SESSION["m_locationbase"]."' ";
