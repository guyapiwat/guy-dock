...
for($i=0;$i<mysql_num_rows($rs);$i++){
	...
	$bill[$i] = $obj->id;
	$bill1[$i] = $obj->sano;
$sqlLog1 = "SELECT sys_id,logdate,logtime FROM ".$dbprefix."log  WHERE object ='".$bill1[$i]."' and subject = 'เพิ่มบิล' order by id desc";
$sqlLog2 = "SELECT sys_id,logdate,logtime  FROM ".$dbprefix."log  WHERE object ='".$bill1[$i]."' and subject = 'แก้ไขบิล' order by id desc";
...
for($i=0;$i<sizeof($bill);$i++){
	...
	$pdf->SetX($offsetx+(6.5*$offsettab));
	...
	$pdf->Cell((2*$offsettab),10,"ใบเสร็จรับเงินออนไลน์",1,0,"C"); 
	//$pdf->SetY($offsety+(6*$offsetnline));
