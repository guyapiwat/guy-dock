...
function chknum(key){
	...
	function saleadd(pcode,pdesc,price,pv){
		...
		window.parent.document.getElementById('checkstate').innerHTML = "<font color='#FFFFFF' style='background:#990000'> &nbsp;<?=$wording_lan['tab']['1_41'] ?>&nbsp; </font>";
		//alert(tag.length);
		...
		for(i=0;i<(tag.length-skip)/8;i++){
			...
			place += "<td style='"+style_l+style_bd+"' align='right'><input onkeyup='cal()' style='text-align:right;' name='qty[]' type='text' size='5' value='" + num + "'   onKeyPress='return chknum(window.event.keyCode)' onChange=\"if(this.value == '' || this.value.substring(0,1) == '0'){alert('<?=$wording_lan['tab4']['1_40']?>');this.value=1;cal();}\"></td>";
			step++;
		...
		//alert(tag.length)
			...
			place += "<td style='"+style_l+style_bd+"' align='right'><input onkeyup='cal()' name='qty[]' style='text-align:right;' type='text' size='5' value='1'  onKeyPress='return chknum(window.event.keyCode)' onChange=\"if(this.value == '' || this.value.substring(0,1) == '0'){alert('<?=$wording_lan['tab4']['1_40']?>');this.value=1;cal();}\"></td>";
...
$sql = "SELECT pcode,pdesc,price,'0' as pv FROM ".$dbprefix."product where price <> 0  and (st = 1 or sst = '1')  and locationbase = '".$_SESSION["m_locationbase"]."' ";
...
$rec->setSearchDesc($wording_lan["productshow"]["1"].",".$wording_lan["productshow"]["2"].",".$wording_lan["productshow"]["3"].",".$wording_lan["productshow"]["4"].",".$wording_lan["productshow"]["5"]."");
