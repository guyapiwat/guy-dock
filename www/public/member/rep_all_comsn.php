...
<?
		...
		sumabonus($dbprefix,$cmc,$ftrc[0],$ftrc[1]);
		...
		$rec->setLink($PHP_SELF,"sessiontab=5&sub=100&ftrcode=$ftrcode&fmcode=$cmc");
...
function sumabonus($dbprefix,$amcode,$srcode,$trcode){
	...
	/*	$sql2="SELECT mcode,rcode,sum(total) as tot_pv FROM ".$dbprefix."bmpersonal ";
		$sql2 .= $wheresql." GROUP BY mcode,rcode ";
		//echo "$sql2<BR>";
		$rs2 = mysql_query($sql2);
		for($i=0;$i<mysql_num_rows($rs2);$i++){
