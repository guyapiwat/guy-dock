			...
			case 'SH_RESULT':
				...
				if(mysql_num_rows($this->rs)<=0){
					...
					if($this->pageLinkShow['top']){
						...
						echo "แสดง ".($this->cPage==$this->nPage?$this->nInfo-(($this->nPage-1)*$this->lPage):$this->lPage)." รายการ จาก ".$this->nInfo." รายการ หน้าละ <input name='lp' type='text' size='1' style='text-align:right;font-size:10px;background-color: #CCCCCC;border: 1px solid #000000;' onKeyUp='pageset(window.event.keyCode)' title='Enter เพื่อปรับแต่งการแสดงผล' value='".$this->lPage."'> ข้อมูล</font></div>";
						$this->pageShow();
						echo "</td></tr>";
					}
						...
						if($info = mysql_fetch_array($this->rs)){
							...
							for($j=0;$j<sizeof($showList);$j++){
								...
								if($showAlign[$j]=="" || strtolower($showAlign[$j])=="left")
									echo "&nbsp;";
								if($showLink[$j]!="") echo "<a href='".$showLink[$j].$info['rcode']."'>";
								//echo $info['rcode'].'<br>';
								if($formatList[$j]!="") 
