...
class chartGenerator
{
    ...
    public function script()
    {
        ...
        <script language="javascript" type="text/javascript">
            ...
            function divshow(divname, state, left, infor, right, QPV) {
                ...
                //else parent.document.getElementById(divname).style.visibility = "hidden";
            }
        </script>
        <?
    }
    ...
    public function showChart()
    {
        ...
        if (empty($pos1[$mcode[$k]])) $pos1[$mcode[$k]] = mysql_result($rs, 0, 'pos_cur2');
        ...
        while (sizeof($mcode) < $stk_size) {
            for ($j = 0; $j < $this->numChild; $j++) {
                ...
                if (mysql_num_rows($rs) > 0) {
                    ...
                    if (empty($pos1[$mcode[$i]])) $pos1[$mcode[$i]] = mysql_result($rs, 0, 'pos_cur2');
        ...
        for ($i = 1; $i <= $this->maxLevel; $i++) {
            ...
            for ($j = 0; $j < pow($this->numChild, $i - 1); $j++) {    //pos  j
                ...
                if ($mcode[$k] != "") { //แสดงคะแนน ซ้าย ขวา
                    ...
                    echo "<tr><td nowrap $clspan align='center' style='cursor:pointer;' onClick='parent.location=\"" . $this->redctlink . "$mcode[$k]\"' bgcolor='" . $this->tabDDef[$pos[$mcode[$k]]] . "'><font color='" . $this->txtDDef[$pos[$mcode[$k]]] . "'>" . $wording_lan["pos_cur"] . "  : " . $pos[$mcode[$k]] . "</font><br>" . $scr->getQuota($mcode[$k]) . "</td></tr>";
                    //    echo $pos[$mcode[$k]];
                    echo "</table>";
                    if ($i == ($this->maxLevel)) {
                    ...
                    <table width="35" border="0">
                        <tr valign="top">
                            <td>
                                ...
                                if (!$this->add) $tablink = "";
                                ?>
                                <table style=" background-repeat:no-repeat" <?= $tablink ?>  <?= $tablink != "" ? $alt : "" ?>
                                       width="42" height="42" border="0" cellpadding="0"
                                       cellspacing="0" <?= ($this->nullTypeDef == "COL" ? "bgcolor='" . $this->nullDef . "'" : "background='" . $this->nullDef . "'") ?> >
