...
class chartGenerator{
    ...
    function multitree($mcode,$cm,$maxlv){
		include("../member/"."wording".$_SESSION["lan"].".php");
		$scr = new memberScore();
		...
		if($lrxx == '1'){$lr = $wording_lan["endleft"];}
		else if($lrxx == '2'){$lr = $lrxxx = $wording_lan["endright"];}
		else{$lr = "";}
        ...
        if(mysql_num_rows($rs)<=0){
            ...
            if($this->imgShow)
            echo "<tr><td align='center' ><a $alt href='".$this->redctlink.$mcode."' target='_parent' ><img src='".$this->imgDef[$pppp]."' border='0'></a></td></tr>";
		...
		$status_terminate = $sqlObj2->status_terminate; 
			...
			<a $alt href='".$this->redctlink.$mcode."' target='_parent' ><img src='".$this->imgDef[$pos_cur]."' border='0'></a></td></tr>";
        //---------text information
