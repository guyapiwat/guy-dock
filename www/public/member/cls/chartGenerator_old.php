...
class chartGenerator{
	...
	$array_mpos = array('MB'=>0,'GC'=>125,'GB'=>125,'GS'=>125,'GG'=>125);
						...
						if($this->altShow){
							...
							$img_logo = "../logo.gif";
							$altOver .= "alt('<table><tr><td><table><tr><td>".$wording_lan["mcode"]."</td><td> : </td><td> ".$mcode[$k]."[".$name_b[$mcode[$k]]."]</td></tr><tr><td>".$wording_lan["sp_code"]." </td><td> : </td><td> ".$sp_code[$mcode[$k]]."[".$name_b[$sp_code[$mcode[$k]]]."]</td></tr><tr><td>".$wording_lan["upa_code"]." </td><td> : </td><td> ".$upa_code[$mcode[$k]]."[".$name_b[$upa_code[$mcode[$k]]]."]</td></tr><tr><td>ยอดส่วนตัวเดือนนี้ </td><td> : </td><td> ".number_format($scr->getAPV($mcode[$k]))."</td></tr><tr><td>".$wording_lan["OldLeft"]."(".number_format($scr->getCarryPV($mcode[$k],1),0,'.',',').") </td><td> : </td><td>".$wording_lan["OldRight"]."(".number_format($scr->getCarryPV($mcode[$k],2),0,'.',',').")</td></tr><tr><td>".$wording_lan["NewLeft"]."(".number_format($scr->getSumPV($mcode[$k],1),0,'.',',').") </td><td> : </td><td>ขวาใหม่(".number_format($scr->getSumPV($mcode[$k],2),0,'.',',').")</td></tr><tr><td>".$wording_lan["TotalLeft"]."(".number_format($scr->getCarryPV($mcode[$k],1)+$scr->getSumPV($mcode[$k],1),0,'.',',').") </td><td> : </td><td>ขวารวม(".number_format($scr->getCarryPV($mcode[$k],2)+$scr->getSumPV($mcode[$k],2),0,'.',',').")</td></tr><tr><td>สำเนาบัตรประชาชน</td><td> : </td><td>".$cmp[$mcode[$k]]." ".$bmdate1[$mcode[$k]]."</td></tr><tr><td>สำเนาบัญชีธนาคาร</td><td> : </td><td>".$cmp2[$mcode[$k]]." ".$bmdate2[$mcode[$k]]."</td></tr></table></td><td valign=top></td></tr></table>')\"";
						}
						$sp_code = $sp_code2;
						$upa_code = $upa_code2;
						$alt = $altOut."\" ".$altOver."\"";
