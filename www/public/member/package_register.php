...
function chknum(key){
	...
	//	alert(tag.length);
		for(i=0;i<Math.floor((tag.length-skip)/8);i++){
			...
			place += "<td style='"+style_l+style_bd+"' align='right'><input onkeyup='cal()' style='text-align:right;' name='qty[]' type='text' size='5' value='" + num + "'  onKeyPress='return isNumberKey(event)' onChange=\"if(this.value == '' || this.value.substring(0,1) == '0' ){alert('ไม่สามารถใส่ค่า 0 หรือ ช่องว่างได้');this.value=1;cal();}\">";      
			step++;
...
$sql = "SELECT * FROM ".$dbprefix."product_package where st = 1  and price > 0  and locationbase = '".$_SESSION["m_locationbase"]."' ";
