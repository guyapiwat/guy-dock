...
function selectitem(mcode){  
    ...
    link = '<?=$actual_link?>cart/member_memitem.php'; 
    $.post(link,queryString,function(data){   
        doc= window.opener.document; 
        doc.getElementById('member-item').innerHTML=data;  
    }); 
    
    queryString = 'action=empty';
    link = '<?=$actual_link?>cart/cart_member.php'; 
    $.post(link,queryString,function(data){   
        doc= window.opener.document; 
        doc.getElementById('cart-item').innerHTML=data;    
        var namefiled = "<?=$_GET['namefiled']?>";    
        var txt = namefiled.split(',');  
        for(var i=0;i<txt.length;i++){    
            doc.getElementById('txt'+txt[i]).value=0;  ///* txtchack
            doc.getElementById('txt'+txt[i]).disabled = true;  ///* txtchack
            doc.getElementById('chk'+txt[i]).checked=false;   
            var check =  doc.getElementById('select'+txt[i]);
            if(check != null){
               doc.getElementById('select'+txt[i]).value='';
            }
            doc.getElementById('option'+txt[i]).value='';
        }
