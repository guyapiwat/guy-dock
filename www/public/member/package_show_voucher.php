...
function chknum(key){
	...
	function saleadd(pcode,pdesc,price,pv){
		...
		window.parent.document.getElementById('checkstate').innerHTML = "<div class='alert alert-danger'><?=$wording_lan["tab4"]["1_41"] ?> </div>";
		//alert(tag.length);
		...
		for(i=0;i<(tag.length-skip)/8;i++){
			...
			place += "<td class='visible-lg visible-md visible-sm' style='"+style_l+style_bd+"' align='right'><input size='8' readonly type='text' style='"+hidden+ "text-align:right;' name='price[]' value='" + tag[++step].value + "'></td>";

			place += "<td class='visible-lg visible-md visible-sm' style='"+style_l+style_bd+"' align='right'><input size='8' readonly type='text' style='"+hidden+ "text-align:right;' name='pv[]' value='" + tag[++step].value + "'></td>";
		...
		//alert(tag.length)
			...
			place += "<td class='visible-lg visible-md visible-sm' style='"+style_l+style_bd+"' align='right'><input size='8' readonly type='text' style='text-align:right;"+hidden+ "' name='price[]' value='" + price + "'></td>";
			place += "<td class='visible-lg visible-md visible-sm' style='"+style_l+style_bd+"' align='right'><input size='8' readonly type='text' style='text-align:right;"+hidden+ "' name='pv[]' value='" + pv + "'></td>";
...
$sql = "SELECT *,'0' as ppv ,pdesc AS pdesc1 FROM ".$dbprefix."product_package where price <> 0 and locationbase = '".$_SESSION["m_locationbase"]."' and sst = '1' ";
...
if(isset($page))
	$rec->setCurPage($page);
$rec->setShowField("pcode,pdesc1,price,ppv");
...
$rec->setSpecial("./images/add_pic.gif","","saleadd","pcode,pdesc,price,ppv","IMAGE","");
