...
//if(!empty($value) ){
	...
	if(mysql_num_rows($rs) > 0){
		...
		if($object->c_sp_code2 < 5){
			...
			echo "FULL:ใต้ผู้แนะนำรหัส ".$value." เต็ม";
		}
	}
	else{
...
function isLine($dbprefix,$scode,$testcode){
	$sql = "SELECT sp_code2 FROM ".$dbprefix."member WHERE mcode='$scode' LIMIT 1 ";
	$rs = mysql_query($sql);
	...
	if(mysql_num_rows($rs)<=0)
		return false;
	else if(mysql_result($rs,0,'sp_code2')!=$testcode)
		return isLine($dbprefix,mysql_result($rs,0,'sp_code2'),$testcode);
	else
		return true;
}
