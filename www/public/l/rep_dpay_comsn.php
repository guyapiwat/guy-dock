...
<?
		...
		$sql = "SELECT ".$dbprefix."dmbonus.rcode, ".$dbprefix."dmbonus.mcode,name,acno,bankname,sum(total) as tot";
		$sql .= ",(sum(total)*5)/100 as tax,(sum(total)-(sum(total)*5)/100) as totall ";
		$sql .= "FROM ".$dbprefix."dmbonus ";
		$sql .= "LEFT JOIN (select mcode as mc,acc_name as name,acc_no as acno,bankcode as bcode from ".$dbprefix."member) AS taba ON (".$dbprefix."dmbonus.mcode=taba.mc) ";
		$sql .= "LEFT JOIN (SELECT bankcode AS bc,bankname FROM ".$dbprefix."bank) AS tbbank ON (taba.bcode=tbbank.bc) ";
		$sql .= " group by ".$dbprefix."dmbonus.rcode,".$dbprefix."dmbonus.mcode ";
		$sql .= "HAVING sum(total)>0";
		if($ftrcode!=""&&$cmc!=""){
			$sql .= " AND rcode between '".$ftrc[0]."' and '".$ftrc[1]."' and ".$dbprefix."dmbonus.mcode='".$cmc."'";
		}else if($ftrcode!=""){
			$sql .= " AND rcode between '".$ftrc[0]."' and '".$ftrc[1]."'";
		}else if($cmc!=""){
			$sql .= " AND ".$dbprefix."dmbonus.mcode='".$cmc."'";
		}
...
<form name="rform" method="post" action="./index.php?sessiontab=5&sub=8">
