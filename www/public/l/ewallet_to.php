...
$sql = "SELECT cancel,ew.id,ew.uid as uid,sano,total,txtCash,txtFuture,txtTransfer,txtInternet,txtWithdraw,txtCredit1+txtCredit2+txtCredit3 as txtCredit,txtCommission,
CASE checkportal WHEN '1' THEN 'HQ' WHEN '2' THEN 'Branch' WHEN '3' THEN 'ONLINE' END AS checkportal,sadate,txtMoney,ew.smcode as smcode1 ";
...
$sql .= "LEFT JOIN ".$dbprefix."member m ON (ew.mcode=m.mcode) "; 
$sql .= "LEFT JOIN ".$dbprefix."invent iv ON (ew.inv_code=iv.inv_code) where ew.uid = '".$_SESSION["usercode"]."' and cancel = 0 and ew.smcode <> '".$_SESSION["usercode"]."' and ew.sa_type = 'TO' "; 
...
//$wherecause = " WHERE ";
    ...
    if($_GET['state']==1){
        ...
        $rec->setLink($PHP_SELF,"sessiontab=4&sub=24");
        $rec->setBackLink($PHP_SELF,"sessiontab=4");
        if(isset($page))
            $rec->setCurPage($page);
        //$rec->setShowField("sano,smcode,name_t,preserve,ability,hold,sadate,tot_pv,total");
         $rec->setShowField("sadate,sano,smcode1,name_t,txtWithdraw,checkportal");
        ...
        $rec->setFieldDesc("วันที่,เลขที่บิล,รหัสผู้รับโอน,ชื่อผู้รับโอน,จำนวนเงิน,ช่องทาง");
        ...
        $rec->setSearch("ew.sadate,ew.sano,ew.smcode,m.name_t,ew.total");
        $rec->setSearchDesc("วันที่,เลขที่บิล,รหัสผู้รับโอน,ชื่อผู้รับโอน,จำนวนเงิน");
