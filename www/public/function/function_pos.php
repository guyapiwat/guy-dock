...
function func_status($dbprefix, $mcode, $tot_pv, $sadate, $pos_cur, $sano_x = "")
{ // 1500
...
function func_first_register($mcode, $sadate)
{ // 1500
...
function func_status_B($dbprefix, $mcode, $tot_pv, $sadate, $pos_cur)
{ // 1500
    global $array_mpos_cls;
    $month = explode("-", $sadate);
    $thismonth = $month[0] . '-' . $month[1];
...
*/
    ...
    function set_data($id, $billb, $status, $checkportal)
    {
        ...
        if (isset($checkportal)) {
            $this->checkportal = $checkportal;
        }
        if (isset($checkportal)) {
            switch ($checkportal) {
                case "1":
                    $this->table_stock = "ali_product";
                    $this->action_code = $_SESSION["adminusercode"];
                    $this->action_name = $_SESSION["adminusername"];
                    $this->action_inv = "HQ";
                    break;
                case "2":
                    $this->table_stock = "ali_product_invent";
    ...
    function cal_stocks_sale($calc, $change)
    {
        if (!empty($this->table_stock) and !empty($this->id) and $this->cancel == '0') {
            ...
            if (mysql_num_rows($rs) > 0) {
                for ($i = 0; $i < mysql_num_rows($rs); $i++) {
                    ...
                    if (mysql_num_rows($rs_pp) > 0) {
                        for ($m = 0; $m < mysql_num_rows($rs_pp); $m++) {
                            ...
                            if ($this->checkportal == "2" or $this->checkportal == "3") {
                                $sql_qty .= "and inv_code = '" . $this->inv_code . "'";
                            }
                            ...
                            if ($this->checkportal == "2" or $this->checkportal == "3") {
                                $sql_chr .= "and inv_code = '" . $this->inv_code . "'";
                            }
                            $rs_chr = mysql_query($sql_chr);
                            if (mysql_num_rows($rs_chr) > 0) {
                                $sql_update = "UPDATE " . $this->table_stock . " SET qty = qty+'" . $pp_qty . "' ";
                                if ($this->billb == "isale" or $this->billb == "tsale" or ($this->billb == "ostock" and $this->sa_type == "BB") or ($this->billb == "istock" and $this->sa_type == "BR")) {
                                    $sql_update .= ",qtyr = qtyr+'" . $qty . "' ";
                                }
                                $sql_update .= "WHERE pcode = '" . $pp_pcode . "' ";
                                if ($this->checkportal == "2" or $this->checkportal == "3") {
                                    $sql_update .= "and inv_code = '" . $this->inv_code . "' ";
                                }
                            } else {
                                $sql_update = "INSERT INTO " . $this->table_stock . "(pcode,qty,qtyr,inv_code) ";
                                $sql_update .= "VALUES ('" . $pp_pcode . "','" . $pp_qty . "',";
                                if ($this->billb == "isale" or $this->billb == "tsale" or ($this->billb == "ostock" and $this->sa_type == "BB")) {
                                    $sql_update .= "'" . $pp_qty . "',";
                                } else {
                                    $sql_update .= "'',";
                                }
                            ...
                            if (($this->billb == "isale" and $this->status == "receive") or ($this->billb == "tsale" and $this->status == "sender") or ($this->billb == "ostock" and $this->sa_type == "BB")) {
                                $this->log_stockcard_s($pp_pcode, $pp_pdesc, $qty_after_r, $pp_qty, $qty_before_r, "ali_stockcard_r");
                            }
                        }
                    } else {
                        ...
                        if ($this->checkportal == "2" or $this->checkportal == "3") {
                            $sql_p .= "and inv_code = '" . $this->inv_code . "' ";
                        }
                        ...
                        if (mysql_num_rows($rs_chr) > 0) {
                            ...
                            if ($this->billb == "isale" or $this->billb == "tsale") {
                                $sql_update .= ",qtyr = qtyr+'" . $qty . "' ";
                            }
                        ...
                        if (($this->billb == "isale" and $this->status == "receive") or ($this->billb == "tsale" and $this->status == "sender") or ($this->billb == "tsale" and $this->status == "cancel")) {
                            $this->log_stockcard_s($pcode, $pdesc, $qty_after_r, $qty, $qty_before_r, "ali_stockcard_r");
                        }
                    }
                }
                ...
                $sqlUpdate .= "," . $this->status . "_date = '" . $this->date . "' ";
                if ($change == '1') {
                    $sqlUpdate .= ",uid_" . $this->status . " = '" . $this->action_name . "' ";
                } else {
                    $sqlUpdate .= ",uid_" . $this->status . " = '' ";
                }
                    ...
                    if (mysql_num_rows($rs_pp) > 0) {
                        for ($m = 0; $m < mysql_num_rows($rs_pp); $m++) {
                            ...
                            if (mysql_num_rows($rs_chr) > 0) {
                                ...
                                $sql_update = "INSERT INTO " . $this->table_stock . "(pcode,qty,qtyr,inv_code) ";
                                $sql_update .= "VALUES ('" . $pp_pcode . "','" . $pp_qty . "',";
                                if ($this->billb == "isale" or $this->billb == "asale_qtyr" or $this->billb == "istock" or $this->billb == "ostock") {
                                    $sql_update .= "'" . $pp_qty . "',";
                                } else {
                                    $sql_update .= "'',";
                                }
                            ...
                            mysql_query($sql_update);
                            if ($calc != 0) {
                                $this->log_stockcard_s($pp_pcode, $pdesc, $qty_after, $pp_qty, $qty_before, "ali_stockcard_r");
                            }
                        }
                    } else {
                        ...
                        mysql_query($sql_update);
                        if ($calc != 0) {
                            $this->log_stockcard_s($pcode, $pdesc, $qty_after, $qty, $qty_before, "ali_stockcard_r");
                        }
                    }
                }
                $sqlUpdate = "UPDATE " . $this->table_saleh . " SET ";
    ...
    function log_stockcard_s($pcode, $pdesc, $qty_after, $qty, $qty_before, $table_stock)
    {
		...
		*/
        ...
        if ($this->checkportal == '3') {
            $this->action_inv = $this->inv_code;
        }
        if ($this->status == "cancel") {
            $cancel = 1;
        }
            ...
            foreach ($arr_bills as $keys => $values) {
                if (!empty($values)) {
                    ...
                    if ($num_arr_values > 1) {
                        ...
                        if ($last_0 == $last_1) {
                            $num_last_ = $last_0;
                            $num_loop = $last_0 - $last_1;
                        } else if ($last_0 > $last_1) {
                            $num_last_ = $last_1;
                            $num_loop = $last_0 - $last_1;
                        } else {
                            $num_last_ = $last_0;
                            $num_loop = $last_1 - $last_0;
                        }
