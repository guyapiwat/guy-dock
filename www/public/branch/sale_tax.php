...
rpdialog_sale_list($_GET['sub'],$fdate,$tdate,$sale,$s_list);
?>
...
$sql = "SELECT '".$page."' as page,".$dbprefix."member.id_card,cancel,print,".$dbprefix."asaleh.id,'00000' as lid,sano,DATE_FORMAT(sadate, '%Y-%m-%d') as sadate 
...
$sql .= " where 1=1 and sano <> '' "; //WHERE smcode='".$_SESSION['usercode']."' 
 $sql .= " and ".$dbprefix."asaleh.inv_code = '".$_SESSION['admininvent']."' ";
...
//
	...
	if($_GET['state']==1){
		...
		$rec->setLink($PHP_SELF,"sessiontab=".$sesstab."&sub=".$_GET["sub"]."&fdate=$fdate&tdate=$tdate&sale=$sale&s_list=$s_list");
