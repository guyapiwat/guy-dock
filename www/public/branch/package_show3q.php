...
function chknum(key){
    ...
    function saleadd(pcode,pdesc,price,pv){
        ...
        for(i=0;i<(tag.length-skip)/8;i++){
            ...
            place += "<input type='button' value='<?=$wording_lan['tab2_billedit']['1_36']?>' onclick=\"saledel('" + tag[step].value + "','" + tag[step+1].value + "','" + tag[step+1].value + "','" + tag[step+1].value + "')\"></td>";
        ...
        //alert(tag.length)
            ...
            place += "<td style='"+style_l+style_bd+"' align='right'><input onkeyup='cal()' name='qty[]' style='text-align:right;' type='text' size='5' value='1'  onKeyPress='return chknum(window.event.keyCode)' onChange=\"if(this.value == '' || this.value.substring(0,1) == '0'){alert('<?=$wording_lan['tab2_billedit']['1_40']?>');this.value=1;cal();}\"></td>";
...
$sql = "SELECT * FROM ".$dbprefix."product_package where bf = 1  and locationbase = '".$_SESSION["inv_locationbase"]."' and sa_type = 'Q' ";
