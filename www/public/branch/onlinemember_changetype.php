 ...
 if(isset($_GET['tstype'])){
	...
	$rsss = mysql_query($sql);
			...
			if (mysql_num_rows($result)>0 and mysql_num_rows($result123)>0  ) {
				...
				$id_tax=$row->id_tax;$id_card=$row->id_card;$address=$row->address;$province=$row->provinceId;$amphur=$row->amphurId;
				$district=$row->districtId;$zip=$row->zip;$home_t=$row->home_t;$fax=$row->fax;$mobile=$row->mobile;
				...
				$cdistrict=$row->cdistrictId;$camphur=$row->camphurId;$cprovince=$row->cprovinceId;$cfax=$row->cfax;$street=$row->street;
		...
		//			$insert_id = mysql_insert_id($link);
					...
					//=================END LOG===========================
						...
						for($i=0;$i<sizeof($pcode1);$i++){
							$sql="insert into ".$dbprefix."asaled (sano,pcode,pdesc,price,bprice,pv,qty,amt,locationbase,inv_code) values ('$mid','$pcode1[$i]','$pdesc1[$i]','$price[$i]' ,'$bprice' ,'$pv[$i]','$qty[$i]','$totalprice[$i]','$locationbase','".$sbinv_code."') ";
							//====================LOG===========================
					$text="uid=".$_SESSION["usercode"]." action=saleoperate =>$sql";
					writelogfile($text);
					//=================END LOG===========================
							//echo $sql."<br />";
							mysql_query($sql);
							updatestockcard($dbprefix,$mcode,$sbinv_code,$_SESSION["admininvent"],$sano,$sanox,$sadate,$rccode,$satype,$pcode1[$i],$_SESSION["inv_usercode"],$qty[$i],$price[$i],$totalprice[$i]);
							calc_stock($dbprefix,$pcode1[$i],$sbinv_code,$qty[$i],$sano,$_SESSION["inv_usercode"],$sbinv_code,$satype,'0');
