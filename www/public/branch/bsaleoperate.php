...
//exit;
	...
	$sql="insert into ".$dbprefix."ostockd (sano,pcode,pdesc,price,pv,qty,amt,inv_code,inv_ref) values ('$mid','$pcode[$i]','$pdesc[$i]','$price[$i]' ,'$pv[$i]','$qty[$i]','$totalprice[$i]','$inv_code','{$_SESSION["admininvent"]}') ";
	//{$_SESSION["admininvent"]}
	...
	$rs = mysql_query($sql);
		...
		//calc_minusProduct($dbprefix,$pcode[$i],$inv_ref,$qty[$i],$sano,$_SESSION['inv_usercode'],$inv_code);

	}
		$object_stocks = new stocks ();
		$object_stocks->set_data($mid,"ostock","sender","2"); 
...
$_SESSION["perbuy"] = 0;
echo "<script language='JavaScript'>window.location='index.php?sessiontab=3&sub=21'</script>";	
