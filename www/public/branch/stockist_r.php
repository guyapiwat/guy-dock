<script language="javascript" type="text/javascript">
	...
	function sale_cancel(id){
			if(confirm("<?=$wording_lan['Bill_21']?>"))
				{
				window.location='index.php?sessiontab=5&sub=148&state=3&bid='+id;
		}
	}
	function sale_status(id,page,chktype,receive){
		if(receive == '1'){
			if(confirm("<?=$wording_lan['Bill_22']?>")){
				window.location='index.php?sessiontab=5&sub=148&state=6&sender='+id+'&page='+page+'&chktype='+chktype;
				exit;
			}
		}else{
			window.location='index.php?sessiontab=5&sub=148&state=6&sender='+id+'&page='+page+'&chktype='+chktype;
		...
		if(confirm("<?=$wording_lan['Bill_23']?>")){
			window.location='index.php?sessiontab=5&sub=148&state=7&sender='+id;
		}
	}
</script>
<?
...
if(isset($_POST["fdate"]))
		...
		$fdate = $_GET["fdate"];
$sql = "SELECT '".$_GET["sessiontab"]."' as sessiontab,'".$_GET["sub"]."' as sub,'".$_GET["chktype"]."' as chktype,".$dbprefix."isaleh.uid_receive,".$dbprefix."isaleh.name_t,".$dbprefix."isaleh.uid,".$dbprefix."isaleh.lid,".$dbprefix."isaleh.uid_sender,'".$page."' as page,cancel,print,".$dbprefix."isaleh.id,".$dbprefix."isaleh.sender,".$dbprefix."isaleh.sender_date,".$dbprefix."isaleh.inv_code,".$dbprefix."isaleh.tot_pv/100 as discount,total+".$dbprefix."isaleh.tot_pv/100 as alltotal,sano,sadate,tot_pv,tot_bv,tot_fv,total,".$dbprefix."isaleh.inv_code AS smcode ";
...
$sql .= "LEFT JOIN ".$dbprefix."invent ON (".$dbprefix."isaleh.inv_code=".$dbprefix."invent.inv_code) where  ".$dbprefix."isaleh.inv_from = '{$_SESSION["admininvent"]}' and  ".$dbprefix."isaleh.cancel = 0 and send != '1' and ".$dbprefix."isaleh.receive <> '1' ";
