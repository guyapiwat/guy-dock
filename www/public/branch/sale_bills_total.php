...
<?
	...
	switch($stype){
		case 1:
			...
			include("sale_bills_total_ordernum.php");
			break;
		case 2:
...
<? function searchbox($dbprefix,$inv_code,$fdate,$tdate,$satype,$logistic,$strpv,$strtotal,$struid,$sspv,$strtype,$strSearch,$locationbase){
	global $wording_lan;
	?>
<form style="margin-bottom:0;" action="index.php?sessiontab=6&sub=46" method="post">
      ...
      <td width="1000" align="left" ><fieldset>
       ...
       Stockist
        <select name="inv_code" id="inv_code"  >
          ...
          <?					
						$result1=mysql_query("select * from ".$dbprefix."invent where inv_type = '2' and locationbase = '$locationbase'  order by inv_code");
		...
		<select size="1" disabled name="locationbase" id="locationbase" tabindex="10">
