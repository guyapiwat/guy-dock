...
<script language="javascript" type="text/javascript">
	function saleadd(pcode,pdesc,price,pv,bv,fv){
		...
		window.parent.document.getElementById('checkstate').innerHTML = "<font color='#FFFFFF' style='background:#990000'> &nbsp;<?=$wording_lan['tab1_mem_141']?>&nbsp; </font>";
		//alert(tag.length);
		...
		//alert(tag.length)
			...
			place += "<td style='"+style_l+style_bd+"' align='center'><input type='button' value='<?=$wording_lan['tab2_billedit']['1_36']?>' onclick=\"saledel('" + tag[step].value + "','" + tag[step+1].value + "','" + tag[step+1].value + "','" + tag[step+1].value + "','" + tag[step+1].value + "','" + tag[step+1].value + "')\"></td>";
			place += "<td style='"+style_l+style_bd+"' align='center'>" + (i+1) + "</td>";
...
//$discount = $_SESSION["discount"];
$discount = 0;
$sql = "SELECT pcode,pdesc,price,pv,bv,fv FROM ".$dbprefix."product where  bf=1 and locationbase = '".$_SESSION["inv_locationbase"]."'  ";
