...
for($i=0;$i<sizeof($bill);$i++){
	$sql="SELECT * FROM ".$dbprefix."holddesc WHERE hono='$sano[$i]' ";
	$rs=mysql_query($sql);
	...
	$sumpv = 0;
	for($k=0;$k<$npage;$k++){
	$sql="SELECT * FROM ".$dbprefix."holddesc WHERE hono='$sano[$i]' LIMIT ".($k*$nitem).", $nitem ";
	$rs=mysql_query($sql);
	...
	$pdf->Image('../logo.jpg',$offsetx+$offsettab-12,$offsety+$offsetnline+2,35); //file,x,y,w=0,h=0
	...
	$pdf->Cell((3*$offsettab),10,"บิลแจงยอด",0,0,"R"); 
