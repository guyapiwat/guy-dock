			...
			//	if($totalamt1[$j] > 0 and $mstatus[$j] == '0'){
				...
				if($totalpv>=0 or $cstatus <> '1' or $mcode[$j] == '0000001' or $mcode[$j] == '0000002' or $mcode[$j] == '0000003'){
					if($totalamt1[$j] > 0){
						...
						//echo $mcode[$j].' : '.$btotal.' : '.$total12.' : '.$totalamt1[$j].' : '.$totalpv.'<br>';
						if($totalpv>= 250 and $cmp[$j] == 'ครบ' and $cmp2[$j] == 'ครบ'  and  $cmp3[$j] == 'ครบ'  and !empty($acc_no[$j]) and $mem_cntday[$j] > -90 and $status_suspend[$j] <> '1'  ){
							...
							if(!empty($acc_no[$j]))$c_note3 = 1;else $c_note3 = "";
								$sql = "INSERT INTO ".$dbprefix."cmbonus (rcode,mcode,status,pv,pvb,pvh,fob,cycle,smb,matching,onetime,total,totaly,mdate,month_pv,mpos,tot_vat,tot_tax,title,paydate,status_pv) ";
								$sql .= "VALUES('$ro','".$mcode[$j]."','1','".$moneyb[$j]."','".$totalamt[$j]."','0','".$totalfast[$j]."','".$totalbinary[$j]."','".$totalstar[$j]."','".$totalmatching[$j]."','".$totalonetime[$j]."','".$totalamt1[$j]."','".$total12."','".$strfdate."','".$month[0].$month[1]."','".$pos_cur[$j]."','".$vat[$j]."','".$tax[$j]."','".$title[$j]."','$paydate','$totalpv')";
							...
							$sql = "INSERT INTO ".$dbprefix."cmbonus (rcode,mcode,status,pv,pvb,pvh,fob,cycle,smb,matching,onetime,total,totaly,mdate,month_pv,mpos,c_note1,c_note2,c_note3,c_note4,c_note5,tot_vat,tot_tax,title,paydate,status_pv) ";
								$sql .= "VALUES('$ro','".$mcode[$j]."','0','".$moneyb[$j]."','0','".$totalamt1[$j]."','".$totalfast[$j]."','".$totalbinary[$j]."','".$totalstar[$j]."','".$totalmatching[$j]."','".$totalonetime[$j]."','0','".$btotal."','".$strfdate."','".$month[0].$month[1]."','".$pos_cur[$j]."','".$c_note1."','".$c_note2."','".$c_note3."','".$c_note4."','".$c_note5."','".$vat[$j]."','".$tax[$j]."','".$title[$j]."','$paydate','$totalpv')";
							}
					...
					if($cstatus == '1'){
						...
						if($totalpv1 < 250 and $totalpv2 < 250){ 
							...
							$sql2 = "update ".$dbprefix."bmbonus set carry_l =carry_l/2 , carry_c =carry_c/2 WHERE mcode='".$mcode[$j]."' and  tdate = '$tdate'  ";
						}
						$rs2=mysql_query($sql2);
					}
				}
