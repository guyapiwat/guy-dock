...
(
select '3' as number1,mcode,count(id) as cid,sum(total) as total,sum(txtCash) as txtCash,sum(txtCredit1+txtCredit2+txtCredit3) as txtCredit,sum(txtInternet) as txtEwallet,sum(txtTransfer) as txtTransfer,sum(txtFuture) as txtFuture,sum(txtDiscount) as txtDiscount,'บิลขาย'  as typee ,'0' as txtOther  from ali_asaleh where scheck = ''  $sqlwhere

union select '4' as number1,mcode,count(id) as cid, sum(txtMoney) as total,sum(txtCash) as txtCash,sum(txtCredit1+txtCredit2+txtCredit3) as txtCredit,'0' as txtEwallet,sum(txtTransfer) as txtTransfer,'0' as txtFuture,'0' as txtDiscount,'เติมเงิน' as typee,'0' as txtOther  from ali_ewallet where 1=1 $sqlwhere

union select '1' as number1,mcode,count(id) as cid, sum(total) as total,sum(txtCash) as txtCash,sum(txtCredit1+txtCredit2+txtCredit3) as txtCredit,sum(txtInternet) as txtEwallet,sum(txtTransfer) as txtTransfer,sum(txtFuture) as txtFuture,sum(txtDiscount) as txtDiscount,'บิลสมัคร' as typee,'0' as txtOther  from ali_asaleh where scheck = 'register'  $sqlwhere
...
//echo $sql;
	...
	if($_GET['state']==1){
		...
		$rec->setLink($PHP_SELF,"sessiontab=6&sub=46&fdate=$fdate&tdate=$tdate&satype=$satype&logistic=$logistic&strpv=$strpv&strtotal=$strtotal&struid=$struid&sspv=$sspv&inv_code=$inv_code&strSearch=$strSearch&strtype=$strtype&sregister=$sregister&locationbase=$locationbase");
