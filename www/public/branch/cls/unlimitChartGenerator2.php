...
class chartGenerator{
    ...
    function multitree($mcode,$cm,$maxlv){
        ...
        $sql2 = "select mcode,name_t,name_b,pos_cur,pos_cur1,pos_cur2,sp_code2 as sp_code,sp_code as sss,sp_code2 as sss2,upa_code,status_terminate from ".$this->dbPrefix."member where mcode = '$mcode' ";
        $rs = mysql_query($sql);
        ...
        /*$sql = "SELECT sum(tot_fv) AS all_pv FROM ali_asaleh WHERE mcode='$cmc' and  sa_type='Q' and sadate like (select  DATE_FORMAT(max(fdate), '%Y-%m-%') from ali_around where calc = 1 )";
        //echo $sql;
        $rs3 = mysql_query($sql);
        $all_Qpv = mysql_result($rs3,0,'all_pv'); 
        mysql_free_result($rs3);

        $sql3 = "SELECT sum(tot_fv) AS all_pv FROM ali_holdhead WHERE mcode='$cmc' and sa_type='Q' and sadate like (select  DATE_FORMAT(max(fdate), '%Y-%m-%') from ali_around where calc = 1 ) ";
        //echo $sql;
        $rsrs3= mysql_query($sql);
        $all_Qpv = ($all_Qpv+mysql_result($rs3,0,'all_pv')); 
        mysql_free_result($rs3);*/
        /*$alt = "onmouseout=\"alt('')\" onmouseover=\"alt('<table><tr><td colspan=3 align=center><img src=../logo.gif width=50 height=50></td></tr><tr><td>สมาชิก</td><td> : </td><td>".mysql_result($rs2,0,'mcode')."[ ".$name_show."]</td></tr><tr><td>ผู้แนะนำ</td><td> : </td><td>".mysql_result($rs2,0,'sp_code')."[ ".mysql_result($rs2,0,'sp_name')."]</td></tr><tr><td>อัพไลน์</td><td> : </td><td>".mysql_result($rs2,0,'upa_code')."[ ".mysql_result($rs2,0,'upa_name')."]</td></tr></tr><tr><td>PVสะสม(ขึ้นตำแหน่ง)</td><td> : </td><td>".$all_pv."</td></tr></tr><tr><td>PVรักษายอด</td><td> : </td><td>".$all_Qpv."</td></tr></table>')\"";*/
    ...
    //$stk_size = 0;
        ...
        if(mysql_result($rs,0,'sp_code')!=""){ //up arrow
            if($this->block==false){ 
                ...
                echo "onClick='parent.location.href=\"".$this->redctlink.mysql_result($rs,0,'sp_code')."\"' ";
