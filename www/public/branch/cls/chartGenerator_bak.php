...
class chartGenerator{
	...
	//$stk_size = 0;
		...
		//echo "select * from ".$this->dbPrefix."calc_poschange4 where mcode='$mcode[$k]' order by id desc limit 0,1  <br>";
		$rs1=mysql_query("select * from ".$this->dbPrefix."calc_poschange4 where mcode='$mcode[$k]' order by id desc limit 0,1  ");
		...
		while(sizeof($mcode)<$stk_size){
			for($j=0;$j<$this->numChild;$j++){
				...
				if(mysql_num_rows($rs)>0){
					...
					$rs1=mysql_query("select * from ".$this->dbPrefix."calc_poschange4 where mcode='$mcode[$i]'  order by id desc limit 0,1 ");
	...
	$array_mpos = array('MB'=>0,'GC'=>125,'GB'=>125,'GS'=>125,'GG'=>125);
						...
						if($this->altShow){
							$altOut .= "alt('');";
							$altOver .= "alt('<table><tr><td>PV ส่วนตัวเดือนนี้ </td><td> : </td><td>".$scr->getMaxQuota($mcode[$k])."</td></tr><tr><td>ซ้ายเก่า(".number_format($scr->getCarryPV($mcode[$k],1),0,'.',',').") </td><td> : </td><td>ขวาเก่า(".number_format($scr->getCarryPV($mcode[$k],2),0,'.',',').")</td></tr><tr><td>ซ้ายใหม่(".number_format($scr->getSumPV($mcode[$k],1),0,'.',',').") </td><td> : </td><td>ขวาใหม่(".number_format($scr->getSumPV($mcode[$k],2),0,'.',',').")</td></tr><tr><td>ซ้ายรวม(".number_format($scr->getCarryPV($mcode[$k],1)+$scr->getSumPV($mcode[$k],1),0,'.',',').") </td><td> : </td><td>ขวารวม(".number_format($scr->getCarryPV($mcode[$k],2)+$scr->getSumPV($mcode[$k],2),0,'.',',').")</td></tr></tr></table>')\"";
						}
...
class memberScore{
	...
	/*$sql="SELECT rcode,fdate,tdate FROM ".$this->dbPrefix."around WHERE calc='1' ORDER BY rcode DESC LIMIT 1";
		...
		$sql="SELECT rcode,fdate,tdate FROM ".$this->dbPrefix."around WHERE calc='1' and tdate like '%".date("Y-m", strtotime("-1 month", strtotime(date("F") . "10")) )."%' ORDER BY rcode DESC LIMIT 1";
		//echo $sql;
		$rs = mysql_query($sql);
		$where_cause = "";
		$max_rcode = 0;
		...
		$where_cause = "AND sadate between '".date("Y-m")."-01' and '".date("Y-m")."-31' ";
			
		$sql="SELECT * FROM ".$this->dbPrefix."asaleh  WHERE sa_type='A'  and cancel = 0 $where_cause ORDER BY sadate"; //AND (sa_type='T' OR sa_type='P') AND sa_type='A'
		//echo $sql;
		$rs = mysql_query($sql);
		for($i=0;$i<mysql_num_rows($rs);$i++){
			$sqlObj = mysql_fetch_object($rs);
		...
		mysql_free_result($rs);
		$sql="SELECT * FROM ".$this->dbPrefix."holdhead  WHERE sa_type='A' and cancel = 0 $where_cause ORDER BY sadate"; //AND (sa_type='T' OR sa_type='P') AND sa_type='A'
		//echo $sql;
		$rs = mysql_query($sql);
		for($i=0;$i<mysql_num_rows($rs);$i++){
			$sqlObj = mysql_fetch_object($rs);
		...
		mysql_free_result($rs);
	//	echo "select * from ".$this->dbPrefix."calc_poschange1 where 1=1 order by id desc group by mcode  ";
	//	exit;
		//$result11=mysql_query("select * from ".$this->dbPrefix."calc_poschange1 where 1=1 group by mcode order by id desc  ");
