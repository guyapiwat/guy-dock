...
$sql .= "FROM ".$dbprefix."transferewallet_h ";
$sql .= "LEFT JOIN ".$dbprefix."member ON (".$dbprefix."transferewallet_h.mcode=".$dbprefix."member.mcode) where ".$dbprefix."transferewallet_h.locationbase = '".$_SESSION["inv_locationbase"]."' "; //WHERE smcode='".$_SESSION['usercode']."' 
//echo $sql;
...
//$wherecause = " WHERE "
	...
	if($_GET['state']==1){
		...
		$rec->setLink($PHP_SELF,"sessiontab=6&sub=43");
		$rec->setBackLink($PHP_SELF,"sessiontab=6");
		if(isset($page))
			$rec->setCurPage($page);
		$rec->setShowField("id,smcode,name_t,mobile,total,paytype1,credittype,sano");
		$rec->setFieldDesc($wording_lan["supervisor_19"].",".$wording_lan["supervisor_20"].",".$wording_lan["supervisor_21"].",".$wording_lan["supervisor_22"].",".$wording_lan["supervisor_23"].",".$wording_lan["supervisor_24"].",".$wording_lan["supervisor_25"].",".$wording_lan["supervisor_27"]);
		...
		$rec->setSearch("".$dbprefix."transferewallet_h.id,".$dbprefix."transferewallet_h.mcode,".$dbprefix."transferewallet_h.name_t");
