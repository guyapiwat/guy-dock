...
$arrpos = array('P'=>"Platinum",'G'=>"Gold",'S'=>"Silver",'A'=>"Agent",''=>"ไม่มีตำแหน่ง");
...
$mpos = mysql_result($rs,0,'pos_cur');
	   	$result1=mysql_query("select * from ".$dbprefix."member where sp_code='".$mcode."' order by lr desc");
		...
		echo "<td> รหัส <a href='index.php?sessiontab=1&sub=9&cmc=$mcode'>".$mcode."</a> ชื่อ ".$mname." </td></tr>";
		...
		for($i=0;$i<mysql_num_rows($result1);$i++){
			...
			$result2=mysql_query("select * from ".$dbprefix."member where sp_code='".$ccode."' order by lr desc");
			$useScript = "";
			$tabAttr = "cellpadding='0' cellspacing='0'";
			$tstat	=	"o";
			...
			echo "<td> รหัส <a href='index.php?sessiontab=1&sub=9&cmc=$ccode'>".$ccode."</a> ชื่อ ".$cname." </td>";
			echo "</tr></table></div>";
			echo "</td></tr>";
		}
		echo "</table></div>";
