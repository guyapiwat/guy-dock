...
function selectitem(inv_code,inv_desc,ewallet,voucher,inv_type){
		doc= window.opener.document;
		//alert(document.listform.idx.value);
		doc.frm.inv_code.value=inv_code;
		doc.getElementById('inv_desc').innerHTML=inv_desc+' <br> Ewallet : '+ewallet+' <br> Voucher : '+voucher;
...
$sql = "SELECT inv_code,inv_desc,cname,CASE inv_type WHEN '1' THEN 'Branch' WHEN '2' THEN 'Stockist' END AS inv_type,ewallet,voucher FROM ".$dbprefix."invent left join ".$dbprefix."location_base on (".$dbprefix."invent.locationbase = ".$dbprefix."location_base.cid) where inv_type = 2 and locationbase = '".$_SESSION["inv_locationbase"]."' ";
//$wherecause = " WHERE ";
	...
	if($_GET['state']==1){
		...
		$rec->setOrder($_GET['ord']==""?" inv_code ":$_GET['ord']);
		...
		$rec->setSpecial("./images/add_pic.gif","","selectitem","inv_code,inv_desc,ewallet,voucher","IMAGE","");
