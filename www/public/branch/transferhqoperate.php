...
//echo "<script language='JavaScript'>window.location='index.php?sessiontab=5&sub=23&state=2'</script>";
//exit;
}
...
if($_GET['state']==0){
	...
	$sql = "SELECT MAX(id) AS id ,MAX(sano) as sano FROM ".$dbprefix."tsaleh   ";
	$rs = mysql_query($sql);
	$mid = mysql_result($rs,0,'id')+1;
	$sano = gencodesaleo('I','tsaleh');
	mysql_free_result($rs);
	$mid = ++$mid;
	...
	for($i=0;$i<sizeof($pcode);$i++){
		...
		$sql="insert into ".$dbprefix."tsaled (sano,pcode,pdesc,price,bprice,pv,bv,fv,qty,amt,inv_code,uidbase,locationbase) values ('$mid','$pcode[$i]','$pdesc[$i]','$price[$i]' ,'$bprice[$i]' ,'$pv[$i]','$bv[$i]','$fv[$i]','$qty[$i]','$totalprice[$i]','$inv_code','1','".$arr_invent["locationbase"]."') ";
		//====================LOG===========================
		...
		//calc_minusProduct($dbprefix,$pcode[$i],$inv_code,$qty[$i],$sano,$_SESSION['admininvent'],$inv_code);//updatestockcard($dbprefix,$mcode,$inv_code,$_SESSION["admininvent"],$sano,$sanox,date("Y-m-d"),$rccode,$satype,$pcode[$i],$_SESSION["inv_usercode"],$qty[$i],$price[$i],$totalprice[$i]);
		//calc_stock($dbprefix,$pcode[$i],$inv_code,-$qty[$i],$sano,$_SESSION["inv_usercode"],$_SESSION["admininvent"],$satype,'0');
		  
		//
		//$object_stocks->set_data($mid,"tsale","sender","2"); 
...
$_SESSION["perbuy"] = 0;
echo "<script language='JavaScript'>window.location='index.php?sessiontab=5&sub=23'</script>";	
