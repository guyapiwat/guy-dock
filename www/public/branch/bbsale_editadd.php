...
//check(nullCheck, sizeCheck, formatCheck, dupCheck, realCheck, idName, errmsg)
	...
	flag = flag+",1-0-0-0-0";
	errDesc = errDesc + ",เลือกสาเหตุรับสินค้า";
...
var wi=null;
	...
	function saledel(pcode,pdesc,price,pv,bv,fv){
		...
		place += "<tr><td colspan='9' align='right'><input name='button' type='button' onclick='<?=(isset($_GET['id'])?"ebillcheck()":"ibillcheck()")?>' value='ตรวจสอบ' />&nbsp;<input type='submit' value='บันทึก' name='ok' id='ok' disabled='disabled' />&nbsp;<input name='reset' type='reset'  onclick=\"window.location='index.php?sessiontab=3&sub=23'\" value='ยกเลิก' /></td></tr>";
		place += "</table>";
		//alert(place);
      ...
      <tr valign="top">
        <td>
            ...
            <tr valign="top">
             ...
             <option  value="" <?=($mapping_code==""?"selected":"")?>>เลือกสาเหตุการรับสินค้า</option>
<?					
