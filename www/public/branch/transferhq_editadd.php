...
document.getElementById('frm').action='transferhqoperate.php?state=0';
//alert(document.getElementById('frm').target);
//alert(document.getElementById('frm').action);

	document.getElementById('checkstate').innerHTML= "<img align='center' src='./images/loading.gif' />";
	startRQ(field,val,"",flag,errDesc,"tsaleh","checkstate");
}
function ebillcheck(){
...
document.getElementById('frm').action='transferhqoperate.php?state=1';
//alert(document.getElementById('frm').target);
//alert(document.getElementById('frm').action);
	document.getElementById('checkstate').innerHTML= "<img align='center' src='./images/loading.gif' />";
	//alert(skipval);
	startRQ(field,val,skipval,flag,errDesc,"tsaleh","checkstate");
}
...
if(isset($_GET['id'])){
		...
		$sql = "SELECT * FROM ".$dbprefix."tsaleh WHERE id='".$_GET['id']."'  LIMIT 1";
		$rs = mysql_query($sql);
...
?>
<form method="post" action="transferhqoperate.php?state=<?=$_GET['id']==""?0:1?>" id="frm" name="frm"  onsubmit="return checkForm(this)" onKeyDown="document.getElementById('ok').disabled=true;" >
  <table border="0">
  <tr valign="top">
    <td>				
...
เลือกรูปแบบการซื้อ                  </option>                <option  value="HO" <?=($satype=="HO"?"selected":"")?>>
            ...
            <tr valign="top">
              <td width="16%" align="right">สาขาที่คืน </td>
              <td><input style="background-color:#FFFF99;" size="15" type="text" id="inv_code"  readonly="" name="inv_code" value="<?=$_SESSION["admininvent"]?>">
               ...
               <input type="button" onClick="get_mem_listpicker_invcode()" style="display:none" value="เลือก">
<div id="inv_desc"></div> </td>
              <td align="right" valign="bottom" nowrap="nowrap">&nbsp;รับที่สาขา</td>
              <td nowrap="nowrap">
			    <input type="radio" name="radsend" id="radsend" checked="true" value="1"  >
			    &#3592;&#3633;&#3604;&#3626;&#3656;&#3591;
                <input type="radio" name="radsend2" id="radsend2" value="2" style="display:none"  onClick="document.getElementById('inv_ref').disabled = false;document.getElementById('ccheckr').value = '2';"  <?=($radsend=="2"?"checked":"")?> >
<?/*&#3652;&#3617;&#3656;&#3592;&#3633;&#3604;&#3626;&#3656;&#3591; */?><br>
<br>
<select name="inv_ref" id="inv_ref"  disabled="disabled">
      ...
      <tr>
        ...
        <? 
		if(!empty($_GET['id'])){
			$sql = "SELECT * FROM ".$dbprefix."tsaled WHERE sano='".$_GET['id']."' ";
			$rs = mysql_query($sql);
			if(mysql_num_rows($rs)>0){
