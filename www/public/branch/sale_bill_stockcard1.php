...
if($fdate!=""){
	$sqlwhere .= " and ".$dbprefix."stockcard_s.sadate BETWEEN '$fdate' AND '$tdate' ";
	$where = 1;
}else $where = 0;
...
if($inv_code !="" and $inv_code1 !=""){
    ...
    for($i=0;$i<@mysql_num_rows($rs);$i++){
		$sqlwhere .= " ".$dbprefix."stockcard_s.inv_action = '".mysql_result($rs,$i,'inv_code')."' or  ";
	}

	$sqlwhere .= " ".$dbprefix."stockcard_s.inv_action = '9999' ) ";
}
...
if($pcode !="" and $pcode1 !=""){
    ...
    for($i=0;$i<mysql_num_rows($rs);$i++){
		$sqlwhere .= " ".$dbprefix."stockcard_s.pcode = '".mysql_result($rs,$i,'pcode')."' or  ";
	}

	$sqlwhere .= " ".$dbprefix."stockcard_s.pcode = '9999' ) ";
}
...
if($strSearch1!=""){
	$sqlwhere .= " and ".$dbprefix."stockcard_s.pcode = '$strSearch1'  ";
}
if($strSearch2!=""){
	$sqlwhere .= " and ".$dbprefix."stockcard_s.sano like '$strSearch2'  ";
}

$sql = "SELECT *,".$dbprefix."stockcard_s.pcode as pcode123,".$dbprefix."stockcard_s.price as price123";
$sql .=" ,name_t as name  "; 
$sql .=" ,CASE ".$dbprefix."stockcard_s.action WHEN 'A' THEN 'ปกติ' WHEN 'HO' THEN 'โอนคืน' WHEN 'I' THEN 'ส่งสาขา' WHEN 'BRHO' THEN 'ใบรับ(HQ)' WHEN 'BBHO' THEN 'ใบเบิก(HQ)' WHEN 'BB' THEN 'ใบเบิก(สาขา)' WHEN 'BR' THEN 'ใบรับ(สาขา)' WHEN 'IM' THEN 'สินค้านำเข้า' END as type  ";  
...
if($_GET['print_all']==true){
	...
	exit;
		...
		$rec->setOrder($_GET['ord']==""?" ".$dbprefix."stockcard_s.id ":$_GET['ord']);
