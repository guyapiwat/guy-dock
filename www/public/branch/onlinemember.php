...
<script language="javascript" type="text/javascript">
	...
	function sale_look(id){
		...
		window.open(wlink);
		//window.location.reload();
		//window.location='index.php?sessiontab=3&sub=147&sanooo='+id;
	}
	function sale_cancel(id){
		if(confirm("<?=$wording_lan['Bill_21']?>")){
			window.location='index.php?sessiontab=3&sub=147&state=3&bid='+id;
		}
	}
...
if (isset($_GET["pg"])){$page=$_GET["pg"];} else {$page="1";}
 $sql = "SELECT '".$_GET["sessiontab"]."' as sessiontab,'".$_GET["sub"]."' as sub,'".$_GET["chktype"]."' as chktype,".$dbprefix."asaleh.inv_code,uid_receive,uid_sender,cancel,print,".$dbprefix."asaleh.id,".$dbprefix."asaleh.lid,sano,DATE_FORMAT(sadate, '%Y-%m-%d') as sadate 
...
$sql .= "where ".$dbprefix."asaleh.locationbase = '".$_SESSION["inv_locationbase"]."'  and scheck='register' and checkportal='3' "; 
//$sql .= "LEFT JOIN ".$dbprefix."member ON (".$dbprefix."asaleh.mcode=".$dbprefix."member.mcode) where ".$dbprefix."asaleh.locationbase = '".$_SESSION["inv_locationbase"]."' "; //WHERE smcode='".$_SESSION['usercode']."' 
//echo $sql;
...
//$fdate = $monthmonth[0].'-'.$monthmonth[1];
	...
	if($_GET['state']==1){
		...
		$rec->setShowField("print,sadate,sano,smcode,name_t,ability,tot_pv,total,uid,sendsend,sender,receive,remark,inv_code,uid_sender,uid_receive,checkportal");
	...
	$rec->setFieldDesc("P,".$wording_lan["Bill_1"].",".$wording_lan["Bill_2"].",".$wording_lan["Bill_3"].",".$wording_lan["Bill_4"].",".$wording_lan["Bill_5"].",".$wording_lan["Bill_6"].",".$wording_lan["Bill_7"].",".$wording_lan["Bill_8"].",".$wording_lan["Bill_9"].",".$wording_lan["Bill_10"].",".$wording_lan["Bill_11"].",".$wording_lan["Bill_12"].",".$wording_lan["Bill_14"].",".$wording_lan["Bill_15"].",".$wording_lan["Bill_16"].",".$wording_lan["Bill_17"]."");

		$rec->setFieldAlign("center,left,left,left,left,center,right,right,center,center,center,left,center,center,center,center");
	...
	//	$rec->setSearchDesc("เลขบิล,รหัสผู้ซื้อ,ชื่อผู้ซื้อ,สาขา,วันที่ซื้อ,จำนวนรวม  PV,จำนวนเงินรวม,พนักงาน");
		...
		if($acc->isAccess(4) and $_SESSION["inventobj6"] == '7'){
		//	$rec->setSpecial("./images/download.jpg","","sale_cancelh","id","IMAGE",$wording_lan["Change to BMC"]);
		}
		...
		//exit;
	//	if($acc->isAccess(2))
			//$rec->setEdit("index.php","id","id","sessiontab=3&sub=147");
