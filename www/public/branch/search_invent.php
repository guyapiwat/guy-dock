...
//  $sql .= " FROM ".$dbprefix."member  where mcode = '%$value%' limit 0,1";
	//	$sql .= " FROM ".$dbprefix."invent  where mcode like '%$value%' limit 0,1";
		$sql = "SELECT inv_code,inv_desc,CASE inv_type WHEN '1' THEN 'ศูนย์' WHEN '2' THEN 'โมบาย' END AS inv_type FROM ".$dbprefix."invent where inv_code = '$value' and inv_type = 2 ";
