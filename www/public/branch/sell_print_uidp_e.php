...
$where = "and sadate >= '{$fdate}' and sadate <= '{$tdate}' and  lid = '$inv_ref' ";

$set_payment = query("*",'ali_payment pm '," 1=1 and pm.shows_ewallet ='1' and (payment_name <> 'Ewallet' and payment_name <> 'Voucher' and payment_name <> 'Discount') ORDER BY pm.order_by "); 
...
else $inv = "and user.inv_ref = '{$_SESSION["admininvent"]}'";
...
foreach($inv as $keyx => $valx):
  if($valx['inv_code'])
  {
    ...
    echo "รายงานการรับชำระเงินประจำวัน แยกตามเคาน์เตอร์และประเภทการชำระ(เติมเงิน)";
    ...
    foreach($arr as $key => $val): 
       ...
       $sum = query($select[$key],'ali_ewallet ash'," lid = '{$valx['inv_code']}' and uid = '{$valx['usercode']}' $where ");  
      ...
      if($a >= 0)
      {
        ...
        if($paymem_option)
        {
            ...
            foreach($paymem_option as $k => $v):   
               ...
               $sumx = query("IFNULL(SUM({$t}),0) as {$valxx['payment_column']} ",'ali_ewallet ash'," lid = '{$valx['inv_code']}' and uid = '{$valx['usercode']}' and {$s} = '{$v['id']}' $where ");  
               echo "<div class='right'>".$sumx[0][$valxx['payment_column']]."</div>";
               $allex[$keyx] += $sumx[0][$valxx['payment_column']];
            endforeach;   
            $sumo = query("IFNULL(SUM({$t}),0) as {$valxx['payment_column']} ",'ali_ewallet ash'," lid = '{$valx['inv_code']}' and uid = '{$valx['usercode']}' and {$s} NOT IN (".substr($c,0,-1).") $where ");   
