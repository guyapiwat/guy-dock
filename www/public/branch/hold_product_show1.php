...
<script language="javascript" type="text/javascript">
	function holdadd(pcode,pdesc,price,pv,qty){
		...
		place += "</tr>";
		place += "<tr><td colspan='9' align='right'><input name='button' type='button' onclick='<?=(isset($_GET['id'])?"ebillcheck()":"ibillcheck()")?>' value='ตรวจสอบ' />&nbsp;<input type='submit' value='บันทึก' name='ok' id='ok'  disabled='disabled' />&nbsp;<input name='reset' id='reset' type='reset'  onclick=\"window.location='index.php?sessiontab=3&sub=6'\" value='ยกเลิก' /></td></tr>";
		place += "</table>";
...
$sql .= "LEFT JOIN ".$dbprefix."asaleh as ah1 ON (ah.sano=ah1.sano_ref and ah1.cancel=0 and substr(ah1.sano,1,2) <> 'W3') ";
...
$sql .= "WHERE ad.sano='".$_GET['bid']."'  GROUP BY pcode) as a where a.qty > 0      ";//echo $sql;
//$sql .= "WHERE ad.id='".$_GET['bid']."'  and ah.inv_ref = '".$_SESSION["admininvent"]."' GROUP BY pcode) as a where a.qty > 0      ";//echo $sql;
//echo $sql;
//if(empty($_GET["id"]))$sql .= "and a.price > 0 ";
//else if(!empty($_GET["id"]))$sql .= "and a.price = 0 ";
//echo $sql;
