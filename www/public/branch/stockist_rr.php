<script language="javascript" type="text/javascript">
	...
	//	window.location='index.php?sessiontab=5&sub=149&sanoo='+id;
	}
	function sale_look(id){
		...
		window.open(wlink);
		//window.location.reload();
		//window.location='index.php?sessiontab=5&sub=149&sanooo='+id;
	}
	function sale_cancel(id){
			if(confirm("<?=$wording_lan['Bill_21']?>"))
				{
				window.location='index.php?sessiontab=5&sub=149&state=3&bid='+id;
		}
	}
	function sale_status(id,page,chktype,receive){
		if(receive == '1'){
			if(confirm("<?=$wording_lan['Bill_22']?>")){
				window.location='index.php?sessiontab=5&sub=149&state=6&sender='+id+'&page='+page+'&chktype='+chktype;
				exit;
			}
		}else{
			window.location='index.php?sessiontab=5&sub=149&state=6&sender='+id+'&page='+page+'&chktype='+chktype;
		...
		if(confirm("<?=$wording_lan['Bill_23']?>")){
			window.location='index.php?sessiontab=5&sub=149&state=7&sender='+id;
		}
	}
</script>
<?
...
if(isset($_POST["fdate"]))
		...
		$fdate = $_GET["fdate"];
$sql = "SELECT '".$_GET["chktype"]."' as chktype, ".$dbprefix."isaleh.inv_code,uid_receive,uid_sender,'".$page."' as page,cancel,print,".$dbprefix."isaleh.id,sano,DATE_FORMAT(sadate, '%Y-%m-%d') as sadate ,".$dbprefix."isaleh.lid
,tot_pv,tot_bv,tot_fv,remark,total,CONCAT(".$dbprefix."isaleh.name_f,' ',".$dbprefix."isaleh.name_t) as name_t,".$dbprefix."isaleh.inv_code AS smcode";
...
$sql .= ",CASE ".$dbprefix."isaleh.receive WHEN '1' THEN concat('',".$dbprefix."isaleh.receive_date) ELSE '<img src=./images/false.gif>' END AS receive1 ,".$dbprefix."isaleh.receive,
...
$sql .= " where   ".$dbprefix."isaleh.inv_from = '{$_SESSION["admininvent"]}' and  ".$dbprefix."isaleh.cancel = 0   "; //WHERE smcode='".$_SESSION['usercode']."' 
//echo $sql;
...
//echo $sql;
	...
	//	$rec->setFieldDesc("P,วันที่ซื้อ,เลขบิล,รหัสผู้ซื้อ,ชื่อผู้ซื้อ,ชนิด, PV,จำนวนเงิน,ผู้คีย์,รับของ,อ้างอิง,ซื้อสาขา,user<br>รับของ,ช่องทาง");
		...
		$rec->setFieldSpace("1%,7%,12%,5%,17%,3%,6%,6%,8%,7%,8%,8%,7%,4%,4%");
