...
$sql .= " LEFT JOIN ".$dbprefix."invent ON (".$dbprefix."asaleh.inv_code=".$dbprefix."invent.inv_code) WHERE ".$dbprefix."asaleh.mcode='$id' order by ".$dbprefix."asaleh.id asc limit 0,1 ";
...
for($i=0;$i<sizeof($bill);$i++){
    ...
    $pdf->Image('../logo.jpg',$offsetx+$offsettab-10,$offsety+$offsetnline,25); //file,x,y,w=0,h=0
...
//---------------------------------------------
    ...
    $pdf->SetY($offsety+(7*$offsetnline+2.5));
    $pdf->SetX($offsetx+$offsettab);
...
//------------------------------------------
	...
	if(count($textshows[$sano[$i]])){
		foreach($textshows[$sano[$i]] as $keyxx => $valxx):
			...
			$pdf->SetX($offsetx+(2*$offsettab));
			$pdf->Cell($offsettab,10,'-'.$valxx['text']." : ".$valxx['total'].' -'.$valxx['select']." ".''.$valxx['option']."",0,0,"L");
			$tab = $tab+5;
		endforeach;
	}
