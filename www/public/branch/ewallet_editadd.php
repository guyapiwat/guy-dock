...
showtotal1 = document.getElementById('txtMoney').value;
...
?>
<form method="post" action="ewalletoperate.php?state=<?=$_GET['id']==""?0:1?>" name="frm" id="frm" onsubmit="return checkForm(this)">
    ...
    <table border="0" width="100%">
      <tr valign="top">
        <td>
            ...
            <td width="40%"><input type="text" id="sadate" name="sadate" value="<?=$sadate==""?$_SESSION["datetimezone"]:$sadate?>" readonly="">
              <a href="javascript:NewCal('sadate','yyyymmdd',false,24)"><img src="./datetimepick/images/cal.gif" style="display:none" width="16" height="16" border="0" alt="เลือกวันที่"></a>            </td>
            </tr>
            <tr valign="top">
		...
		if(isset($_GET['id'])){
			...
			if(mysql_num_rows($rs)>0){
				...
				?>
					...
					<tr><td colspan='9' align='right'><input name='button' type='button' onclick='<?=(isset($_GET['id'])?"ebillcheck()":"ibillcheck()")?>' value='<?=$wording_lan["check"]?>' />&nbsp;<input type='submit' value='<?=$wording_lan["insert"]?>' name='ok'  id='ok' disabled='disabled' />&nbsp;<input name='reset' type='reset'  onclick="window.location='index.php?sessiontab=6&sub=146'" value='<?=$wording_lan["cancel"]?>' /></td></tr>
                </table>
				<?
			}else
			...
			?> <input name='button' type='button' onclick='<?=(isset($_GET['id'])?"ebillcheck()":"ibillcheck()")?>' value='<?=$wording_lan["check"]?>' />&nbsp;<input type='submit' value='<?=$wording_lan["insert"]?>' name='ok'  id='ok' disabled='disabled' />&nbsp;<input name='reset' type='reset'  onclick="window.location='index.php?sessiontab=6&sub=146'" value='<?=$wording_lan["cancel"]?>' /><?
		}
		?>
    	</td>
      </tr>
