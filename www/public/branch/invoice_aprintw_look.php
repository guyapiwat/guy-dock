...
for($i=0;$i<mysql_num_rows($rs);$i++){
	...
	$txtShow[$i] = "";
	$txtShow[$i] = "";
	if($txtCash[$i] > 0 or $txtCash[$i] < 0) $txtShow[$i] .= ' เงินสด : '.$txtCash[$i].'('.$optionCash[$i].'),';
	if($chkFuture[$i] == 'on') $txtShow[$i] .= 'เงินรับล่วงหน้า : '.$txtFuture[$i].'('.$optionFuture[$i].'),';
	if($txtTransfer[$i] > 0 or $txtTransfer[$i] < 0) $txtShow[$i] .= 'เงินโอน : '.$txtTransfer[$i].'('.$optionTransfer[$i].'),';
	if($txtAllCredit[$i] > 0 or $txtAllCredit[$i] < 0) $txtShow[$i] .= 'บัตรเครดิต  : '.$txtAllCredit[$i].'('.$optionCredit1[$i].' '.$optionCredit2[$i].' '.$optionCredit3[$i].')';
	if(!empty($mcode[$i])){
...
for($i=0;$i<sizeof($bill);$i++){
	...
	$pdf->Image('images/logo.jpg',$offsetx+$offsettab,$offsety+$offsetnline+2,20); //file,x,y,w=0,h=0
...
//------------------------------------------
	...
	/*$pdf->MultiCell(158,5,$txtShow[0],1,"L");*/
