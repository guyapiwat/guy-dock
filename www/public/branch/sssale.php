...
$sql .= "LEFT JOIN ".$dbprefix."member ON (".$dbprefix."asaleh.mcode=".$dbprefix."member.mcode)  where cancel=0 and send = '1' and receive != '1' and (receive <> '1') and ".$dbprefix."asaleh.sender ='1' and ".$dbprefix."asaleh.inv_code = '".$_SESSION["admininvent"]."' ";
//echo $sql;
...
//		$rec->setFieldDesc("วันที่ซื้อ,เลขบิล,รหัสผู้ซื้อ,ชื่อผู้ซื้อ,ที่อยู่จัดส่ง,มือถือ,รวมpv,รวมราคา,วันส่งของ,รับของ,อ้างอิง,user<br>จัดส่ง,user<br>รับของ,ช่องทาง"); 
	...
	/*edit search*/
		$rec->setSearch("sano,".$dbprefix."asaleh.mcode,".$dbprefix."asaleh.name_t,".$dbprefix."asaleh.sadate,tot_pv,inv_code,total,".$dbprefix."asaleh.uid");
