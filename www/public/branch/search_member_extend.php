...
if(!empty($value)){
  $sql = "SELECT ".$dbprefix."member.id,cname,ewallet,voucher,mdate,pos_cur,pos_cur2,name_t,name_f,mcode,caddress,cbuilding,cvillage,csoi,cstreet,czip,cprovinceId,cdistrictId,camphurId ";
//  $sql .= " FROM ".$dbprefix."member  where mcode = '%$value%' limit 0,1";
		...
		$sql = "select exp_date from ".$dbprefix."expdate where mid = '$mid' order by id desc limit 0,1";
		$rs123 = mysql_query($sql);
		if(mysql_num_rows($rs123)>0){
			$expdate = mysql_result($rs123,0,"exp_date");
		}
	...
	,'CD'=>"Crown Diamond",'ID'=>"Imperial Diamond",'PD'=>"Presidential Diamond",'RD'=>"Royal Diamond");
		...
		//$chkshow .= 'Voucher : '.$voucher.' <br>';
		//$chkshow .= 'Location Base : '.$locationbase.' <br>';
		//$chkshow .= "Expire Date : ".$expdate;
