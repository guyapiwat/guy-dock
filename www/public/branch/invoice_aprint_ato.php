...
$sql = "SELECT * FROM ".$dbprefix."atoasaleh WHERE id='$sano' ";
...
for($i=0;$i<sizeof($bill);$i++){
	$sql="SELECT * FROM ".$dbprefix."atoasaled WHERE sano='$bill[$i]' ";
	$rs=mysql_query($sql);
	...
	$sumpv = 0;
	for($k=0;$k<$npage;$k++){
	$sql="SELECT * FROM ".$dbprefix."atoasaled WHERE sano='$bill[$i]' LIMIT ".($k*$nitem).", $nitem ";
	$rs=mysql_query($sql);
	...
	//$pdf->Cell((3*$offsettab),10,"ใบเสร็จรับเงิน/ใบกำกับภาษี",0,0,"C"); 
	//$pdf->SetY($offsety+(6*$offsetnline));
...
//---------------------------------------------
	...
	$pdf->SetY($offsety+(7*$offsetnline));
	$pdf->SetX($offsetx+$offsettab);
	$pdf->Cell((2*$offsettab),10,"ซื้อเพื่อ",0,0,"L"); 
