...
//check(nullCheck, sizeCheck, formatCheck, dupCheck, realCheck, idName, errmsg)
	//var value = document.getElementById('mcode').value;
	//var value1 = <?=$mmcode?>;
	//check_isline(value,value1);
...
function check_isline(value,value1) {
     var req = Inint_AJAX(); //สร้าง Object
	//alert(value)
	value = str_pad(value,7,0,false);
	value1 = str_pad(value1,7,0,false);
	//alert(test);
     req.open('GET', 'search_member.php?value='+encodeURIComponent(value)+'&value1='+encodeURIComponent(value1), true); //กำหนด สถานะการทำงานของ AJAX แบบ GET และส่งข้อมูลผ่านทาง URL
     req.onreadystatechange = function() { //เหตุการณ์เมื่อมีการตอบกลับ
       ...
       var data=req.responseText; //ข้อความที่ได้มาจากการทำงานของ test3.php
			//alert(data);		//alert(req.responseText);
					if(data == 1234){
...
$sql = "SELECT * FROM ".$dbprefix."asaleh WHERE id='".$_GET['bid']."' LIMIT 1";
$rs = mysql_query($sql);
		if(mysql_num_rows($rs)<=0){
			...
			$mmcode = mysql_result($rs,0,'mcode');
		}
...
?>
<form method="post" action="holdoperate.php?state=<?=$_GET['id']==""?0:1?>" name="frm">
  <table border="0">
  <tr valign="top">
    <td>
      ...
      <tr valign="top">
        <td>
            ...
            <tr valign="top">
              ...
              <td colspan="2" nowrap><input style="background-color:#FFFF99"   size="15" type="text" id="mcode" name="mcode" value="<?=$mcode?>"><input type="button" onClick="check_isline(document.getElementById('mcode').value,<?=$mmcode?>)" value="ตรวจสอบ"><div id="mname"></div> </td>
              <td align="right">&nbsp;</td>
              <td><input style="background-color:#FFFF99;display:none" readonly size="15" type="text" id="inv_code" name="inv_code" value="<?=$inv_code?>">
                <input type="button" onClick="get_mem_listpicker_invcode()" style="display:none" value="เลือก"><div id="inv_desc"></div></td>
            </tr>            
            <tr valign="top">
              <td align="right">แผน&nbsp;</td>
              <td colspan="2">   <select name="satype" id="satype">
                <option  value="" >เลือกรูปแบบการซื้อ  </option>     
				<?php		
					foreach($arr_satype_m as $key => $value):			
