...
$sql = "SELECT * FROM ".$dbprefix."holdhead WHERE 1 = 1 $andxx $wherexx ";
...
//---------------------------------------------
	...
	$pdf->SetX($offsetx+$offsettab);
	$pdf->Cell((2*$offsettab),18,"จัดส่งตามที่อยู่",0,0,"L"); 
...
//info---------------------------------------
	...
	$pdf->Cell((2*$offsettab),18,$add[$mcode[$i]],0,0,"L"); 
	...
	$pdf->SetX($offsetx+(5*$offsettab)+20);
	$pdf->Cell((2*$offsettab),13,"โทรศัพท์",0,0,"L"); 
	...
	$pdf->Cell((2*$offsettab),13,$home_t[$mcode[$i]],0,0,"L"); 
	...
	$pdf->SetY($offsety+(6*$offsetnline)+2);
	$pdf->SetX($offsetx+(9*$offsettab)+10);
	$pdf->Cell((2*$offsettab),13,"วันที่",0,0,"L");

	$pdf->SetY($offsety+(7*$offsetnline)+2);
	$pdf->SetX($offsetx+(7.7*$offsettab)+10);
	$pdf->Cell((2*$offsettab),16,"ซื้อเพื่อ",0,0,"R"); 
	


	$pdf->SetY($offsety+(7*$offsetnline)+2);
	$pdf->SetX($offsetx+(10*$offsettab)+6);
	$pdf->Cell((3*$offsettab),16,$typedef[$sa_type[$i]],0,0,"L"); 
	$pdf->SetY($offsety+(5*$offsetnline)+2);
	$pdf->SetX($offsetx+(10*$offsettab)+6);
	$pdf->Cell((2*$offsettab),10,$sano[$i],0,0,"L"); 
	$pdf->SetY($offsety+(6*$offsetnline)+2);
	$pdf->SetX($offsetx+(10*$offsettab)+6);
	$pdf->Cell((2*$offsettab),13,$sadate[$i],0,0,"L");
...
//------------------------------------------
	...
	$pdf->Cell($offsettab,11,"จำนวนเงินทั้งสิ้น (",0,0,"L"); 
	$pdf->SetY($offsety+(20*$offsetnline));
	$pdf->SetX($offsetx+(8*$offsettab));
	$pdf->Cell($offsettab,11,")",0,0,"L"); 
	$pdf->SetY($offsety+(20*$offsetnline));
	$pdf->SetX($offsetx+(7*$offsettab));
	$pdf->Cell($offsettab,11,"",0,0,"L"); 
 ...
 */
	...
	if($k==$npage-1){
		...
		$pdf->SetY($offsety+(20*$offsetnline));
		$pdf->SetX($offsetx+(4.5*$offsettab));
		$pdf->Cell($offsettab,11,moneytotext($sum),0,0,"C"); 
	}
}
