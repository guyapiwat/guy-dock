...
$sql = "SELECT * FROM ".$dbprefix."import_stock_h WHERE id='$id' ";
...
for($i=0;$i<sizeof($bill);$i++){
    $sql="SELECT * FROM ".$dbprefix."import_stock_d WHERE sano='$bill[$i]' ";
    $rs=mysql_query($sql);
    ...
    $sumpv = 0;
    for($k=0;$k<$npage;$k++){
    $sql="SELECT * FROM ".$dbprefix."import_stock_d WHERE sano='$bill[$i]' LIMIT ".($k*$nitem).", $nitem ";
    $rs=mysql_query($sql);
    ...
    $pdf->Image('../logo.jpg',$offsetx+$offsettab,$offsety+$offsetnline+2,18); //file,x,y,w=0,h=0
    ...
    $pdf->Cell((3*$offsettab),10,"รับสินค้าเข้า ",0,0,"R"); 
