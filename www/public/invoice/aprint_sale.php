...
for($i=0;$i<mysql_num_rows($rs);$i++){ 
    ...
    $bill[$i] = $obj->isis;
    $sano[$i] = $obj->sano;    $mcode[$i] = $obj->mcode;
...
for($i=0;$i<sizeof($bill);$i++){
    ...
    $sumpv = 0;
    for($k=0;$k<$npage;$k++){
    $sql="SELECT * FROM ".$dbprefix."asaled WHERE sano='$bill[$i]' and pcode <> 'D001' LIMIT ".($k*$nitem).", $nitem ";
    $rs=mysql_query($sql);
    ...
    $pdf->SetY($offsety+2*$offsetnline+1.5);
    $pdf->SetX($offsetx+(2*$offsettab)+5);
	...
	$pdf->SetY($offsety+3*$offsetnline+3);
    $pdf->SetX($offsetx+(2*$offsettab)+5);
    ...
    $pdf->Cell($offsettab,10,"RECEIPT / TAX INVOICE",0,0,"C"); 
...
//------------------------------------------
    ...
    $pdf->SetX($offsetx+($offsettab));
  //  $pdf->Cell($offsettab,40,"Operator : ".$uid[$i]." Date : ".$logdate11." Time : ".$logtime1[$i],0,0,"L");
    ...
    for($j=0;$j<mysql_num_rows($rs);$j++){    
		...
		if($obj->pcode <> 'D001'){
			...
			$pdf->SetX($offsetx+$offsettab-5);
			$pdf->Cell($offsettab,10,$x+($k*$nitem),0,0,"L"); 
