...
for($i=0;$i<sizeof($bill);$i++){
    ...
    $pdf->SetY($offsety+$offsetnline-2);
    $pdf->SetX($offsetx+(2*$offsettab)+5);
    $pdf->Cell((4*$offsettab),10,$employee_name,0,0,"L"); 
    $pdf->SetY($offsety+2*$offsetnline-2);
    $pdf->SetX($offsetx+(2*$offsettab)+5);
    $pdf->Cell((4*$offsettab),10,$wording_lan["company_address"],0,0,"L"); 
    $pdf->SetY($offsety+3*$offsetnline-2);
    $pdf->SetX($offsetx+(2*$offsettab)+5);
    $pdf->Cell((4*$offsettab),10,$wording_lan["company_address2"],0,0,"L"); 
    ...
    $pdf->Cell((3*$offsettab),10,"บิลส่งสาขา",0,0,"R"); 
    $pdf->SetY($offsety+(5*$offsetnline)-5);
    $pdf->SetX($offsetx+(7*$offsettab)-7);
