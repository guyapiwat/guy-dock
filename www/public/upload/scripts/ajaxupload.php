<?php
session_start();
	function uploadImage($fileName,$rename, $maxSize, $maxW, $fullPath, $relPath, $colorR, $colorG, $colorB, $maxH = null){
		$folder = $relPath;
		$maxlimit = $maxSize;
		...
		if($filesize > 0){	
			...
			foreach($allowed_ext as $ext){
				if($ext==end($file_ext)){
					...
					//$newfilename = $rename.".".end($file_ext);
					...
					if(!file_exists($save)){
						...
						if($maxH == null){
							...
							if($fheight == 0 || $fwidth == 0 || $height_orig == 0 || $width_orig == 0){
								die("FATAL ERROR REPORT ERROR CODE [add-pic-line-67-orig] to AT WEB RESULTS");
							}
							if($fheight < 45){
								$blank_height = 45;
	...
	$rename = strip_tags($_REQUEST['rename']);
	
	$filesize_image = $_FILES[$filename]['size'];
	if($filesize_image > 0){
		$upload_image = uploadImage($filename,$rename, $maxSize, $maxW, $fullPath, $relPath, $colorR, $colorG, $colorB, $maxH);
...
<?php
	if($imgUploaded){
		...
		//echo '<img src="images/success.gif" width="16" height="16" border="0" style="marin-bottom: -4px;" /> Success!<br /><img src="uploads/'.$rename.'.gif" border="0" />';
	}else{
