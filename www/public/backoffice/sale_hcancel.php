...
require_once ("function.log.inc.php");
	...
	if(mysql_num_rows($sqlSS) > 0){
	 ...
	 $sqlS = "select * from ".$dbprefix."asaleh where id='$bid' and hcancel = 1";
	$sqlSS = mysql_query($sqlS);
...
//	exit;
	if(mysql_num_rows($sqlSS) > 0){
			echo "<script language='JavaScript'>alert('ไม่สามารถยกเลิกบิลนี้ได้');window.location='index.php?sessiontab=3&sub=9'</script>";	
			exit;
	}		
