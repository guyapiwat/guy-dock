...
$sql = "SELECT ".$dbprefix."asaled.id,".$dbprefix."asaled.sano,".$dbprefix."asaleh.sadate,
	".$dbprefix."asaleh.mcode,name_t,pcode,pv,bv,fv,price,qty,amt FROM ".$dbprefix."asaled ";
	...
	if($satype !="")
	$sql .= " and  ".$dbprefix."asaleh.sa_type = '$satype'";	$rs1 = mysql_query($sql);
...
//$sql = "SELECT pcode,pdesc,price,SUM(qty) AS qty,SUM(amt) AS amt FROM ".$dbprefix."asaled "; 
	...
	//$wherecause = " WHERE ";
		...
		$rec->setLink($PHP_SELF,"sessiontab=3&sub=".$_GET['sub']."&fdate=$fdate&tdate=$tdate&satype=$satype&stype=$stype");
			...
			$rec->setShowField("sadate,sano,mcode,name_t,qty,amt,pv");
			$rec->setFieldFloatFormat(",,,,2");
			$rec->setFieldDesc("วันที่,เลขบิล,รหัสสมาชิก,ชื่อสมาชิก,จำนวน,จำนวนเงินรวม,pv");
