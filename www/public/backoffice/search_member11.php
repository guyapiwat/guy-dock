...
//if(!empty($value)){
		...
		if(mysql_num_rows($result) > 0){
			...
			$sql = "SELECT lr";
			$sql .= " FROM ".$dbprefix."member  where upa_code like '%$value%' and mcode != '".$value2."' and lr=1 limit 0,1";
			$result = mysql_query($sql) or die("haha") ;
			if(mysql_num_rows($result) > 0){
				$left = '1';
			}else $left = '0';
			...
			$sql = "SELECT lr";
			$sql .= " FROM ".$dbprefix."member  where upa_code like '%$value%' and mcode != '".$value2."' and lr=2 limit 0,1";
			$result = mysql_query($sql) or die("haha") ;
			if(mysql_num_rows($result) > 0){
				$right = '1';
			}else $right = '0';
