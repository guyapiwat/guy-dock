...
//check(nullCheck, sizeCheck, formatCheck, dupCheck, realCheck, idName, errmsg)
	...
	document.getElementById('checkstate').innerHTML= "<img align='center' src='./images/loading.gif' />";
	//alert(skipval);
	startRQ(field,val,skipval,flag,errDesc,"matround","checkstate");
}
</script>
<?
if($_GET['rid']){
		$sql = "SELECT * FROM ".$dbprefix."matround WHERE rid='".$_GET['rid']."' LIMIT 1";
		$rs = mysql_query($sql);
		...
		$sql = "SELECT MAX(rcode) AS maxs FROM ".$dbprefix."matround ";
		$rs = mysql_query($sql);
...
<table width="95%" border="0">
   <tr>
     <td width="63%">
<form name="form1" method="post" action="matroundoperate.php?state=<?=$_GET['rid']==""?0:1?>">
