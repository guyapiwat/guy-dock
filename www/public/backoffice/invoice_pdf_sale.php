...
for($i=0;$i<sizeof($bill);$i++){
	$sqlkk = "SELECT * FROM ".$dbprefix."asaleh WHERE id='$bill[$i]'   ";
	...
	if(mysql_num_rows($rskk) > 0){
		...
		$Transfer[$i] = $objkk->optionTransfer;
		$chkCash[$i] = $objkk->chkCash;
		$chkFuture[$i] = $objkk->chkFuture;
		$chkTransfer[$i] = $objkk->chkTransfer;
		$chkInternet[$i] = $objkk->chkInternet;
		$chkDiscount[$i] = $objkk->chkDiscount;
		$chkCredit1[$i] = $objkk->chkCredit1;
		$chkCredit2[$i] = $objkk->chkCredit2;
		$chkCredit3[$i] = $objkk->chkCredit3;
		$txtCash[$i] = $objkk->txtCash;
		$txtFuture[$i] = $objkk->txtFuture;
		$txtTransfer[$i] = $objkk->txtTransfer;
		$txtInternet[$i] = $objkk->txtInternet;
		$txtDiscount[$i] = $objkk->txtDiscount;
		$txtCredit1[$i] = $objkk->txtCredit1;
		$txtCredit2[$i] = $objkk->txtCredit2;
		$txtCredit3[$i] = $objkk->txtCredit3;

		$optionCash[$i] = $objkk->optionCash;
		$optionFuture[$i] = $objkk->optionFuture;
		$optionTransfer[$i] = $objkk->optionTransfer;
		$optionInternet[$i] = $objkk->optionInternet;
		$optionDiscount[$i] = $objkk->optionDiscount;
		$optionCredit1[$i] = $objkk->optionCredit1;
		$optionCredit2[$i] = $objkk->optionCredit2;
		$optionCredit3[$i] = $objkk->optionCredit3;
		...
		$totall[$i] = $alltotal1[$i] = $objkk->txtCash+$objkk->txtFuture+$objkk->txtTransfer+$objkk->txtCredit1+$objkk->txtCredit2+$objkk->txtCredit3+$objkk->txtCredit4+$objkk->txtInternet+$objkk->txtDiscount+$objkk->txtCreditweb+$objkk->txtMLM+$objkk->txtCoupon;
		 
			$txtoption[$i] = $objkk->txtoption;
			$sano_ref[$i] = $objkk->sano_ref;
			$satype = $objkk->sa_type;
			if($satype == 'E' or substr($bill1[$i],1,1) == '6' or substr($bill1[$i],0,2) == 'S5'){
				$txtoption[$i] = 'สาเหตุการลดหนี้ '.$txtoption[$i];
				$txtoption1[$i] = "ตามใบกำกับภาษีเลขที่ ".$sano_ref[$i];
				$pv[$i] = 0;
...
//	echo  $chklimit[0].' : '.$chklimit[1].' : '.$chklimit[2].' : '.$chklimit[3].' : '.$kkk.'<br>';
	...
	$pdf->Image('../logo.jpg',$offsetx+$offsettab-2,$offsety+$offsetnline,25); //file,x,y,w=0,h=0
	...
	$pdf->SetX($offsetx+$offsettab);
	$pdf->Cell((2*$offsettab),10,$wording_lan["company_phone"],0,0,"L"); 
...
if($k == $npage-1){
	...
	if($satype == 'E' or substr($bill1[$i],1,1) == '6'){
		...
		$pdf->SetX($offsetx+$offsettab);
		$pdf->Cell(10*$offsettab-7,0,"จำนวนคะแนน ในใบส่งของนี้           ".number_format($pv[$i],0,'.',',')."                      คะแนน",0,0,"L"); 
		$pdf->SetY($offsety+(21*$offsetnline)+3);
...
//-------------------table---------------------
	...
	$pdf->Cell((3*$offsettab),10,"ซื้อเพื่อ",0,0,"L"); 
	$pdf->SetY($offsety+$offsetnline+15);
	$pdf->SetX($offsetx+(7*$offsettab)+33);
	$pdf->Cell((3*$offsettab),10,"เลขที่",0,0,"L"); 
	$pdf->SetY($offsety+$offsetnline+20);
	$pdf->SetX($offsetx+(7*$offsettab)+33);
