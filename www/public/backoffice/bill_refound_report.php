...
<script language="javascript" type="text/javascript">
	function sale_print(id){
		var wlink = 'invoice_aprint_refund.php?bid='+id;
...
if($_GET["sanoo"]){
$sqlupdate = "update ".$dbprefix."asaleh_refund set print = print+1 where id = '{$_GET["sanoo"]}'";
...
$sql = "SELECT ".$dbprefix."asaleh_refund.sano1,".$dbprefix."asaleh_refund.inv_code,".$dbprefix."asaleh_refund.uid,cancel,print,".$dbprefix."asaleh_refund.id,sano,DATE_FORMAT(sadate, '%Y-%m-%d') as sadate 
,tot_pv,tot_bv,tot_fv,remark,total,CONCAT(".$dbprefix."asaleh_refund.name_f,' ',".$dbprefix."asaleh_refund.name_t) as name_t,".$dbprefix."asaleh_refund.mcode AS smcode";
...
$sql .= ",CASE ".$dbprefix."asaleh_refund.uid WHEN '' THEN ".$dbprefix."asaleh_refund.inv_code ELSE ".$dbprefix."asaleh_refund.uid END AS uid ";
...
$sql .= ",CASE ".$dbprefix."asaleh_refund.receive WHEN '1' THEN concat('',".$dbprefix."asaleh_refund.receive_date) ELSE '<img src=./images/false.gif>' END AS receive ,
CASE ".$dbprefix."asaleh_refund.sender WHEN '1' THEN concat('',".$dbprefix."asaleh_refund.sender_date) ELSE '<img src=./images/false.gif>' END AS sender  ";

$sql .= "FROM ".$dbprefix."asaleh_refund ";
//$sql .= "LEFT JOIN ".$dbprefix."member ON (".$dbprefix."asaleh_refund.mcode=".$dbprefix."member.mcode) "; //WHERE smcode='".$_SESSION['usercode']."' 
$sql .= " where  ".$dbprefix."asaleh_refund.sano not like 'PO%' and ".$dbprefix."asaleh_refund.inv_code = '{$_SESSION["admininvent"]}'  "; //WHERE smcode='".$_SESSION['usercode']."' 
...
$sql .= " and sadate >= '$fdate'  and sadate <= '$tdate'  ";
 ...
 //$sql .= " WHERE  ".$dbprefix."asaleh_refund.sa_type <> 'I'";
...
//$fdate = $monthmonth[0].'-'.$monthmonth[1];
	...
	//	$rec->setSearchDesc("เลขบิล,รหัสผู้ซื้อ,ชื่อผู้ซื้อ,สาขา,วันที่ซื้อ,จำนวนรวม  PV,จำนวนเงินรวม,พนักงาน");
		...
		//if($acc->isAccess(2))
		if(isset($_GET['lp'])){
			$rec->setEdit("index.php","id","id","sessiontab=3&sub=139&refund=true");
		}
		$rec->showRec(1,'SH_QUERY');
/*$sql = "SELECT cancel,".$dbprefix."asaleh_refund.id,sano,sadate,tot_pv,total,name_t,".$dbprefix."asaleh_refund.mcode AS smcode";
