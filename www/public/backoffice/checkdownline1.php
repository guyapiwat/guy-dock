...
if(!empty($_POST))
{
	...
	//exit;
	$sql = "select * from ".$dbprefix."member where mcode='$mcode[$k]' ";
	$rs=mysql_query($sql);
	...
	//		echo $sqlt.'<br>';
		...
		mysql_query("insert into  ".$dbprefix."checkdownline (mcode,name_t,total,fast,weakstrong,matching,star,onetime,alltotal,upa_code,lv,lr,mdate)
	...
	//	exit;
		...
		//$sql .= "DELETE FROM ".$dbprefix."checkdownline where mdate > '2013-09-30' ";
		if(!empty($fdate)) mysql_query("DELETE FROM ".$dbprefix."checkdownline where mdate > '$tdate' ");
...
$sql = "SELECT * FROM ".$dbprefix."checkdownline ";
