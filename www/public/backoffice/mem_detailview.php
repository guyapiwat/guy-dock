	...
	//		$notfound = "[<a href=\"javascript:window.location='index.php?sessiontab=1';\">ไปหน้าสมาชิก</a>]";
      //  	dialogbox("50%","#990000","ไม่พบข้อมูลตามเงื่อนไข",$notfound);
		echo '<center>ไม่พบข้อมูลตามเงื่อนไข</center>';
			...
			$meminfo['provinceId'] = getprovince_1($row->provinceId);
			$meminfo['amphurId'] = getamphur_1($row->amphurId);

			$meminfo['districtId'] = getdistrict_1($row->districtId);
			$meminfo['zip'] = $row->zip;
			...
			$meminfo['mtype1'] = $row->mtype1;

	
			$meminfo['iname_f'] = $row->iname_f;
			$meminfo['iname_t'] = $row->iname_t;
			$meminfo['iacc_no'] = $row->iacc_no;
...
<form name='frm' method="post">
  <p><img src="images/logo.jpg" height=100> <br>
  <?
  ...
  <tr>
    ...
    <td><span class="texd"> 
			...
			foreach($arr_mtype1 as $key => $value):		
				if($meminfo['mtype1']==$key)echo $value;		
			endforeach;
			?>
	</span></td>
...
<table width="100%" border="0">
  ...
  <tr>
    ...
    <td class="texd">&nbsp;<? echo ($meminfo['facebook_name']!="")?$meminfo['facebook_name']:$meminfo['nodata']; ?></td>
    <td align="right" class="texh">&nbsp;</td>
    <td class="texd">&nbsp;</td>
  </tr>
    <tr>
    <td align="right" class="texh"><b>Facebook  :</b></td>
    <td class="texd">&nbsp;<? echo ($meminfo['line_id']!="")?$meminfo['line_id']:$meminfo['nodata']; ?></td>
    <td align="right" class="texh">&nbsp;</td>
    <td class="texd">&nbsp;</td>
  </tr>
   <tr>
