...
//--running
		...
		switch($type){
			...
			case 'SH_RESULT':
				...
				if(mysql_num_rows($this->rs)<=0){
					...
					if($this->del){
						echo "<td bgcolor='#99CCCC' style='".$style_l.$style_t.$style_b."'>";
						echo "<a href=\"javascript:if(confirm('��س��׹�ѹ���ź�����Ŵѧ�����')) document.".$this->delFormAttr[0].".submit();\">ź</a>";
					...
					if($this->sum==true){
						...
						if($this->allSum==true){
							...
							echo "<td align='right' colspan='".($col-sizeof($showList))."' style='color:#FFFFFF;".$style_b.$style_l."'>�ҡ</td>";						
							for($i=0;$i<sizeof($showList);$i++){
								if($showSumList[$i]==true){
	...
	/*	$endtm = date("H:i:s");
		...
		//echo "S[".$starttm."]-E[".$endtm."] ������ [".mysql_result($rs,0,'tm')."]<br>";
		echo "������ [".mysql_result($rs,0,'tm')."]<br>";
	*/	
	}
...
//--running
	public function showRec($isshow,$type){
		...
		switch($type){
			...
			case 'SH_RESULT':
				...
				//show report
				if(mysql_num_rows($this->rs)<=0){
					echo "<tr><td bgcolor='#990000' align='center'><font color='#FFFFFF'>��辺������</font></td></tr>";
					echo "<tr></tr><td align='center'>[<a href='javascript:history.back()'>Back</a>]</td></tr>";
				}else{
					echo "<tr><td colspan='".$col."'><table><tr><td>";
					if($this->search){
					...
					if($this->pageLinkShow['top']){
						echo "<tr><td align='".$this->pageLinkAlign."' colspan='".$col."'>";
						echo "<div><font color='#999999'>";
						echo "�ʴ� ".($this->cPage==$this->nPage?$this->nInfo-(($this->nPage-1)*$this->lPage):$this->lPage);
						echo " ��¡�� �ҡ ".$this->nInfo." ��¡�� �ӹǹ ".$this->nPage." ˹�� ˹���� ";
						echo "<input name='lp' type='text' size='1' style='text-align:right;font-size:10px;background-color: ";
						echo "#CCCCCC;border: 1px solid #000000;' onKeyUp='pageset(window.event.keyCode,\"lp\",\"lp\" )' ";
						echo "title='Enter ���ͻ�Ѻ�觡���ʴ��� \n(ALL=�ʴ�������)' value='".$this->lPage."'> ������</font></div>";
						$this->pageShow();
						echo "</td></tr>";
					}
					echo "<form name='".$this->delFormAttr[0]."' id='".$this->delFormAttr[0]."' action='".$this->delFormAttr[1]."' method='".$this->delFormAttr[2]."'>";
	...
	*/	
	...
	public function pageShow(){
		...
		echo "<font color='#999999'> | ��ѧ˹�� <input type='text' name='pg' id='pg'  size='1' ";
		echo "style='text-align:right;font-size:10px;background-color: #CCCCCC;border: 1px solid #000000;' ";
		echo "onKeyUp='pageset(window.event.keyCode,\"pg\",\"pg\")' title='Enter ������ѧ˹�ҷ���ͧ���'/></font>";
	}
	//row_flag will be 'CALC' or 'NCALC'
	//CALC refer to calculate number of record
	//NCALC refer to not calculate number of record
		...
		switch($type){
			...
			case 'SH_RESULT':
				...
				if(mysql_num_rows($this->rs)<=0){
					...
					if($this->del){
						echo "<td bgcolor='#99CCCC' style='".$style_l.$style_t.$style_b."'>";
						echo "<a href=\"javascript:if(confirm('��س��׹�ѹ�����������Ŵѧ�����')) document.".$this->delFormAttr[0].".submit();\">���</a>";
						echo "<input name='delbutton' id='delbutton' type='checkbox' onclick='checkall()'>";
						echo "</td>";
					}
					if($this->edit){
			...
			case 'SH_RESULT':
				...
				if(mysql_num_rows($this->rs)<=0){
					...
					if($this->pageLinkShow['top']){
						...
						echo "<input name='lp' type='text' size='1' style='text-align:right;font-size:10px;background-color: ";
						echo "#CCCCCC;border: 1px solid #000000;' onKeyUp='pageset(window.event.keyCode,\"lp\",\"lp\" )' ";
						echo "title='Enter ���ͻ�Ѻ�觡���ʴ��� \n(ALL=�ʴ�������)' value='".$this->lPage."'> ".$wording_lan["word"]["data"]."</font></div>";
						$this->pageShow();
						echo "</td></tr>";
					}
					echo "<form name='".$this->delFormAttr[0]."' id='".$this->delFormAttr[0]."' action='".$this->delFormAttr[1]."' method='".$this->delFormAttr[2]."'>";
