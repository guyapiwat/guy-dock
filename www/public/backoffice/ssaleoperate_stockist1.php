...
*/
	//$sql="insert into ".$dbprefix."risaleh (id,name_f,name_t, hono, sano, sadate,  mcode,  sa_type, inv_code,  total, tot_pv, uid ) values ('$mid' ,'$name_f' ,'$name_t' ,'$hono' ,'".$_POST['hid']."' ,'$sadate' ,'$mcode', '$satype' ,'".$inv_code."' ,'$total' ,'$tot_pv' ,'".$_SESSION['adminusercode']."') ";
	...
	logtext(true,$_SESSION['adminusercode'],'‡ª‘¥∫‘≈ √—∫ Ëß¢ÕßStockist',$mid);
		//echo "$sql<br>";
	if (! mysql_query($sql)) {
		echo "<script language='JavaScript'>alert('This Process can not be success.Please try again.');window.location='index.php?sessiontab=".$_REQUEST["sessiontab"]."&sub=".$_REQUEST["sub"]."&page=".$_REQUEST["page"]."&chktype=".$_REQUEST["chktype"]."''</script>";	
		exit;
	...
	for($i=0;$i<sizeof($pcode);$i++){
		...
		logtext(true,$_SESSION['adminusercode'],'‡ª‘¥∫‘≈√—∫ Ëß¢Õß',$mid);
		//echo "$sql<br>";
		mysql_query($sql);
		$inv_ref = $_SESSION["admininvent"];
		//$inv_code = $_SESSION["admininvent"];
		$satype = "A";
		updatestockcardr($dbprefix,$inv_code,$inv_ref,$inv_ref,'RI'.$hono,$$_POST['hid'],$sadate,$rccode,$satype,$pcode[$i],$_SESSION['adminusercode'],$qty[$i],$price[$i],$totalprice[$i]);
		calc_minusProductr($dbprefix,$pcode[$i],$inv_ref,$qty[$i],'RI'.$hono,$_SESSION['adminusercode'],$inv_code);
		
		
		updatestockcardr1($dbprefix,$inv_code,$inv_code,$inv_ref,'RI'.$hono,$$_POST['hid'],$sadate,$rccode,$satype,$pcode[$i],$_SESSION['adminusercode'],$qty[$i],$price[$i],$totalprice[$i]);
		calc_plusProductr($dbprefix,$pcode[$i],$inv_code,$qty[$i],'RI'.$hono,$_SESSION['adminusercode'],$inv_code);

		updatestockcard1($dbprefix,$inv_code,$inv_code,$inv_ref,'RI'.$hono,$$_POST['hid'],$sadate,$rccode,$satype,$pcode[$i],$_SESSION['adminusercode'],$qty[$i],$price[$i],$totalprice[$i]);
		calc_plusProduct($dbprefix,$pcode[$i],$inv_code,$qty[$i],'RI'.$hono,$_SESSION['adminusercode'],$inv_code);
...
//echo "<script language='JavaScript'>window.location='index.php?sessiontab=6&sub=17&chktype=$chktype'</script>";	
