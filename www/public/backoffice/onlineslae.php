...
if(isset($_POST["fdate"]))
		...
		$fdate = $_GET["fdate"];
$sql = "SELECT '".$page."' as page,mobile,cancel,print,".$dbprefix."transfersale_h.id,".$dbprefix."transfersale_h.lid,sano,DATE_FORMAT(sadate, '%Y-%m-%d') as sadate 
...
$sql .= "FROM ".$dbprefix."transfersale_h ";
$sql .= "LEFT JOIN ".$dbprefix."member ON (".$dbprefix."transfersale_h.mcode=".$dbprefix."member.mcode) where 1=1 "; //WHERE smcode='".$_SESSION['usercode']."' 
//echo $sql;
...
//$wherecause = " WHERE "
	...
	if($_GET['state']==1){
		...
		if(isset($page))
			$rec->setCurPage($page);
		$rec->setShowField("id,smcode,name_t,mobile,total,paytype1,credittype,ability,sano");
