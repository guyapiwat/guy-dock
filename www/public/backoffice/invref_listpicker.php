...
function selectitem(inv_code,inv_desc){
		...
		doc.frm.inv_ref.value=inv_code;
...
$sql = "SELECT inv_code,inv_desc,CASE inv_type WHEN '1' THEN 'Center' WHEN '2' THEN 'Stockist' WHEN '3' THEN 'Mobile' END AS inv_type FROM ".$dbprefix."invent ";
//$wherecause = " WHERE ";
	...
	if($_GET['state']==1){
		...
		if(isset($page))
			$rec->setCurPage($page);
		$rec->setShowField("inv_code,inv_desc");
		$rec->setFieldDesc("รหัส,ชื่อ");
		$rec->setFieldAlign("center,left,left");
		$rec->setFieldSpace("30%,70%,30%");
		$rec->setFieldLink("");
