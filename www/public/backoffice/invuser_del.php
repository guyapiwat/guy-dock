...
require_once ("function.log.inc.php");
	...
	for ($i=0;$i<$numpost;$i++) {
		...
		$rs=mysql_query("SELECT * FROM ".$dbprefix."user WHERE usertype='2' AND uid='".$postval[$postkey[$i]]."' LIMIT 1");
		...
		logtext(true,$_SESSION['adminusercode'],'ลบผู้ใช้สาขา',$row->usercode);
		mysql_free_result($rs);
		//====================LOG===========================
$text="uid=".$_SESSION["adminusercode"]." action=invuser_del =>delete from ".$dbprefix."user where uid='".$uid."'";
writelogfile($text);
//=================END LOG===========================
