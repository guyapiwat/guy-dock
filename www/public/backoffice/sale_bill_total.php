...
rpdialog_amount_lb1($_GET['sub'],$fdate,$tdate);
if(!empty($_GET['struid'])){$struid = $_GET['struid'];}else {if(!empty($_POST['struid'])){$struid = $_POST['struid'];}else{$struid='';}}
...
if($fdate){
		...
		/*edit sum if branch*/
		//if($inv!="")$where .= "and ash.inv_code ='{$inv_code}' ";
		/*-----------------*/

        if($uid!="")$where .= "and ash.lid ='{$uid}' ";
        ...
        if($sspv!="")$where .= "and ash.checkportal ='{$sspv}' and ash.checkportal <> '5' ";  
