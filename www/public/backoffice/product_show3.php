...
<script language="javascript" type="text/javascript">
	...
	function saleadd(pcode,pdesc,price,pv,bv,fv,qty){
		...
		for(i=0;i<(tag.length-skip)/13;i++){
			...
			place += "<td style='"+style_l+style_bd+"' align='right'><input onkeyup='cal()' onKeyPress='return chknum(window.event.keyCode)' onChange=\"if(this.value == '' || this.value.substring(0,1) == '0'){alert('ไม่สามารถใส่ค่า 0 หรือ ช่องว่างได้');this.value=1;cal();}\" style='text-align:right;' name='qty[]' type='text' size='5' value='" + num + "' ></td>";
		...
		//alert(tag.length)
			...
			place += "<td style='"+style_l+style_bd+"' align='right'><input  onKeyPress='return chknum(window.event.keyCode)' onkeyup='cal()' onChange=\"if(this.value == '' || this.value.substring(0,1) == '0'){alert('ไม่สามารถใส่ค่า 0 หรือ ช่องว่างได้');this.value=1;cal();}\" name='qty[]' style='text-align:right;' type='text' size='5' value='1'	></td>";
		...
		place += "</tr>";
		place += "<tr><td colspan='9' align='right'><input name='button' type='button' onclick='<?=(isset($_GET['id'])?"ebillcheck()":"ibillcheck()")?>' value='ตรวจสอบ' />&nbsp;<input type='submit' value='บันทึก' name='ok' id='ok'  disabled='disabled' />&nbsp;<input name='reset' type='reset'  onclick=\"window.location='index.php?sessiontab=6&sub=23'\" value='ยกเลิก' /></td></tr>";
		place += "</table>";
...
$sql = "SELECT a.pcode,a.pv,a.bv,a.fv,a.price,a.pdesc,a.price,a.qty FROM ".$dbprefix."product a";
 //echo $sql;
...
$rec->setLink($PHP_SELF,"sessiontab=3&sub=23");
$rec->setBackLink($PHP_SELF,"sessiontab=3");
if(isset($page))
	$rec->setCurPage($page);
$rec->setShowField("pcode,pdesc");
$rec->setFieldDesc("รหัส,รายละเอียด");
$rec->setFieldAlign("center,left,center,right,right");
$rec->setFieldSpace("20%,80%,20%");
$rec->setFieldLink(",");
