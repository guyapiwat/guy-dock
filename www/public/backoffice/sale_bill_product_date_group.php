...
<?
	...
	switch($stype){
		case 1:
			...
			$sregister = 0;
			if($_REQUEST){
				include("sale_bill_product_date_group1.php");
				break;
			}
	}
?>
...
<? function searchbox($dbprefix,$inv_code,$inv_code1,$fdate,$tdate,$satype,$logistic,$strpv,$strtotal,$struid,$sspv,$strtype,$strSearch,$sregister,$fpcode,$tpcode){
	...
	?>
<form style="margin-bottom:0;" action="index.php?sessiontab=<?=$_GET["sessiontab"]?>&sub=54" method="post"  id="frm" name="frm">
...
<select name="inv_code" id="inv_code"  >
  ...
  <!-- <br> <input tabindex="2" style="background-color:#FFFFFF" readonly="" type="text" name="fpcode" id="fpcode" size="20"  maxlength="20" value="<?=$fpcode?>" />
