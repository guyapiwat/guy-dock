...
if(isset($_GET['state'])){
	$sql = "SELECT MAX(id) AS id FROM ".$dbprefix."ostockh ";
	$rs = mysql_query($sql);
...
if($_GET['state']==0){
	$satype = 'BBHO';
	$mid = ++$sano;
	//logtext(true,$_SESSION['adminuserid'],'?????? ????? : '.$sano,$mid);
	//
		if(empty($chkInternet))$txtInternet = 0;
//??S??????subinventory56110001
//??????? run ??????????????
...
//echo $sano;
//exit;
	$sql="insert into ".$dbprefix."ostockh (id,  sano, sadate,  sa_type, inv_code,inv_coden,inv_ref,inv_refn,  total, tot_pv,uid,txtoption,mapping_code,rccode ) 	values ('$mid' ,'$sano' ,'$sadate' ,'$satype' ,'$inv_ref','".$_SESSION["adminusername"]."','$inv_ref' ,'{$_SESSION["adminusername"]}' ,'$total' ,'$tot_pv','".$_SESSION['adminusercode']."','$txtoption','$mapping_code','$rccode') ";
		mysql_query($sql);
	...
	$sql="insert into ".$dbprefix."ostockd (sano,pcode,pdesc,price,pv,qty,amt,inv_code,inv_ref) values ('$mid','$pcode[$i]','$pdesc[$i]','$price[$i]' ,'$pv[$i]','$qty[$i]','$totalprice[$i]','".$_SESSION["adminusercode"]."','{$_SESSION["adminusercode"]}') ";
	...
	$rs = mysql_query($sql);
		...
		//	updatestockcardr($dbprefix,$mcode,$inv_code,$inv_ref,$sano,$sanox,$sadate,$rccode,$satype,$pcode[$i],$uid,$qty[$i],$price[$i],$totalprice[$i]);
		//calc_minusProduct($dbprefix,$pcode[$i],'',$qty[$i],$sano,$_SESSION['adminusercode'],$inv_code);
		
	}
	
		$object_stocks = new stocks ();
		$object_stocks->set_data($mid,"ostock","sender","1"); 
	...
	//logtext(true,$_SESSION['adminusercode'],'????????',$id);
	if(empty($chkInternet))$txtInternet = 0;
	//updateEwallet1($dbprefix,$mcode,$txtInternet,$id);
	$sql="update ".$dbprefix."ostockh set sano='$id', id='$id', ";
...
//=================END LOG===========================
	mysql_query($sql);
	$sql = "select * from ".$dbprefix."ostockd where sano='$id'";
	...
	mysql_query("DELETE FROM ".$dbprefix."ostockd WHERE sano='$id'");
	if(isset($_POST['pcode'])){
		$pcode=$_POST['pcode'];
		$pdesc=$_POST['pdesc'];
		$price=$_POST['price'];
	...
	for($i=0;$i<sizeof($pcode);$i++){
		$sql="insert into ".$dbprefix."ostockd (sano,pcode,pdesc,price,pv,bv,fv,qty,amt) values ('$id','$pcode[$i]','$pdesc[$i]','$price[$i]' ,'$pv[$i]','$bv[$i]','$fv[$i]','$qty[$i]','$totalprice[$i]') ";
...
$_SESSION["perbuy"] = 0;
echo "<script language='JavaScript'>window.location='index.php?sessiontab=6&sub=20'</script>";	
...
function updateEwallet1($dbprefix,$mcode,$oldInternet,$id){
	$sql = "select * from ".$dbprefix."ostockh where sano='$id'";
	$result = mysql_query($sql);
		$data = mysql_fetch_object($result);
