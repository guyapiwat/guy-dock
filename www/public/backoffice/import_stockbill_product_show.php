...
$sql = "SELECT *,".$dbprefix."product.qty as qtys FROM ".$dbprefix."product LEFT JOIN ".$dbprefix."productgroup  ON (".$dbprefix."productgroup.id=".$dbprefix."product.group_id) where bf = 1";
//echo $sql;
...
if(isset($page))
	$rec->setCurPage($page);
$rec->setShowField("pcode,pdesc,price,pv,qtys,groupname");
$rec->setFieldDesc("รหัส,รายละเอียด,ราคา,คะแนน,คงเหลือ,ประเภท");
$rec->setFieldAlign("center,left,right,right,left");
$rec->setFieldSpace("8%,45%,10%,10%,22%");
$rec->setFieldLink(",");
$rec->setSpecial("./images/add_pic.gif","","saleadd","pcode,pdesc,price,pv,qtys","IMAGE","");
