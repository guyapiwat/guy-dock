...
,tot_pv,tot_bv,tot_fv,remark,total,CONCAT(".$dbprefix."asaleh.name_f,' ',".$dbprefix."asaleh.name_t) as name_t,".$dbprefix."asaleh.mcode AS smcode,'00000' as br";
...
if(!empty($inv)){
				...
				$sql_1 = "select * from ".$dbprefix."invent  where inv_code = '$inv' ";
				$r1 = mysql_query($sql_1);
				$rs1=mysql_fetch_object($r1);
				echo '<center><font size=4>บิล Branch '.$rs1->inv_desc.'</font></center>';
				echo '<center><font size=4>วันที่ '.$fdate.' ถึง '.$tdate.'</font></center>';
...
//
	...
	if($_GET['state']==1){
		...
		$rec->setShowField("sadate,sano,name_t,id_card,br,total_tax,tax,cencels");
