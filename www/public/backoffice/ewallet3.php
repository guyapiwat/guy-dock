...
CASE checkportal WHEN '1' THEN 'HQ' WHEN '2' THEN 'Branch' WHEN '3' THEN 'ONLINE'  WHEN '9' THEN 'Commission' WHEN '8' THEN 'RESET CAL' END AS checkportal,".$dbprefix."voucher.uid as uid,sano,txtCash,txtFuture,txtTransfer,txtCredit1+txtCredit2+txtCredit3 as txtCredit,sadate,txtMoney ,".$dbprefix."member.name_t ";
...
$sql .= " LEFT JOIN ".$dbprefix."member on (".$dbprefix."voucher.mcode=".$dbprefix."member.mcode) and cancel = 0 ";
...
//$wherecause = " WHERE ";
	...
	if($_GET['state']==1){
		...
		$rec->setLink($PHP_SELF,"sessiontab=3&sub=232");
		...
		$rec->setFieldDesc("วันที่ซื้อ,เลขบิล,รหัสผู้ซื้อ,ชื่อผู้ซื้อ,จำนวน,สาขา หรือ พนักงาน,ช่องทาง");
