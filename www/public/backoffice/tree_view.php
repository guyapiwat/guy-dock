...
$sql = "SELECT sum(tot_pv) AS all_pv FROM ".$dbprefix."asaleh WHERE mcode='$mcode' and  sa_type='A' ";
		...
		echo "<td> รหัส <a href='index.php?sessiontab=1&sub=8&cmc=$mcode'>".$mcode."</a> ชื่อ ".$mname." ตำแหน่ง ".$arrpos[$mpos]." คะแนน topup ".$all_pv." </td></tr>";
		...
		for($i=0;$i<mysql_num_rows($result1);$i++){
			...
			echo "<td> รหัส <a href='index.php?sessiontab=1&sub=8&cmc=$ccode'>".$ccode."</a> ชื่อ ".$cname." ตำแหน่ง ".$arrpos[$mpos]." คะแนน topup ".$all_pv." </td>";
			echo "</tr></table></div>";
			echo "</td></tr>";
		}
		echo "</table></div>";
