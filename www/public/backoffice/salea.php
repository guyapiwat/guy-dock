<script language="javascript" type="text/javascript">
	...
	function sale_cancel(id){
		if(confirm("ต้องการยกเลิกบิลนี้")){
			window.location='index.php?sessiontab=3&sub=21&state=3&bid='+id;
		}
	}
</script>
<?
...
$sql = "SELECT DATE_FORMAT(".$dbprefix."asaleh.sadate, '%d-%m-%Y') as sadate,sano,".$dbprefix."asaleh.mcode AS smcode,name_t,total,tot_pv,".$dbprefix."asaleh.uid as uid";
...
$sql .= ",CASE sa_type WHEN 'I' THEN '<img src=./images/true.gif>' ELSE '' END AS invent ,".$dbprefix."member.pos_cur as pos_cur ";

$sql .= "FROM ".$dbprefix."asaleh ";
$sql .= "LEFT JOIN ".$dbprefix."member ON (".$dbprefix."asaleh.mcode=".$dbprefix."member.mcode) where ".$dbprefix."asaleh.sa_type = 'A' ";
/*$sql = "  
SELECT DATE_FORMAT(".$dbprefix."holdhead.sadate, '%d-%m-%Y') as sadate,sano,tot_pv,total,name_t,".$dbprefix."holdhead.mcode AS smcode";
$sql .= ",CASE sa_type WHEN 'Q' THEN '<img src=./images/true.gif>' ELSE '' END AS preserve ";
$sql .= ",CASE sa_type WHEN 'A' THEN '<img src=./images/true.gif>' ELSE '' END AS ability ";
$sql .= ",CASE sa_type WHEN 'C' THEN '<img src=./images/true.gif>' ELSE '' END AS imd ,".$dbprefix."member.pos_cur as pos_cur ";
$sql .= "FROM ".$dbprefix."holdhead ";
$sql .= "LEFT JOIN ".$dbprefix."member ON (".$dbprefix."holdhead.mcode=".$dbprefix."member.mcode) where ".$dbprefix."holdhead.sa_type = 'A' "; //WHERE smcode='".$_SESSION['usercode']."' 
echo $sql.'<br><br>';*/
...
//$wherecause = " WHERE ";
	...
	if($_GET['state']==1){
		...
		$rec->setShowField("sadate,sano,smcode,name_t,pos_cur,preserve,ability,invent,tot_pv,total,uid");
		...
		$rec->setFieldDesc("วันที่ซื้อ,เลขบิล,รหัสผู้ซื้อ,ชื่อผู้ซื้อ,ตำแหน่ง,รักษายอด,ทำคุณสมบัติ,ส่งศูนย์, PV,จำนวนเงินรวม,สาขา หรือ พนักงาน");
		$rec->setFieldAlign("center,center,center,left,center,center,center,right,right,right,right,right,center");
		$rec->setFieldSpace("10%,5%,7%,15%,5%,7%,7%,7%,7%,7%,7%,7%,10%,15%,15%,8%");
		...
		if($acc->isAccess(4)){
			$rec->setDel("index.php","id","id","sessiontab=3&sub=21");
			$rec->setFromDelAttr("maindel","./index.php?sessiontab=3&sub=21&state=1","post","delfield");
		}
		//var_dump($acc->isAccess(2));
		//exit;
		if($acc->isAccess(2))
			$rec->setEdit("index.php","id","id","sessiontab=3&sub=21");
