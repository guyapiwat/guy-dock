...
<?
		...
		if($_GET['state']==1){
		include("comsn/com_d/rep_change.php");
		}
		...
		$rec->setLink($PHP_SELF,"sessiontab=4&sub=32&ftrcode=$ftrcode&fmcode=$fmcode");
		$rec->setBackLink($PHP_SELF,"sessiontab=4");
		if(isset($page))
			$rec->setCurPage($page);
		//$rec->setShowIndex(true);
		$rec->setShowField("rcode,mcode,name_t,acc_no,bankname,total,tax,bonus");
		$rec->setFieldDesc("รอบ,รหัสมาชิก,ชื่อ,เลขบัญชี,ธนาคาร,คอมมิชชัน,ภาษี 3%,สุทธิ");
...
<form name="rform" method="post" action="./index.php?sessiontab=4&sub=32">
