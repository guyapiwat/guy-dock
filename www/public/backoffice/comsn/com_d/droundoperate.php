...
if($_GET['state']==0){
	...
	$sql="insert into ".$dbprefix."dround (rcode,  rdate, fdate,  tdate,paydate,  calc,remark,fpdate,  tpdate) values ('$rcode' ,'$rdate' ,'$fdate' ,'$tdate' ,'$paydate' ,'$calc' ,'$remark' ,'$fdate' ,'$tdate') ";
	if (! mysql_query($sql)) {
		...
		echo "<script language='JavaScript'>window.location='../../index.php?sessiontab=4&sub=28'</script>";	
	}
}else if($_GET['state']==1){
	
	$sql="update ".$dbprefix."dround set rcode='$rcode', rdate='$rdate', fdate='$fdate', tdate='$tdate',fpdate='$fpdate', tpdate='$tpdate' where rid= '$oid' ";
	//logtext(true,$_SESSION['adminuserid'],$sql);
	mysql_query($sql);
//	echo $sql;
	$sql="update ".$dbprefix."dround set paydate='$paydate', calc='$calc', remark='$remark' where rid= '$oid' ";
//	echo $sql;
	if (!mysql_query($sql)) {
		ob_end_flush();
		echo "<font color='#FF0000'>error</font><br>";
