...
<?
if($_GET['rid']){
		$sql = "SELECT * FROM ".$dbprefix."dround WHERE rid='".$_GET['rid']."' LIMIT 1";
		$rs = mysql_query($sql);
		...
		$sql = "SELECT MAX(rcode) AS maxs FROM ".$dbprefix."dround ";
		$rs = mysql_query($sql);
...
<table width="95%" border="0">
   <tr>
     <td width="63%">
<form name="form1" method="post" action="./comsn/com_d/droundoperate.php?state=<?=$_GET['rid']==""?0:1?>">
<table width="98%" border="0">
  ...
  <tr>
    ...
    <td><input name="button" id="button" type="button" onclick="<?=(isset($_GET['rid'])?"earoundcheck()":"iaroundcheck()")?>" value="ตรวจสอบ" />
             ...
             &nbsp;
            <input name="reset" id="reset" type="reset"  onclick="window.location='index.php?sessiontab=4&sub=28'" value="ยกเลิก" /></td>
  </tr>
</table>
</form><td width="37%">
