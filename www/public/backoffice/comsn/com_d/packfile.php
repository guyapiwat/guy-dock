...
?>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<!--a href="./comsn/com_c/rep_cmbonus1_comsn_print.php?ftrcode=<?=$ftrcode?>&fmcode=<?=$fmcode?>" target="_blank"><img border="0" src="./images/Amber-Printer.gif">พิมพ์ทั้งหมด</a><br/-->
...
<?
		...
		$sql = "SELECT 'N' as Yes,oon,a.total-oon as ttttt,b1.acc_no,b.bankname,b1.mobile";
		$sql .= ",a.id,a.rcode,a.mcode,m.inv_code,m.inv_desc as name_t,a.pv,a.pvb,a.total  ";
		$sql .= "FROM ".$dbprefix."oon n,".$dbprefix."cmbonus1 a ";
		$sql .= " LEFT JOIN ".$dbprefix."invent m ON a.mcode=m.inv_code ";
		$sql .= " LEFT JOIN ".$dbprefix."member b1 ON b1.mcode=m.code_ref  ";
		$sql .= " LEFT JOIN ".$dbprefix."bank b ON b1.bankcode=b.bankcode ";
		if($ftrcode!=""&&$fmcode!=""){
		...
		$rec->setLink($PHP_SELF,"sessiontab=4&sub=37&ftrcode=$ftrcode&fmcode=$fmcode");
		$rec->setBackLink($PHP_SELF,"sessiontab=4");
		if(isset($page))
			$rec->setCurPage($page);
		//$rec->setShowIndex(true);
		//$rec->setShowField("rcode,name_t,acc_no,total,bankname,mobile,Yes,oon,ttttt");
		$rec->setShowField("inv_code,name_t,acc_no,ttttt,bankname,mobile,Yes");
		$rec->setFieldDesc("รหัส,ชื่อผู้รับเงิน,เลขที่บัญชี,จำนวนเงิน,ธนาคาร,Fax Number or Mobile,Beneficiary");
...
<form name="rform" method="post" action="./index.php?sessiontab=4&sub=37">
