...
require_once ("function.log.inc.php");
	...
	for ($i=0;$i<$numpost;$i++) {
		...
		$rs=mysql_query("SELECT * FROM ".$dbprefix."cmbonus1  WHERE id='".$postval[$postkey[$i]]."' LIMIT 1");
		...
		if (mysql_num_rows($rs)>0){
            ...
            <?
			mysql_query("update ".$dbprefix."cmbonus1   set status = 0,pv = pv+pvb,pvb = 0  where id='$id' and status = 1 ");
			//====================LOG===========================
$text="uid=".$_SESSION["adminuserid"]." action=cmbonus1_update=>update ".$dbprefix."cmbonus1   set status = 0 ,pv = pv+pvb where id='$id'  and status = 1  ";
writelogfile($text);
//=================END LOG===========================
		}
		logtext(true,$_SESSION['adminuserid'],'ย้ายการจ่ายตัง แผน A',$row->sano);
