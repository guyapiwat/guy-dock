...
for($ro=$ftrc[0];$ro<=$ftrc[1];$ro++){
	...
	if (mysql_num_rows($result)>'0') {
		...
		$sqlupdate = "update ".$dbprefix."bmbonus set carry_l = carry_ll , carry_c = carry_cc where fdate = '$fdate' and tdate = '$tdate' ";
	//	echo $sqlupdate;
		mysql_query($sql);
	//	exit;
...
//$cpaydate[0] = '2013';
	$sql = "SELECT min(rcode) as minrcode ";
	$sql .= " FROM ".$dbprefix."around a WHERE a.paydate like '%".$cpaydate[0]."%' and a.paydate <= '$paydate' and calc = '1' ";
	$rs2=mysql_query($sql);
	if (mysql_num_rows($rs2)>0) {
		$sqlObj2 = mysql_fetch_object($rs2);
		$around_minrcode= $sqlObj2->minrcode;
		mysql_free_result($rs2);
	}

	$sql = "SELECT max(rcode) as maxrcode ";
	$sql .= " FROM ".$dbprefix."around a WHERE a.paydate like '%".$cpaydate[0]."%' and a.paydate <= '$paydate' and calc = '1' ";
	$rs2=mysql_query($sql);
	if (mysql_num_rows($rs2)>0) {
		$sqlObj2 = mysql_fetch_object($rs2);
		$around_maxrcode= $sqlObj2->maxrcode;
		mysql_free_result($rs2);
	}

	$sql = "SELECT min(rcode) as minrcode ";
	$sql .= " FROM ".$dbprefix."bround a WHERE a.paydate like '%".$cpaydate[0]."%' and a.paydate <= '$paydate' and calc = '1' ";
	$rs2=mysql_query($sql);
	if (mysql_num_rows($rs2)>0) {
		$sqlObj2 = mysql_fetch_object($rs2);
		$bround_minrcode= $sqlObj2->minrcode;
		mysql_free_result($rs2);
	}
	...
	if (mysql_num_rows($rs2)>0) {
		$sqlObj2 = mysql_fetch_object($rs2);
		$bround_maxrcode= $sqlObj2->maxrcode;
		mysql_free_result($rs2);
	}
	
	//echo $around_minrcode.' : '.$around_maxrcode.' : '.$bround_minrcode.' : '.$bround_maxrcode;
	//exit;
...
//exit;
	...
	$sql = "SELECT m.mcode,m.locationbase,m.name_t,m.status_suspend,m.status_terminate,taba.exp_date,m.mtype,m.pos_cur,m.sp_code,m.cmp,m.status,m.cmp2,m.cmp3,m.acc_no,m.name_f as title,m.id_tax,m.id_card 
...
//	exit;
	...
	if(mysql_num_rows($rs1) > 0){
		for($j=0;$j<mysql_num_rows($rs1);$j++){
			...
			$sqlfast = "SELECT ifnull(sum(total),0) as totalfast from ".$dbprefix."ambonus  where   ".$whereclass1."  and total > 0 and pstatus =0 and mcode = '".$mcode[$j]."' group by mcode";
			$rsfast = mysql_query($sqlfast);
			if(mysql_num_rows($rsfast) > 0){
				$sqlObjfast = mysql_fetch_object($rsfast);
				$totalfast[$j] =$sqlObjfast->totalfast;	
			}



			$sqlfast = "SELECT ifnull(sum(total),0) as totalbinary from ".$dbprefix."bmbonus  where   ".$whereclass1."  and total > 0 and pstatus =0 and mcode = '".$mcode[$j]."' group by mcode";
			$rsfast = mysql_query($sqlfast);
			if(mysql_num_rows($rsfast) > 0){
				$sqlObjfast = mysql_fetch_object($rsfast);
			...
			$sqlfast = "SELECT ifnull(sum(total),0) as totalmatching from ".$dbprefix."dmbonus  where   ".$whereclass1."  and total > 0 and pstatus =0 and mcode = '".$mcode[$j]."' group by mcode";
			$rsfast = mysql_query($sqlfast);
			if(mysql_num_rows($rsfast) > 0){
				$sqlObjfast = mysql_fetch_object($rsfast);
				$totalmatching[$j] =$sqlObjfast->totalmatching;	
			}

			$sqlfast = "SELECT ifnull(sum(total),0) as totalonetime from ".$dbprefix."ombonus  where   ".$whereclass1."  and total > 0 and pstatus =0 and mcode = '".$mcode[$j]."' group by mcode";
			$rsfast = mysql_query($sqlfast);
			if(mysql_num_rows($rsfast) > 0){
				$sqlObjfast = mysql_fetch_object($rsfast);
				$totalonetime[$j] =$sqlObjfast->totalonetime;	
			}

			$sqlfast = "SELECT ifnull(sum(total),0) as totalstar from ".$dbprefix."smbonus  where   ".$whereclass1."  and total > 0 and pstatus =0 and mcode = '".$mcode[$j]."' group by mcode";
			$rsfast = mysql_query($sqlfast);
			if(mysql_num_rows($rsfast) > 0){
				$sqlObjfast = mysql_fetch_object($rsfast);
				$totalstar[$j] =$sqlObjfast->totalstar;	
			}
		...
		//	}
			...
			if($mcode[$j] == '0007243'){
			//	echo $mcode[$j].' : '.$voucher[$j].' : '.$totalamt1[$j].' : '.$moneyb[$j].' : '.$totalamt[$j].' : <br>';
			//	exit;
			}
			...
			//}
				...
				if($mtype[$j] == '1'){
					...
					$chkdate1 = $chkdate[0];
					$sql = "select sum(total) as total1 from ".$dbprefix."ambonus a where  a.total > 0 and a.rcode between '$around_minrcode' and '$around_maxrcode' and a.tdate < '$fpdate' and mcode = '".$mcode[$j]."' group by mcode";

					//if($mcode[$j] == '0002979')echo $sql.'<br>';
					$rs1234 = mysql_query($sql);
					if(mysql_num_rows($rs1234) > 0){
						$sqlObj123 = mysql_fetch_object($rs1234);
						$total1 = $total1+$sqlObj123->total1;
					}
					$sql = "select sum(total) as total1 from ".$dbprefix."bmbonus a where  a.total > 0  and a.rcode between '$around_minrcode' and '$around_maxrcode'  and a.tdate < '$fpdate' and mcode = '".$mcode[$j]."' group by mcode";
					//if($mcode[$j] == '0002979')echo $sql.'<br>';
					$rs1234 = mysql_query($sql);
					if(mysql_num_rows($rs1234) > 0){
						$sqlObj123 = mysql_fetch_object($rs1234);
					...
					//if($cstatus  == '1'){
						...
						$sql = "select sum(total) as total1 from ".$dbprefix."dmbonus a where  a.total > 0 and a.rcode between '$bround_minrcode' and '$bround_maxrcode'   and a.tdate < '$fpdate' and mcode = '".$mcode[$j]."' group by mcode";
						...
						$sql = "select sum(total) as total1 from ".$dbprefix."smbonus a where  a.total > 0 and a.rcode between '$bround_minrcode' and '$bround_maxrcode'   and a.tdate < '$fpdate' and mcode = '".$mcode[$j]."' group by mcode";
						//if($mcode[$j] == '0002979')echo $sql.'<br>';
						$rs1234 = mysql_query($sql);
						if(mysql_num_rows($rs1234) > 0){
							$sqlObj123 = mysql_fetch_object($rs1234);
						...
						$sql = "select sum(total) as total1 from ".$dbprefix."ombonus a where  a.total > 0 and a.rcode between '$bround_minrcode' and '$bround_maxrcode'   and a.tdate < '$fpdate' and mcode = '".$mcode[$j]."' group by mcode";
						//if($mcode[$j] == '0002979')echo $sql.'<br>';
						$rs1234 = mysql_query($sql);
						if(mysql_num_rows($rs1234) > 0){
							$sqlObj123 = mysql_fetch_object($rs1234);
					...
					if($mcode[$j] == '0002979'){
					//	echo $mcode[$j].' : '.$totaly.' : '.$total1.' : '.$totalamt[$j].' : '.$totalamt1[$j].' : '.$fdate.' : '.$tdate.'<br>';
					//	exit;
					}
					if($totaly >= 200000)$tax[$j] = $totalamt1[$j]*0.05;
					else if($totaly+$totalamt1[$j] >= 200000)$tax[$j] = ($totaly+$totalamt1[$j]-200000)*0.05;
					else $tax[$j] = 0;
				}
			...
			//	if($totalamt1[$j] > 0 and $mstatus[$j] == '0'){
				$sql2 = "SELECT sum(tot_pv) as totalpv from ".$dbprefix."asaleh WHERE mcode='".$mcode[$j]."' and cancel = 0 and  sadate >= '$fpdate' and sadate <= '$tpdate' and (sa_type='Q' or sa_type='A' or sa_type='C') and (sa_type='Q' or sa_type='A' or sa_type='C')  group by mcode ";
				//echo $sql2.'<br>';
				//exit;
				$rs2=mysql_query($sql2);
				$totalpv = 0;
				...
				$sql2 = "SELECT sum(tot_pv) as totalpv from ".$dbprefix."holdhead WHERE mcode='".$mcode[$j]."' and cancel = 0 and  sadate >= '$fpdate' and sadate <= '$tpdate' and (sa_type='Q' or sa_type='A' or sa_type='C') group by mcode ";
				...
				if($totalpv>= 250 or $mcode[$j] == '0000001' or $mcode[$j] == '0000002' or $mcode[$j] == '0000003' ){
					...
					if($cstatus == '1'){
						...
						$sql2 = "SELECT sum(tot_pv) as totalpv from ".$dbprefix."asaleh WHERE mcode='".$mcode[$j]."' and  sadate like '%".$last1."%' and (sa_type='Q' or sa_type='A' or sa_type='C') and cancel = 0 group by mcode ";
						$rs2=mysql_query($sql2);
						$totalpv1 = 0;
						if(mysql_num_rows($rs2)>0){
						...
						$sql2 = "SELECT sum(tot_pv) as totalpv from ".$dbprefix."holdhead WHERE mcode='".$mcode[$j]."' and  sadate like '%".$last1."%' and (sa_type='Q' or sa_type='A' or sa_type='C') and cancel = 0 group by mcode ";
						$rs2=mysql_query($sql2);
						if(mysql_num_rows($rs2)>0){
							$totalpv1 = $totalpv1 + mysql_result($rs2,0,"totalpv");
						}

						$sql2 = "SELECT sum(tot_pv) as totalpv from ".$dbprefix."asaleh WHERE mcode='".$mcode[$j]."' and  sadate like '%".$last2."%' and (sa_type='Q' or sa_type='A' or sa_type='C') and cancel = 0 group by mcode ";
						$rs2=mysql_query($sql2);
						$totalpv2 = 0;
						if(mysql_num_rows($rs2)>0){
							$totalpv2 = mysql_result($rs2,0,"totalpv");
						}
						$sql2 = "SELECT sum(tot_pv) as totalpv from ".$dbprefix."holdhead WHERE mcode='".$mcode[$j]."' and  sadate like '%".$last2."%' and (sa_type='Q' or sa_type='A' or sa_type='C') and cancel = 0 group by mcode ";
						$rs2=mysql_query($sql2);
						if(mysql_num_rows($rs2)>0){
					...
					if($totalamt1[$j] > 0){
						...
						if($cmp[$j] == 'ครบ' and $cmp2[$j] == 'ครบ'  and  $cmp3[$j] == 'ครบ'  and !empty($acc_no[$j]) and $mem_cntday[$j] > -90 and $status_suspend[$j] <> '1'  ){
							if($totalpv>= 250 ){
								...
								$sql = "INSERT INTO ".$dbprefix."cmbonus (rcode,mcode,status,pv,pvb,pvh,fob,cycle,smb,matching,onetime,total,totaly,mdate,month_pv,mpos,c_note1,c_note2,c_note3,c_note4,c_note5,tot_vat,tot_tax,title,paydate,status_pv,locationbase,crate,mtype,com_transfer_chagre,name_f,name_t,id_card,id_tax) ";
									$sql .= "VALUES('$ro','".$mcode[$j]."','0','".$moneyb[$j]."','0','".$totalamt1[$j]."','".$totalfast[$j]."','".$totalbinary[$j]."','".$totalstar[$j]."','".$totalmatching[$j]."','".$totalonetime[$j]."','0','".$btotal."','".$strfdate."','".$month[0].$month[1]."','".$pos_cur[$j]."','".$c_note1."','".$c_note2."','".$c_note3."','".$c_note4."','".$c_note5."','".$vat[$j]."','".$tax[$j]."','".$title[$j]."','$paydate','$totalpv','".$locationbase[$j]."','".$crate[$j]."','".$mtype[$j]."','$com_transfer_chagre','".$name_f[$j]."','".$name_t[$j]."','".$id_card[$j]."','".$id_tax[$j]."')";
								}
								...
								//if($mcode[$j] == '0000819')echo $sql.'<br>';
								...
								//if($totalamt1[$j] >= 300 and $cstatus == 0 and $moneyb[$j] > 0){
								...
								/*if($mcode[$j] == '0000819'){
										echo $totalpv.' :: '.$totalamt[$j].' : '.$moneyb[$j];
										exit;
...
//$cpaydate[0] = '2013';
	$sql = "SELECT min(rcode) as minrcode ";
	$sql .= " FROM ".$dbprefix."around a WHERE a.paydate like '%".$cpaydate[0]."%' ";
	$rs2=mysql_query($sql);
	if (mysql_num_rows($rs2)>0) {
		$sqlObj2 = mysql_fetch_object($rs2);
		$around_minrcode= $sqlObj2->minrcode;
	...
	$sql = "SELECT min(rcode) as minrcode ";
	$sql .= " FROM ".$dbprefix."bround a WHERE a.paydate like '%".$cpaydate[0]."%' ";
	$rs2=mysql_query($sql);
	if (mysql_num_rows($rs2)>0) {
		$sqlObj2 = mysql_fetch_object($rs2);
		$bround_minrcode= $sqlObj2->minrcode;
	...
	$sql = "SELECT m.mcode,m.name_t,m.pos_cur2,m.status_suspend,m.status_terminate,m.mtype,m.locationbase,taba.exp_date,m.pos_cur,m.sp_code,m.cmp,m.status,m.cmp2,m.cmp3,m.acc_no,m.name_f as title,m.id_tax,m.id_card
	from ".$dbprefix."member m ";
...
//$id_tax = $id_card = $name_t = $name_f = "";
	...
	$pos_piority = array('RD'=>17,'PD'=>16,'ID'=>15,'CD'=>14,'BL'=>13,'BD'=>12,'DI'=>11,'EM'=>10,'SA'=>9,'RU'=>8,'PE'=>7,'PL'=>6,'GO'=>5,'SI'=>4,'BR'=>3,'EX'=>2,'SU'=>1,'MB'=>0);
		...
		for($j=0;$j<mysql_num_rows($rs1);$j++){
			...
			$locationbase[$j] = $sqlObj1->locationbase;
			$status_suspend[$j] = $sqlObj1->status_suspend;
			$name_t1[$j] = $sqlObj1->name_t;
			$name_f1[$j] = $sqlObj1->title;
			$id_card[$j] = $sqlObj1->id_card;
			$id_tax[$j] = $sqlObj1->id_tax;
			$totalamt[$j] = $totalfast[$j]+$totalbinary[$j]+$totalstar[$j]+$totalmatching[$j]+$totalonetime[$j];
			$totalamt1[$j] = $totalfast[$j]+$totalbinary[$j]+$totalstar[$j]+$totalmatching[$j]+$totalonetime[$j];
				...
				//echo $adjust_binary.' '.$adjust_matching.' '.$adjust_pool.' '.$totalfast[$j].' '.$totalbinary[$j].' '.$totalmatching[$j].' '.$totalpool[$j].' '.$mcode[$j].' '.$cmp[$j].' '.$cmp2[$j].' '.$accno[$j].' '.$totalamt1[$j].' '.$moneyb[$j].' '.$totalamt[$j].'<br>';
			//	if($totalamt1[$j] > 0 and $mstatus[$j] == '0'){
				$sql2 = "SELECT sum(tot_pv) as totalpv from ".$dbprefix."asaleh WHERE mcode='".$mcode[$j]."' and  sadate >= '$fpdate' and sadate <= '$tpdate' and (sa_type='Q' or sa_type='A' or sa_type='C') and (sa_type='Q' or sa_type='A' or sa_type='C') and cancel = 0  group by mcode ";
				//echo $sql2.'<br>';
				//exit;
				$rs2=mysql_query($sql2);
				$totalpv = 0;
				...
				$sql2 = "SELECT sum(tot_pv) as totalpv from ".$dbprefix."holdhead WHERE mcode='".$mcode[$j]."' and  sadate >= '$fpdate' and sadate <= '$tpdate' and (sa_type='Q' or sa_type='A' or sa_type='C') and cancel = 0 group by mcode ";
				...
				if($pos_piority[$pos_cur2[$j]] >=3){
					if($totalamt[$j] >= 1000){
						$totalamt[$j] = $totalamt[$j]-1000;
						$bprice[$j] = 1000;
						$voucher[$j] = 1000/0.030303;
					}else {
						$bprice[$j] =$totalamt[$j];
						$voucher[$j] = $totalamt[$j]/0.030303;
						$totalamt[$j] = 0;							
					}
				}else{
						$bprice[$j] = $totalamt[$j]*0.2;
						$voucher[$j] = $totalamt[$j]*0.2/0.030303;
						$totalamt[$j] = $totalamt[$j]-$bprice[$j];
				...
				if($totalpv>=0 or $cstatus <> '1' or $mcode[$j] == '0000001' or $mcode[$j] == '0000002' or $mcode[$j] == '0000003'){
					if($totalamt1[$j] > 0){
						if($totalpv>= 0 and $cmp[$j] == 'ครบ' and $cmp2[$j] == 'ครบ'  and  $cmp3[$j] == 'ครบ'  and !empty($acc_no[$j]) and $mem_cntday[$j] > -90 and $status_suspend[$j] <> '1' and $cstatus == '1' ){
							...
							$sql = "INSERT INTO ".$dbprefix."cmbonus (rcode,mcode,status,pv,pvb,pvh,fob,cycle,smb,matching,onetime,total,totaly,mdate,month_pv,mpos,c_note1,c_note2,c_note3,c_note4,c_note5,tot_vat,tot_tax,title,paydate,status_pv,locationbase,crate,mtype,com_transfer_chagre,voucher,name_f,name_t,id_card,id_tax) ";
								$sql .= "VALUES('$ro','".$mcode[$j]."','0','".$moneyb[$j]."','0','".$totalamt1[$j]."','".$totalfast[$j]."','".$totalbinary[$j]."','".$totalstar[$j]."','".$totalmatching[$j]."','".$totalonetime[$j]."','0','".$btotal."','".$strfdate."','".$month[0].$month[1]."','".$pos_cur[$j]."','".$c_note1."','".$c_note2."','".$c_note3."','".$c_note4."','".$c_note5."','".$vat[$j]."','".$tax[$j]."','".$title[$j]."','$paydate','$totalpv','".$locationbase[$j]."','".$crate[$j]."','".$mtype[$j]."','$com_transfer_chagre','".$voucher[$j]."','".$name_f1[$j]."','".$name_t1[$j]."','".$id_card[$j]."','".$id_tax[$j]."')";
							}
							//if($mcode[$j] == '0000420')echo $sql.'<br>';
							//echo $sql.'<br>';
							mysql_query($sql);
							...
							$sql = "INSERT INTO ".$dbprefix."cmbonus (rcode,mcode,status,pv,pvb,pvh,fob,cycle,smb,matching,onetime,total,totaly,mdate,month_pv,mpos,c_note1,c_note2,c_note3,c_note4,c_note5,tot_vat,tot_tax,title,paydate,status_pv,locationbase,crate,mtype,com_transfer_chagre,voucher,name_f,name_t,id_card,id_tax) ";
							$sql .= "VALUES('$ro','".$mcode[$j]."','0','".$moneyb[$j]."','0','".$totalamt1[$j]."','".$totalfast[$j]."','".$totalbinary[$j]."','".$totalstar[$j]."','".$totalmatching[$j]."','".$totalonetime[$j]."','0','".$btotal."','".$strfdate."','".$month[0].$month[1]."','".$pos_cur[$j]."','".$c_note1."','".$c_note2."','".$c_note3."','".$c_note4."','".$c_note5."','".$vat[$j]."','".$tax[$j]."','".$title[$j]."','$paydate','$totalpv','".$locationbase[$j]."','".$crate[$j]."','".$mtype[$j]."','$com_transfer_chagre','".$bprice[$j]."','".$name_f1[$j]."','".$name_t1[$j]."','".$id_card[$j]."','".$id_tax[$j]."')";
							mysql_query($sql);
