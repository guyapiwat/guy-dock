...
//$wherecause = " WHERE ";
	...
	if($_GET['state']==1){
		...
		$rec->setFieldDesc("รหัสรอบ,วันที่เพิ่มรอบ,วันที่เริ่มต้น,วันที่สิ้นสุด,ชื่อโปรโมชั่น");
		$rec->setFieldAlign("center,center,center,center,left");
		$rec->setFieldSpace("10%,15%,15%,15%,45%");
		$rec->setFieldLink("index.php?sessiontab=4&sub=60&cmc=,");
		if($acc->isAccess(4)){
			$rec->setDel("index.php","rid","rid","sessiontab=4&sub=60");
			$rec->setFromDelAttr("maindel","./index.php?sessiontab=4&sub=60&state=1","post","delfield");
		}

		if($acc->isAccess(2))$rec->setEdit("index.php","rid","rid","sessiontab=4&sub=44");
