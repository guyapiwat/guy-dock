...
require_once ("function.log.inc.php");
	...
	for ($i=0;$i<$numpost;$i++) {
		...
		$rs=mysql_query("SELECT * FROM ".$dbprefix."pmbonus  WHERE id='".$postval[$postkey[$i]]."' LIMIT 1");
		//echo "SELECT * FROM ".$dbprefix."pmbonus  WHERE id='".$postval[$postkey[$i]]."' LIMIT 1";
		if (mysql_num_rows($rs)>0){
            ...
            <?
			mysql_query("update ".$dbprefix."pmbonus   set status = 0,pv = pv+pvb  where id='$id' ");
			//====================LOG===========================
$text="uid=".$_SESSION["adminusercode"]." action=cmbonus_update=>update ".$dbprefix."pmbonus   set status = 0,pv = pv+pvb where id='$id' ";
writelogfile($text);
//=================END LOG===========================
		}
		logtext(true,$_SESSION['adminusercode'],'ย้ายการจ่ายตัง แผน B',$row->sano);
