...
<?
		...
		$sql = "SELECT DATE_FORMAT(a.fdate, '%d-%m-%Y') as fdate,a.rcode, a.mcode,m.inv_desc,sum(a.total) as total,sum(a.tax) as tax,sum(a.bonus) as bonus ";
		...
		$rec->setShowField("fdate,mcode,inv_desc,total");
		$rec->setFieldDesc("วันที่,รหัส Stockist,ชื่อ,โบนัส");
		$rec->setFieldAlign("center,center,left,right,right,right,right,right");
		$rec->setFieldSpace("10%,10%,70%,10%,10%,10%");//10
...
<form name="rform" method="post" action="./index.php?sessiontab=4&sub=42">
