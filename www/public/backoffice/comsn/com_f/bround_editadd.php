...
//loop check
	...
	startRQ(field,val,"",flag,errDesc,"fround","checkstate");
}
...
//check(nullCheck, sizeCheck, formatCheck, dupCheck, realCheck, idName, errmsg)
	...
	document.getElementById('checkstate').innerHTML= "<img align='center' src='./images/loading.gif' />";
	//alert(skipval);
	startRQ(field,val,skipval,flag,errDesc,"fround","checkstate");
}
</script>
<?
if($_GET['rid']){
		$sql = "SELECT * FROM ".$dbprefix."fround WHERE rid='".$_GET['rid']."' LIMIT 1";
		$rs = mysql_query($sql);
		...
		$sql = "SELECT MAX(rcode) AS maxs FROM ".$dbprefix."fround ";
		$rs = mysql_query($sql);
...
<table width="95%" border="0">
   <tr>
     <td width="63%">
<form name="form1" method="post" action="./comsn/com_f/broundoperate.php?state=<?=$_GET['rid']==""?0:1?>">
<table width="98%" border="0">
  ...
  <tr>
    ...
    <td><input name="button" type="button" onclick="<?=(isset($_GET['rid'])?"earoundcheck()":"iaroundcheck()")?>" value="ตรวจสอบ" />
             ...
             &nbsp;
            <input name="reset" type="reset"  onclick="window.location='index.php?sessiontab=4&sub=38	'" value="ยกเลิก" /></td>
  </tr>
</table>
</form><td width="37%">
