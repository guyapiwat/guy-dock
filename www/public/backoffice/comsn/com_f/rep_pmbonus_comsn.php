...
<?
		...
		$sql = "SELECT DATE_FORMAT(a.fdate, '%d-%m-%Y') as fdate,a.rcode, a.mcode,m.name_t,m.acc_no,m.bankcode,b.bankname,a.total,a.tax,a.bonus ";
		$sql .= " FROM ".$dbprefix."embonus a ";
		$sql .= " LEFT JOIN ".$dbprefix."member m ON a.mcode=m.mcode ";
		$sql .= " LEFT JOIN ".$dbprefix."bank b ON m.bankcode=b.bankcode ";		
		if($strfdate!=""&&$strtdate!=""&&$fmcode!=""){
			$sql .= " WHERE e.fdate>= '$strfdate' and e.tdate<= '$strtdate' and a.fdate>= '$strfdate' and a.tdate<= '$strtdate'  and e.mcode='".$fmcode."'";
		}else if($strfdate!=""&&$strtdate!=""){
			$sql .= " WHERE e.fdate>= '$strfdate' and e.tdate<= '$strtdate' and a.fdate>= '$strfdate' and a.tdate<= '$strtdate' ";
		}else if($fmcode!=""){
		...
		$rec->setFieldDesc("วันที่,รหัสมาชิก,ชื่อ,เลขบัญชี,ธนาคาร,โบนัส,ภาษี (5%),สุทธิ");
