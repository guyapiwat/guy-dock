...
<?
if($_GET['state']==1){
		include("comsn/com_i/rep_renew.php");
		}
...
<?
		...
		$sql = "SELECT a.id,a.rcode,a.mcode,m.inv_desc as name_t,a.pv,a.pvb,a.total  ";
		$sql .= "FROM ".$dbprefix."imbonus a ";
		$sql .= " LEFT JOIN ".$dbprefix."invent m ON a.mcode=m.inv_code ";
		if($ftrcode!=""&&$fmcode!=""){
			$sql .= " WHERE rcode between '".$ftrc[0]."' and '".$ftrc[1]."' and a.mcode=".$fmcode." and a.status = 0   ";
		}else if($ftrcode!=""){
			$sql .= " WHERE rcode between '".$ftrc[0]."' and '".$ftrc[1]."'and a.status = 0 ";
		}else if($fmcode!=""){
		...
		$rec->setLink($PHP_SELF,"sessiontab=4&sub=56&ftrcode=$ftrcode&fmcode=$fmcode");
		$rec->setBackLink($PHP_SELF,"sessiontab=4");
		if(isset($page))
			$rec->setCurPage($page);
		//$rec->setShowIndex(true);
		$rec->setShowField("rcode,mcode,name_t,pv");
		$rec->setFieldDesc("รอบ,รหัส,ชื่อ,ยอดยกไป");
		$rec->setFieldAlign("center,center,left,right,right,right,right,right,right,right,right,right");
		$rec->setFieldSpace("5%,5%,84%,10%,10%,10%,10%,10%,6%,10%,10%");//10
		$rec->setSum(true,false,",,,true,true,true,true,true,true");
		$rec->setFieldFloatFormat(",,,2,2,2,2,2,2");
		if($acc->isAccess(4)){
			$rec->setDel("index.php","id","id","sessiontab=4&sub=56");
			$rec->setFromDelAttr("maindel","./index.php?sessiontab=4&sub=56&state=1","post","delfield");
		}
		if(isset($_POST['skey']))
