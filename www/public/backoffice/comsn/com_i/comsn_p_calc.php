...
//================================================================================
			$sql = "select * from ".$dbprefix."iround where rcode between '".$rof."' and '".$rot."' and calc='1'";
...
for($ro=$ftrc[0];$ro<=$ftrc[1];$ro++){
	...
	$bonuslv2 = 0.25;
	$bonuslv3 = 0.25;
	$sql="select * from ".$dbprefix."iround where  rcode='".$ro."'  ";
	$result=mysql_query($sql);
	if (mysql_num_rows($result)>'0') {
		...
		$sql="delete from ".$dbprefix."imbonus where rcode= '".$ro."'";
		if(mysql_query($sql)){
			mysql_query("COMMIT");
		}else{
			echo "error $sql<BR>";
...
///////////////// Sum bill in round ////////////////////////  รวม PV ของทุกคนใน APV
	...
	$sql="update ".$dbprefix."iround set calc='1' where rcode='$ro' ";
...
function fnc_calc_status($dbprefix,$ro,$strfdate,$strtdate,$fpdate,$tpdate,$fmcode){
		...
		$sql = "SELECT m.inv_code as mcode,m.inv_desc as name_t,		
		(((SELECT ifnull(sum(total),0) from ".$dbprefix."fmbonus b where b.mcode=m.inv_code  ".$whereclass." ))*0.95) as totalamt 
		from ".$dbprefix."invent m 
		where (SELECT ifnull(sum(total),0) from ".$dbprefix."fmbonus b where b.mcode=m.inv_code  ".$whereclass." )>0";
	//	echo $sql;
	//	exit;
		$rs = mysql_query($sql);
		for($m=0;$m<mysql_num_rows($rs);$m++){
			...
			//if($cmp == 'ครบ' and $cmp2 == 'ครบ' and !empty($acc_no)){
				$sql = "INSERT INTO ".$dbprefix."imbonus (rcode,mcode,status,pv,pvb,total,mdate,month_pv,mpos) ";
 ...
 function backmonthpv($dbprefix,$mcode,$ro){
	$sql2 = "select mcode,pv,status from ".$dbprefix."imbonus WHERE  mcode='$mcode' order by rcode desc limit 0,1 ";
