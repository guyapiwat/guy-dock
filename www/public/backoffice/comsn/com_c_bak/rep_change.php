...
require_once ("function.log.inc.php");
	...
	for ($i=0;$i<$numpost;$i++) {
		...
		if (mysql_num_rows($rs)>0){
            ...
            <?
			mysql_query("update ".$dbprefix."cmbonus   set status = 0,pv = pv+pvb,pvb = 0  where id='$id' and status = 1 ");
			//====================LOG===========================
$text="uid=".$_SESSION["adminuserid"]." action=cmbonus_update=>update ".$dbprefix."cmbonus   set status = 0 ,pv = pv+pvb where id='$id'  and status = 1  ";
writelogfile($text);
//=================END LOG===========================
		}
