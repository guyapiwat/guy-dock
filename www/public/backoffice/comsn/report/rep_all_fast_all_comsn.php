...
mysql_query("delete from ".$dbprefix."report_point where mcode = '$cmc'");
	...
	//	if (isset($_GET["pg"])){$page=$_GET["pg"];} else {$page="1";}
		...
		$sql="select *,DATE_FORMAT(fdate, '%m-%Y') as monthpv from ".$dbprefix."bround a where 1=1 order by rcode desc   ";
...
$count_new = 0;$count_sup=0;$count_su=0;$count_ex=0;$count_sup_ex=0;
				...
				$total_l = $carry_l+$ro_l;
				$total_c = $carry_c+$ro_c;
				if($mdate <= $fdate ){
					mysql_query("insert into  ".$dbprefix."report_point values('$cmc','$name_t','$point','$monthpv','$carry_l','$carry_c','$ro_l','$ro_c','$total_l','$total_c','$mexpj','$pos_cur2','$count_new','$count_su','$count_ex','$count_sup_ex')");
					mysql_query("delete from  ".$dbprefix."report_point  where monthpv = '".date("m-Y")."' ");
				}
			//}
		...
		$rec->setLink($PHP_SELF,"sessiontab=1&sub=24&ftrcode=$ftrcode&strfdate=$strfdate&strtdate=$strtdate&fmcode=$cmc");
		...
		$rec->setShowField("mcode,name_t,monthpv,carry_l,carry_c,ro_l,ro_c,all_l,all_c,point,allpv,pos_cur,new_sponsor,new_sup,new_ex,sup_ex");
		$rec->setFieldDesc("รหัสสมาชิก,ชื่อ-สกุล,เดือน,PV ซ้ายยกมา,PV ขวายกมา,PV ซ้ายใหม่,PV ขวาใหม่,PV ซ้ายรวม,PV ขวารวม,Cycle,PV ส่วนตัว,ตำแหน่ง<br>Matching,New Sponser<br>ในเดือน,Mem ไป Sup,Mem ไป Ex,Sup ไป Ex");
		$rec->setFieldAlign("center,left,center,right,right,right,right,right,right,center,right,center,center,center,center,center");
		$rec->setFieldSpace("5%,12%,4%,5%,5%,5%,5%,5%,5%,3%,5%,5%,5%,5%,5%,5%,5%");//10
