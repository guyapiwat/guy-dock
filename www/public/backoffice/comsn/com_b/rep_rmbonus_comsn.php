...
if($fdate != ''){
		...
		$sql = "SELECT DATE_FORMAT(a.fdate, '%Y-%m-%d') as fdate,DATE_FORMAT(a.tdate, '%Y-%m-%d') as tdate,a.rcode, a.mcode,m.upa_code as smcode,m.name_t,m.acc_no,m.bankcode,b.bankname,a.total,a.tax,a.total-a.tax as bonus,a.pos_cur ";
       ...
       if($fmcode != '' )$sql .= " and a.mcode = ".$fmcode."";
		...
		$rec->setShowField("fdate,tdate,mcode,name_t,pos_cur,total");
		$rec->setFieldDesc("วันที่เริ่มต้น,วันที่สิ้นสุด,รหัสมาชิก,ชื่อ,ตำแหน่ง,โบนัสยูนิเลเวล");
		$rec->setFieldAlign("center,center,center,left,center,right");
		$rec->setFieldSpace("10%,10%,10%,55%,5%,10%");//10
		...
		$rec->setFieldLink(",,,,,index.php?sessiontab=4&sub=199&strfdate=,");
		$rec->setSpace($str);
