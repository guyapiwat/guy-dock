...
//loop check
	...
	startRQ(field,val,"",flag,errDesc,"bround","checkstate");
}
...
//check(nullCheck, sizeCheck, formatCheck, dupCheck, realCheck, idName, errmsg)
	...
	document.getElementById('checkstate').innerHTML= "<img align='center' src='./images/loading.gif' />";
	//alert(skipval);
	startRQ(field,val,skipval,flag,errDesc,"bround","checkstate");
}
</script>
<?
if($_GET['rid']){
		$sql = "SELECT * FROM ".$dbprefix."bround WHERE rid='".$_GET['rid']."' LIMIT 1";
		$rs = mysql_query($sql);
		...
		$sql = "SELECT MAX(rcode) AS maxs FROM ".$dbprefix."bround ";
		$rs = mysql_query($sql);
...
<table width="95%" border="0">
   <tr>
     <td width="63%">
<form name="form1" method="post" action="./comsn/com_b/broundoperate.php?state=<?=$_GET['rid']==""?0:1?>">
<table width="98%" border="0">
  
  <tr>
    <td width="34%" align="right">รอบที่&nbsp;</td>
    <td width="66%">
      <input type="text" name="rcode" id="rcode" size="10" value="<?=$rcode?>"> 
	  <input type="hidden" id="orcode" name="orcode" size="10"  value="<?=$rcode?>"><input type="hidden" name="oid" value="<?=$oid?>" >
	  </td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>
		<input type="text" id="dateInput2" name="fdate" value="<?=$fdate?>">
	</td>
  </tr>
  <tr>
    <td align="right">วันที่สิ้นสุด&nbsp;</td>
    <td><input type="text" id="dateInput3" name="tdate" value="<?=$tdate?>"></td>

  </tr>
  <tr>
    <td align="right">วันที่จ่าย&nbsp;</td>
    <td><input type="text" id="dateInput4" name="paydate" value="<?=$paydate?>"></td>
  </tr>
  <tr>
  ...
  <tr>
    <td>&nbsp;</td>
    <td><input name="button" type="button" id="button" onclick="<?=(isset($_GET['rid'])?"earoundcheck()":"iaroundcheck()")?>" value="ตรวจสอบ" />
             &nbsp;
             <input type="submit" value="บันทึก" name="ok" id="ok" disabled="disabled" />
             &nbsp;
            <input name="reset" type="reset" id="reset"  onclick="window.location='index.php?sessiontab=4&sub=11'" value="ยกเลิก" /></td>
  </tr>
</table>
</form><td width="37%">
