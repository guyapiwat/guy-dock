...
<?
		...
		$sql = "SELECT a.rcode,a.mcode,taba.name_t,a.upa_code,tabb.upa_name,a.pv,a.level,a.percer,a.total FROM ".$dbprefix."ac2 a  ";
		...
		$rec->setLink($PHP_SELF,"sessiontab=4&sub=402&ftrcode=$ftrcode&fmcode=$fmcode");
		$rec->setBackLink($PHP_SELF,"sessiontab=4");
		if(isset($page))
			$rec->setCurPage($page);
		$rec->setShowIndex(true);
		//$rec->setSpecial("./images/search.gif","","view","rcode,mcode","IMAGE","ดู");
		$rec->setShowField("rcode,mcode,name_t,upa_code,upa_name,total,level,total");
		$rec->setFieldDesc("รหัสรอบ,รหัสสมาชิก,ชื่อสมาชิก,รหัสผู้คีย์,ชื่อผู้คีย์,".$wording_lan["com"]["1"].",ชั้นที่,ได้โบนัส");
		$rec->setFieldAlign("center,center,left,center,left,right,center,right,right");
		$rec->setFieldLink(",,,,,index.php?sessiontab=4&sub=411&lr=1&strtdate=$tdate&strfdate=$fdate&tot=,,");
