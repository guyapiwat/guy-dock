            ...
            <td><br/>
                <form name="rform" method="post" action="./index.php?sessiontab=4&sub=12">
                    <table width="60%" border="1" cellpadding="0" cellspacing="0" bordercolor="#FF7F00" align="center">
                        ...
                        <tr>
                            <td colspan="2" align="center">กรอกรอบการคำนวณคอมมิชชั่นรายเดือน (Unilevel)
                                ที่ต้องการคำนวนเช่น 1-12
                            </td>
                        </tr>
                        <tr>
                            <td>&nbsp;</td>
...
function fnc_calc_set_position($dbprefix, $ro, $mcode, $name_t, $pos_cur, $sp_code, $lr, $fdate, $tdate)
{
    ...
    if ($chkdate[1] == '01' or $chkdate[1] == '02' or $chkdate[1] == '03' or $chkdate[1] == '04' or $chkdate[1] == '05' or $chkdate[1] == '06') {
        $xfdate_1 = $chkyear . '-01-01';
        $xtdate_1 = $chkyear . '-01-31';
        $xfdate_2 = $chkyear . '-02-01';
        $xtdate_2 = $chkyear . '-02-31';
        $xfdate_3 = $chkyear . '-03-01';
        $xtdate_3 = $chkyear . '-03-31';
        $xfdate_4 = $chkyear . '-04-01';
        $xtdate_4 = $chkyear . '-04-31';
        $xfdate_5 = $chkyear . '-05-01';
        $xtdate_5 = $chkyear . '-05-31';
        $xfdate_6 = $chkyear . '-06-01';
        $xtdate_6 = $chkyear . '-06-31';
    } else {
        $chkmonth = $chkmonth - 6;
        $xfdate_1 = $chkyear . '-07-01';
        $xtdate_1 = $chkyear . '-07-31';
        $xfdate_2 = $chkyear . '-08-01';
        $xtdate_2 = $chkyear . '-08-31';
        $xfdate_3 = $chkyear . '-09-01';
        $xtdate_3 = $chkyear . '-09-31';
        $xfdate_4 = $chkyear . '-10-01';
        $xtdate_4 = $chkyear . '-10-31';
        $xfdate_5 = $chkyear . '-11-01';
        $xtdate_5 = $chkyear . '-11-31';
        $xfdate_6 = $chkyear . '-12-01';
        $xtdate_6 = $chkyear . '-12-31';
    }
    for ($m = 0; $m < mysql_num_rows($rs); $m++) {
        $sqlObj = mysql_fetch_object($rs);
        $mcode1[$m] = $sqlObj->mcode;
...
function calc_pool_bunus($dbprefix, $ro, $fdate, $tdate)
{
    ...
    $sqlzz = " insert into " . $dbprefix . "embonus  (rcode, mcode, mpos,exp, total,tax,bonus,fdate,tdate,pos_cur,weak) ";
    $sqlzz .= "select '$ro',mcode,mpos,exp,sum(total1+total2+total3+total4+total5) as total,(sum(total1+total2+total3+total4+total5)*0.05) as tax,(sum(total1+total2+total3+total4+total5)-(sum(total1+total2+total3+total4+total5)*0.05)) as bonus,'$fdate','$tdate',pos_cur,weak from " . $dbprefix . "em  where rcode='$ro' group by mcode";
    mysql_query($sqlzz) or die(mysql_error());

    $sqlupdate = "update " . $dbprefix . "embonus as em left join " . $dbprefix . "member as m on (em.mcode=m.mcode) set em.name_t = m.name_t where em.rcode = '$ro' ";
