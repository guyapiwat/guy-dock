...
<?
		...
		$sql = "SELECT a.fdate,a.rcode, a.mcode,m.name_t,m.acc_no,m.bankcode,b.bankname,a.total,a.tax,a.netamt,a.pos_cur,a.percent*100 as percent,a.pv_s,a.pv_o ";
		...
		$rec->setLink($PHP_SELF,"sessiontab=4&sub=19&strfdate=$strfdate&strtdate=$strtdate&fmcode=$fmcode");
		$rec->setBackLink($PHP_SELF,"sessiontab=4");
		if(isset($page))
			$rec->setCurPage($page);
		//$rec->setShowIndex(true);
		$rec->setShowField("fdate,mcode,name_t,acc_no,bankname,percent,total,tax,netamt");
		$rec->setFieldDesc("วันที่,รหัสมาชิก,ชื่อ,เลขบัญชี,ธนาคาร,% โบนัส,รายได้,ภาษี (5%),สุทธิ");
		$rec->setFieldAlign("center,center,left,center,left,right,right,right,right,right,right");
		$rec->setFieldSpace("7%,7%,30%,10%,15%,5%,9%,9%,9%");//10
		$rec->setSum(true,false,",,,,,true,true,true,true,true,true");
		$rec->setFieldFloatFormat(",,,,,2,2,2,2,2,2");
		$rec->setFieldLink("");
		if($_GET['excel']==1){
			$rec->exportXls("ExportXls","bmpersonal".date("Ymd").".xls","SH_QUERY");
			$str = "<fieldset><a href='".$rec->download("ExportXls","bmpersonal".date("Ymd").".xls")."' >";
...
<form name="rform" method="post" action="./index.php?sessiontab=4&sub=19">
