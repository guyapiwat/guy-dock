...
if($fdate != ''){
		...
		$sql = "SELECT *,m.name_t,a.pos_cur as pos,(SELECT pershare FROM ali_em WHERE mcode = a.mcode and fdate >= '".$fdate."' and tdate <= '".$tdate."' ) as percer1 ";
...
//echo $sql;
		...
		$rec->setShowField("fdate,tdate,mcode,name_t,pos,total");
		$rec->setFieldDesc("ตั้งแต่วันที่,ถึงวันที่,รหัสมาชิก,ชื่อ,ตำแหน่ง,คอมมิชชัน");
		$rec->setFieldAlign("center,center,center,left,center,right,right");
		$rec->setFieldSpace("7%,7%,8%,59%,9%,10%");//10
		$rec->setSum(true,false,",,,,,true");
		$rec->setFieldFloatFormat(",,,,,2");
		$rec->setFieldLink(",,,,,index.php?sessiontab=4&sub=904&strfdate=,,");
		if($_GET['excel']==1){
