...
if($fdate != ''){
		...
		$sql = "SELECT DATE_FORMAT(a.fdate, '%Y-%m-%d') as fdate,DATE_FORMAT(a.tdate, '%Y-%m-%d') as tdate,a.rcode, a.mcode,m.name_t,m.acc_no,m.bankcode,a.total,a.tot_pv";
		$sql.=",a.tax,a.bonus,a.mcode as smcode";
		$sql .= " FROM ".$dbprefix."rv_am a ";
		...
		$rec->setShowField("fdate,tdate,mcode,name_t,tot_pv,total");
		$rec->setFieldDesc("ตั้งแต่วันที่,ถึงวันที่,รหัสมาชิก,ชื่อ,ค่าแนะนำ,RV(ค่าแนะนำ)");
		$rec->setFieldAlign("center,center,center,left,right,right,right,right");
		$rec->setFieldSpace("10%,10%,12%,40%,10%,15%,10%,10%,10%,10%");//10
		$rec->setSum(true,false,",,,,true,true,true,true,true");
		$rec->setFieldFloatFormat(",,,,2,2,2,2,2");
		$rec->setFieldLink(",,,,index.php?sessiontab=4&sub=4&fdate=$fdate&tdate=$tdate&strfdate=,,,,,,,");//index.php?sessiontab=4&sub=4&strtdate=$tdate&strfdate=
		if($_GET['excel']==1){
