...
<?
		...
		$sql = "SELECT a.*,a.percer*100  as percer,m.name_t FROM ".$dbprefix."ac2 a left join ".$dbprefix."member as m on (a.mcode=m.mcode) ";
		...
		$rec->setLink($PHP_SELF,"sessiontab=4&sub=5&ftrcode=$ftrcode&strfdate=$strfdate&strtdate=$strtdate&fmcode=$fmcode&cmc=$cmc");
		$rec->setBackLink($PHP_SELF,"sessiontab=4");
		if(isset($page))
			$rec->setCurPage($page);
		$rec->setShowField("rcode,mcode,name_t,upa_code,pv,level,gen,percer,total,pos_cur");
		$rec->setFieldDesc("รอบที่,รหัสสมาชิก,ชื่อสมาชิก,รหัสผู้แนะนำ,คะแนน,ชั้น,Gen,%โบนัส,ได้โบนัส,คุณสมบัติ");
		$rec->setFieldAlign("center,center,left,center,right,center,center,center,right,center");
