...
if($fdate != ''){
		...
		$sql = "SELECT DATE_FORMAT(a.fdate, '%Y-%m-%d') as fdate,DATE_FORMAT(a.tdate, '%Y-%m-%d') as tdate,a.rcode, a.mcode,m.name_t,lb.cshort,a.total,a.tax as tax,a.bonus as bonus,a.pos_cur";
		$sql .= ",CASE m.mtype1 WHEN '0' THEN 'Member' WHEN '1' THEN 'Mobile' WHEN '2' THEN 'Center' END AS mtype1";
		$sql .= " FROM ".$dbprefix."fmbonus a ";
		$sql .= " LEFT JOIN ".$dbprefix."location_base lb ON a.locationbase=lb.cid left join ali_member m on a.mcode=m.mcode";
	...
	//	echo $sql;
		...
		$rec->setShowField("fdate,tdate,mcode,name_t,pos_cur,mtype1,total");
		$rec->setFieldDesc("ตั้งแต่วันที่,ถึงวันที่,รหัสมาชิก,ชื่อ,ตำแหน่ง,ประเภทสมาชิก,".$wording_lan["com"]["3"]."");
		$rec->setFieldAlign("center,center,center,left,center,center,right");
		$rec->setFieldSpace("10%,10%,10%,30%,20%,10%,10%");//10
		$rec->setSum(true,false,",,,,,,true");
		$rec->setFieldFloatFormat(",,,,,,2");
		$rec->setFieldLink(",,,,,,index.php?sessiontab=4&sub=422&strfdate=");
		if($_GET['excel']==1){
