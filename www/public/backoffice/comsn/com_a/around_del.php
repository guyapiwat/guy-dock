...
require_once(dirname(__FILE__)."/../../logtext.php");
	...
	for ($i=0;$i<$numpost;$i++) {
		...
		$rs=mysql_query("SELECT * FROM ".$dbprefix."around WHERE rid='".$postval[$postkey[$i]]."' LIMIT 1");
		...
		logtext(true,$_SESSION['adminuserid'],"delete from ".$dbprefix."around where rid='$rid' ");
		mysql_free_result($rs);
		mysql_query("delete from ".$dbprefix."around where rid='$rid' ");
		mysql_query("COMMIT");
	}
	// แสดงรายการที่ลบ
?>
