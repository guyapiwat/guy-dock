...
<?
if (!empty($_GET['s_mcode'])) {
    $s_mcode = $_GET['s_mcode'];
} else {
    $s_mcode = $_POST['s_mcode'];
}

if (!empty($_GET['s_namef'])) {
    $s_namef = $_GET['s_namef'];
} else {
    $s_namef = $_POST['s_namef'];
}

if (!empty($_GET['s_id_card'])) {
    $s_id_card = $_GET['s_id_card'];
} else {
    $s_id_card = $_POST['s_id_card'];
}

if (!empty($_GET['s_poscur'])) {
    $s_poscur = $_GET['s_poscur'];
} else {
    $s_poscur = $_POST['s_poscur'];
}

if (!empty($_GET['s_poscur1'])) {
    $s_poscur1 = $_GET['s_poscur1'];
} else {
    $s_poscur1 = $_POST['s_poscur1'];
}

if (!empty($_GET['s_upa_code'])) {
    $s_upa_code = $_GET['s_upa_code'];
} else {
    $s_upa_code = $_POST['s_upa_code'];
}

if (!empty($_GET['s_sp_code'])) {
    $s_sp_code = $_GET['s_sp_code'];
} else {
    $s_sp_code = $_POST['s_sp_code'];
}

if (!empty($_GET['s_terminate'])) {
    $s_status_terminate = $_GET['s_terminate'];
} else {
    $s_status_terminate = $_POST['s_terminate'];
}
if (!empty($_GET['s_suspend'])) {
    $s_suspend = $_GET['s_suspend'];
} else {
    $s_suspend = $_POST['s_suspend'];
}

if (!empty($_GET['s_ewallet'])) {
    $s_ewallet = $_GET['s_ewallet'];
} else {
    $s_ewallet = $_POST['s_ewallet'];
}
if (!empty($_GET['v_ewallet'])) {
    $v_ewallet = $_GET['v_ewallet'];
} else {
    $v_ewallet = $_POST['v_ewallet'];
}
if (!empty($_GET['smdate2'])) {
    $smdate2 = $_GET['smdate2'];
} else {
    $smdate2 = $_POST['smdate2'];
}

if (!empty($_GET['emdate2'])) {
    $emdate2 = $_GET['emdate2'];
} else {
    $emdate2 = $_POST['emdate2'];
}

if (!empty($_GET['s_list'])) {
    $s_list = $_GET['s_list'];
} else {
    if (!empty($_POST['s_list'])) {
        $s_list = $_POST['s_list'];
    } else {
        $s_list = '500';
    }
...
if ($_GET['state'] == 1) {
    ...
    $rec->setFieldDesc("รหัสสมาชิก,ชื่อ,เบอร์โทร,รหัสประชาชน,วันที่สมัคร,ตำแหน่ง,ตำแหน่งเกียรติยศ,รหัสผู้แนะนำ,รหัสอัพไลน์,ด้าน,เอกสาร,Suspend,Terminate,EW,EA,EV,HPV,ประเภท");
...
function box_member_search($s_mcode, $s_upa_code, $smdate2, $emdate2, $s_namef, $s_sp_code, $s_id_card, $s_ewallet, $s_poscur, $s_list, $mtype1, $terminate, $v_ewallet)
{
    ...
    <form style="margin: 0px 0px 0px 0px; padding: 0px 0px 0px 0px; text-align: center;" name="searh" id="searh"
          action="index.php?sessiontab=1&sub=2" method="post">
        <table width="100%%" border="0">
            <tr>
                <td width="12%" align="right" scope="row">รหัสสมาชิก</td>
                <td width="15%" align="left"><input type="text" name="s_mcode" id="s_mcode" value="<?= $s_mcode ?>">
                </td>
                <td align="right">&#3619;&#3627;&#3633;&#3626;&#3612;&#3641;&#3657;&#3649;&#3609;&#3632;&#3609;&#3635;
                </td>
...
<input value="<?= $emdate2 ?>" type="text" name="emdate2" id="dateInput2" placeholder="2014-06-05">
            ...
            <tr>
                ...
                <td><input type="text" name="s_ewallet" id="s_ewallet" placeholder="&#3648;&#3594;&#3656;&#3609; 0-100"
                           value="<?= $s_ewallet ?>"></td>
                <td align="right">จำนวนรายการ</td>
                <td align="left"><select name="s_list" id="s_list">
                        <option value="500" <? if ($s_list == '500') {
                            echo 'selected';
                        } ?> >500
                        </option>
                        <option value="1000" <? if ($s_list == '1000') {
                            echo 'selected';
                        } ?>>1000
                        </option>
                        <option value="2000" <? if ($s_list == '2000') {
                            echo 'selected';
                        } ?>>2000
                        </option>
                        <option value="5000" <? if ($s_list == '5000') {
                            echo 'selected';
                        } ?>>5000
                        </option>
                        <option value="99999999999999" <? if ($s_list == 99999999999999) {
                            echo 'selected';
                        } ?>>ทั้งหมด
                        </option>
                    </select></td>
            ...
            <tr>
                <td colspan="6" align="center" scope="row"><input type="submit" value="   ค้นหา   "> &nbsp;&nbsp; <input
                            type="reset" value="   ยกเลิก   "></td>
            </tr>
        </table>
