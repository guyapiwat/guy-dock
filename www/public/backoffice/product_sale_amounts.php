...
echo '<center><font size=4>บิล HQ</font></center>';
...
//$wherecause = " WHERE ";
	...
	//echo $sql;
		...
		//---------------------------------
	//}
echo '<center><font size=4>บิล Branch กรุงเทพ</font></center>';
if (isset($_GET["pg"])){$page=$_GET["pg"];} else {$page="1";}
$sql = "SELECT ".$dbprefix."asaled.pcode,".$dbprefix."asaled.price,";
$sql .= "CASE ifnull(".$dbprefix."product.pdesc,0) WHEN '0' THEN ".$dbprefix."product_package.pdesc ELSE ".$dbprefix."product.pdesc END AS pdesc, ";
...
//$wherecause = " WHERE ";
	...
	//echo $sql;
		...
		//---------------------------------
	//}
echo '<center><font size=4>บิล Branch หาดใหญ่</font></center>';
if (isset($_GET["pg"])){$page=$_GET["pg"];} else {$page="1";}
$sql = "SELECT ".$dbprefix."asaled.pcode,".$dbprefix."asaled.price,";
$sql .= "CASE ifnull(".$dbprefix."product.pdesc,0) WHEN '0' THEN ".$dbprefix."product_package.pdesc ELSE ".$dbprefix."product.pdesc END AS pdesc, ";
