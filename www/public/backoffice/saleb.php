<script language="javascript" type="text/javascript">
	...
	function sale_cancel(id){
		if(confirm("ต้องการยกเลิกบิลนี้")){
			window.location='index.php?sessiontab=3&sub=22&state=3&bid='+id;
		}
	}
</script>
<?
...
$sql = "SELECT cancel,".$dbprefix."asaleh.id,".$dbprefix."asaleh.uid as uid,sano,DATE_FORMAT(".$dbprefix."asaleh.sadate, '%d-%m-%Y') as sadate,tot_pv,tot_bv,tot_fv,total,name_t,".$dbprefix."asaleh.mcode AS smcode";
...
$sql .= "LEFT JOIN ".$dbprefix."member ON (".$dbprefix."asaleh.mcode=".$dbprefix."member.mcode) where ".$dbprefix."asaleh.sa_type = 'Q' "; //WHERE smcode='".$_SESSION['usercode']."' 
//echo $sql;
//$wherecause = " WHERE ";
	...
	if($_GET['state']==1){
		...
		$rec->setShowField("sadate,sano,smcode,name_t,preserve,ability,invent,tot_pv,total,uid");
		...
		$rec->setFieldDesc("วันที่ซื้อ,เลขบิล,รหัสผู้ซื้อ,ชื่อผู้ซื้อ,รักษายอด,ทำคุณสมบัติ,ส่งศูนย์, PV,จำนวนเงินรวม,สาขา หรือ พนักงาน");
		...
		if($acc->isAccess(4)){
			$rec->setDel("index.php","id","id","sessiontab=3&sub=22");
			$rec->setFromDelAttr("maindel","./index.php?sessiontab=3&sub=22&state=1","post","delfield");
		}
		//var_dump($acc->isAccess(2));
		//exit;
		if($acc->isAccess(2))
			$rec->setEdit("index.php","id","id","sessiontab=3&sub=22");
