...
$sql = "SELECT * FROM ".$dbprefix."rsaleh WHERE id='$id' ";
...
for($i=0;$i<sizeof($bill);$i++){
	$sql="SELECT * FROM ".$dbprefix."rsaled WHERE sano='$bill[$i]' ";
	$rs=mysql_query($sql);
	...
	$sumpv = 0;
	for($k=0;$k<$npage;$k++){
	$sql="SELECT * FROM ".$dbprefix."rsaled WHERE sano='$bill[$i]' LIMIT ".($k*$nitem).", $nitem ";
	$rs=mysql_query($sql);
