...
//$sql .= ",CASE sa_type WHEN 'Q' THEN 'รักษายอด' ELSE '' END AS sa_type ";
$sql .= ",CASE sa_type WHEN 'A' THEN 'คุณสมบัติ' ELSE '' END AS sa_type ";
//$sql .= ",CASE sa_type WHEN 'H' THEN 'โอลยอด' ELSE '' END AS sa_type ";
//$sql .= ",CASE sa_type WHEN 'I' THEN 'ส่งศูนย์' ELSE '' END AS sa_type ";
//$sql .= ",CASE sa_type WHEN 'C' THEN 'รักษายอดทันที' ELSE '' END AS sa_type ";
...
//sql .= " WHERE  ".$dbprefix."asaleh.sa_type <> 'W'";
	...
	if($_GET['state']==1){
		...
		$rec->setFieldDesc("เลขที่ใบเสร็จ,วันที่ใบเสร็จ,รหัสสมาชิก,ชื่อสมาชิก,ประเภทการซื้อ,PV, จำนวนเงิน");
