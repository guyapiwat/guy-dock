<script language="javascript" type="text/javascript">
	...
	function hold1(id,page,chktype,sessiontab,sub,send){
			window.location='index.php?sessiontab=6&sub=24&sano='+id+'&page='+page+'&chktype='+chktype+'&send='+send;
	}
	function sale_cancel(id){
			if(confirm("<?=$wording_lan['Bill_21']?>")){
			window.location='index.php?sessiontab=6&sub=17&state=3&bid='+id;
		}
	}
...
if(isset($_POST["fdate"]))
		...
		$fdate = $_GET["fdate"];
$sql = "SELECT '".$_GET["sessiontab"]."' as sessiontab,'".$_GET["sub"]."' as sub,'".$_GET["chktype"]."' as chktype,'".$_GET["send"]."' as send1,".$dbprefix."isaleh.uid_receive,".$dbprefix."isaleh.uid_sender,'".$page."' as page,cancel,print,".$dbprefix."isaleh.id,".$dbprefix."isaleh.sender,".$dbprefix."isaleh.sender_date,".$dbprefix."isaleh.inv_code,".$dbprefix."isaleh.tot_pv/100 as discount,total+".$dbprefix."isaleh.tot_pv/100 as alltotal,sano,sadate,tot_pv,tot_bv,tot_fv,total,".$dbprefix."isaleh.mcode AS smcode ";
...
$sql .= ",".$dbprefix."isaleh.name_t,rpv,rtotal,CASE ".$dbprefix."isaleh.sender WHEN '1' THEN concat('',".$dbprefix."isaleh.sender_date) ELSE '<img src=./images/false.gif>' END AS sender  
...
$sql .= "LEFT JOIN ".$dbprefix."invent ON (".$dbprefix."isaleh.inv_code=".$dbprefix."invent.inv_code) where  sa_type = 'I' and sender <> '1' and receive <> '1' and  send = '1'  "; //WHERE 
if(empty($_GET["chk"]))$sql .= " and ".$dbprefix."isaleh.inv_from= '".$_SESSION["admininvent"]."' and (rpv >= 0 or rtotal >= 0 ) ";
//echo $sql;
...
//echo $sql;
	...
	if($_GET['state']==1){
		...
		include("ssale_editadd_stockist.php");
	}
	else{
		...
		$rec->setShowField("sadate,sano,inv_code,inv_code,address123,mobile,tot_pv,total,rpv,rtotal,checkportal");
...
//		$rec->setFieldDesc("วันที่ซื้อ,เลขบิล,รหัสผู้ซื้อ,ชื่อผู้ซื้อ,ที่อยู่จัดส่ง,มือถือ,รวมpv,รวมราคา,วันส่งของ,รับของ,อ้างอิง,user<br>จัดส่ง,user<br>รับของ,ช่องทาง"); 
	$rec->setFieldDesc($wording_lan["Bill_1"].",".$wording_lan["Bill_2"].",".$wording_lan["Bill_3"].",".$wording_lan["Bill_4"].",".$wording_lan["send_11"].",".$wording_lan["send_12"].",".$wording_lan["Bill_18"].",".$wording_lan["Bill_19"].",PV ค้างส่ง,จำนวนเงินค้างส่ง,".$wording_lan["Bill_17"]);
	
		
		
		
		$rec->setFieldAlign("center,left,left,left,left,center,right,right,center,center,center");
		$rec->setFieldSpace("6%,9%,5%,10%,23%,6%,6%,6%,6%,8%,5%");
