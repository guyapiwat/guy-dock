<script language="javascript" type="text/javascript">
	function view(id){
		var wlink = 'sell_print_product_date1.php?bid='+id;
		window.open(wlink);
	}
	function sale_look(id,rcode){
		var wlink = 'report_stock.php?bid='+id+'&rcode='+rcode;
		window.open(wlink);
}
</script>
<?
...
$sql = "SELECT a.pcode,a.uid,p.pdesc,a.qty,a.qtys,a.inv_code from ".$dbprefix."product_invent_tmp a left join ".$dbprefix."product p on (a.pcode = p.pcode)  where a.qty <> '0'  ";
//echo $sql;
$_GET['excel'] = '1';
		//echo $sql;
	...
	$rec->setLink($PHP_SELF,"sessiontab=".$_GET["sessiontab"]."&sub=53&fpcode=$fpcode&tpcode=$tpcode&fdate=$fdate&tdate=$tdate&satype=$satype&logistic=$logistic&strpv=$strpv&strtotal=$strtotal&struid=$struid&sspv=$sspv&inv_code=$inv_code&inv_code1=$inv_code1&strSearch=$strSearch&strtype=$strtype&sregister=$sregister&okok=1");
